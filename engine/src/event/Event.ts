import {Encoding, NNumber, StringBuilder, NString, Int32, List, ApplicationException, Char, Exception, NObject, RefParam, NumberStyles} from "@magic/mscorelib";
import {XmlParser, StorageAttribute, Logger, Base64, XMLConstants, ForceExit, CallOsShow, StrUtil, Misc, InternalInterface} from "@magic/utils";
import {NUM_TYPE, TaskDefinitionIdTableSaxHandler, RecordUtils, PIC, DisplayConvertor, TaskDefinitionId,
        Modifiers, MenuEntryEvent, MenuEntryOSCommand, GuiMenuEntry_MenuType, KeyboardItem, BlobType} from "@magic/gui";
...
/// </summary>
export class Event extends NObject {
  // ATTENTION !
  ...
  constructor(evtOrMenuEntryEventOrOsCommandOrAType?: any, ctlIdx?: number) {
    ...
    if (arguments.length === 1 && (evtOrMenuEntryEventOrOsCommandOrAType === null || evtOrMenuEntryEventOrOsCommandOrAType instanceof Event)) {
      this.constructor_1(evtOrMenuEntryEventOrOsCommandOrAType);
      return;
    }
    if (arguments.length === 2 && (evtOrMenuEntryEventOrOsCommandOrAType === null || evtOrMenuEntryEventOrOsCommandOrAType instanceof MenuEntryEvent) && (ctlIdx === null || ctlIdx.constructor === Number)) {
      this.constructor_2(evtOrMenuEntryEventOrOsCommandOrAType, ctlIdx);
      return;
    }
    if (arguments.length === 1 && (evtOrMenuEntryEventOrOsCommandOrAType === null || evtOrMenuEntryEventOrOsCommandOrAType instanceof MenuEntryOSCommand)) {
  ...
  fillData(xmlParser: XmlParser, taskRef: Task): void {
    ...
    let endContext: number = NString.IndexOf(xmlParser.getXMLdata(), XMLConstants.TAG_CLOSE, xmlParser.getCurrIndex());
    if (endContext !== -1 && endContext < xmlParser.getXMLdata().length) {
      ...
      xmlParser.add2CurrIndex(NString.IndexOf(tag, ConstInterface.MG_TAG_EVENT) + ConstInterface.MG_TAG_EVENT.length);
  ...
  private InitInnerObjects(xmlParser: XmlParser): void {
    ...
    switch (nextTag) {
      case XMLConstants.MG_TAG_TASKDEFINITIONID_ENTRY:
        ...
        let endContext: number = NString.IndexOf(xmlParser.getXMLdata(), ConstInterface.MG_TAG_EVENT + XMLConstants.TAG_CLOSE, xmlParser.getCurrIndex());
    ...
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      ...
      switch (attribute) {
        ...
        case ConstInterface.MG_TAG_USR_DEF_FUC_RET_EXP_ATTR:
          break;
        default:
          Logger.Instance.WriteExceptionToLog(NString.Format("There is no such tag in Event class. Insert case to Event.initElements for {0}", attribute));
          break;
      }
    }
  ...
  private InitUserData(taskRef: Task, userValue: string): void {
    let comma: number = NString.IndexOf(userValue, ",");
    if (comma > -1) {
      let ueTaskId: string = NString.Substring(userValue, 0, comma);
      if (taskRef !== null)
      ...
      let ueIdx: number = XmlParser.getInt(NString.Substring(userValue, comma + 1));

      if (ueTask !== null) {
        this.UserEvt = ueTask.getUserEvent(ueIdx);
        if (this.UserEvt === null) {
          throw new ApplicationException(NString.Concat([
            "in Event.fillData(): user event not found. task id = ", ueTaskId, ", user event index = ", ueIdx
          ]));
        }
      ...
      switch (this._type) {
        ...
        case ConstInterface.EVENT_TYPE_USER_FUNC:
          if (this._userDefinedFuncNameHashCode === evt.getUserDefinedFuncNameHashCode()) {
            if (NString.Compare(evt.getUserDefinedFuncName(), this._userDefinedFuncName, true))
              bEqual = true;
          }
          break;
  ...
  dataEventIsTrue(): boolean {
    ...
    if (this._type !== ConstInterface.EVENT_TYPE_USER)
      return DisplayConvertor.toBoolean(this.Exp.evaluate(StorageAttribute.BOOLEAN, 0));
    return DisplayConvertor.toBoolean(this.UserEvt.getExpression().evaluate(StorageAttribute.BOOLEAN, 0));
  }
  ...
  private seconds2String(): string {
    ...
    let buffer: string = "";
    time[0] = this._seconds / 3600;
    time[1] = (this._seconds - time[0] * 3600) / 60;
    ...
    for (j = 0; j < this._parameters; j++) {
      switch (<StorageAttribute>this._paramAttrs[j]) {
        ...
        case StorageAttribute.UNICODE:
          endOfs = startOfs + 4;
          sLen = NString.Substring(valueStr, startOfs, (endOfs) - (startOfs));
          ...
          if (ClientManager.Instance.getEnvironment().GetDebugLevel() > 1 || (<StorageAttribute>this._paramAttrs[j]) === StorageAttribute.ALPHA || (<StorageAttribute>this._paramAttrs[j]) === StorageAttribute.UNICODE || (<StorageAttribute>this._paramAttrs[j]) === StorageAttribute.MEMO) {
            ...
            // non spanned params
              ...
              sValue = NString.Substring(valueStr, startOfs, (endOfs) - (startOfs));
            }
          }
          else {
            if ((nLen & 0x8000) > 0)
            // parse spanned record encoded to Hex
            {
              ...
              sValue = Base64.decode(NString.Substring(valueStr, startOfs, (endOfs) - (startOfs)));
            }
          }
        ...
        case StorageAttribute.TIME:
          ...
          if (ClientManager.Instance.getEnvironment().GetDebugLevel() > 1) {
            ...
            sValue = Base64.decodeToHex(NString.Substring(valueStr, startOfs, (endOfs) - (startOfs)));
          }
          break;
        ...
        switch (newType) {
            ...
            if (currType === StorageAttribute.ALPHA) {
              ...
              try {
                ...
                if (exception instanceof Exception) {
                  ...
                  sameType = false;
                  Logger.Instance.WriteExceptionToLog(NString.Format("Event.setParamType() cannot transfer {0} to Numeric with picture {1}", currValue, (pic != null ? pic.getFormat() : "null")));
                }
                else
                  throw exception;
              }
          ...
          case StorageAttribute.BOOLEAN:
            if (currType === StorageAttribute.ALPHA) {
              ...
              currValue = NString.Trim(currValue);
              if (currValue === "1" || NString.Compare(currValue, "true", true))
  ...
  setUserDefinedFuncName(name: string): void {
    ...
    this._userDefinedFuncNameHashCode = NUM_TYPE.hash_str_new(Encoding.Default.GetBytes(functionNameUpper));
  }
