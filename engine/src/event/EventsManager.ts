import {
ForceExit, HelpType, MgControlType, Priority, RaiseAt, StorageAttribute,
WindowType, MsgInterface, InternalInterface, Logger, Misc, SubformType, Constants
} from "@magic/utils";
import {
ApplicationException, Int32, List, NNumber, NObject, NString, Stack,
Thread, Timer, RefParam
...
export class EventsManager implements IEventsManager {
  ...
  HandleNonParkableControls(itask: ITask): void {
    ...
    if (task.getMode() === Constants.TASK_MODE_CREATE && task.IsInteractive) {
      ...
      this.handleInternalEvent(task, InternalInterface.MG_ACT_REC_SUFFIX);
      if (!this.GetStopExecutionFlag()) {
        if (oldMode !== Constants.TASK_MODE_CREATE) {
          ...
          this.handleInternalEvent(task, InternalInterface.MG_ACT_REC_PREFIX);
      ...
      if (bRcBefore) {
        ...
        while (handler !== null) {
          ...
          try {
            ...
            // of the handler
            if (!NObject.GenericEquals(rtEvt, handler.getEvent())) {
  ...
  simulateSelection(ctrl: MgControlBase, val: string, line: number, produceClick: boolean): void {
    ...
    if (mgControl.Type !== MgControlType.CTRL_TYPE_BUTTON || (mgControl.getField() === null || !mgControl.checkProp(PropInterface.PROP_TYPE_PARK_ON_CLICK, false, line) || controlIsParkable)) {
  ...
  handleInternalEvent(itask: ITask, eventCode: number): void;
  handleInternalEvent(itask: ITask, eventCode: number, subformRefresh: boolean): void;
  handleInternalEvent(ctrl: MgControlBase, eventCode: number): void;
  handleInternalEvent(task: Task, eventCode: number, eventSubType: EventSubType): void;
  handleInternalEvent(ctrl: MgControl, eventCode: number, eventSubType: EventSubType): void;
  handleInternalEvent(itaskOrCtrlOrTask: any, eventCode: number, subformRefreshOrEventSubType?: any): void {
    if (arguments.length === 2 && (eventCode === null || eventCode.constructor === Number)) {
      this.handleInternalEvent_0(itaskOrCtrlOrTask, eventCode);
      return;
    }
    if (arguments.length === 3 && (eventCode === null || eventCode.constructor === Number) && (subformRefreshOrEventSubType === null || subformRefreshOrEventSubType.constructor === Boolean)) {
      this.handleInternalEvent_1(itaskOrCtrlOrTask, eventCode, subformRefreshOrEventSubType);
      return;
    }
    if (arguments.length === 2 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (eventCode === null || eventCode.constructor === Number)) {
      this.handleInternalEvent_2(itaskOrCtrlOrTask, eventCode);
      return;
    }
    if (arguments.length === 3 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof Task) && (eventCode === null || eventCode.constructor === Number) && (subformRefreshOrEventSubType === null || subformRefreshOrEventSubType.constructor === Number)) {
      this.handleInternalEvent_3(itaskOrCtrlOrTask, eventCode, subformRefreshOrEventSubType);
      return;
    }
    this.handleInternalEvent_4(itaskOrCtrlOrTask, eventCode, subformRefreshOrEventSubType);
  }
  ...
  private handleInternalEvent_0(itask: ITask, eventCode: number): void {
    this.handleInternalEvent(itask, eventCode, false);
  }
  ...
  private handleInternalEvent_1(itask: ITask, eventCode: number, subformRefresh: boolean): void {
    let task: Task = <Task>itask;
    ...
    if (subformRefresh)
      this.handleInternalEvent(itask, InternalInterface.MG_ACT_POST_REFRESH_BY_PARENT);
    ...
    if (task.DataView.isEmptyDataview()) {
      if (eventCode === InternalInterface.MG_ACT_REC_PREFIX || eventCode === InternalInterface.MG_ACT_REC_SUFFIX) {
        ...
        this.handleInternalEvent(task, eventCode, EventSubType.Normal);
    }
    else
      this.handleInternalEvent(task, eventCode, EventSubType.Normal);
  }
  ...
  /// <param name = "eventCode">the code of the event </param>
  private handleInternalEvent_2(ctrl: MgControlBase, eventCode: number): void {
    this.handleInternalEvent(<MgControl>ctrl, eventCode, EventSubType.Normal);
  }
  ...
  buildSubformPath(pathArray: List<MgControlBase>, frmCtrl: MgControlBase, toCtrl: MgControlBase): MgControlBase {
    ...
    if (parentSubformCtrl === null) {
      ...
      while (parentSubformCtrl !== null && (<MgControl>toCtrl).onDiffForm(parentSubformCtrl)) {
        for (let index: number = 0; index < pathArray.Count; index++) {
  ...
  exitWithError(itask: ITask, msgId: string): void {
    ...
    this.handleInternalEvent(task, InternalInterface.MG_ACT_EXIT);
  }
  ...
  addInternalEvent(itask: ITask, code: number): void;
  addInternalEvent(ctrl: MgControlBase, code: number): void;
  ...
  addInternalEvent(itaskOrCtrl: any, codeOrDisplayLine: number, codeOrPriority?: any): void {
    if (arguments.length === 2 && (codeOrDisplayLine === null || codeOrDisplayLine.constructor === Number)) {
      this.addInternalEvent_0(itaskOrCtrl, codeOrDisplayLine);
      return;
    }
    if (arguments.length === 2 && (itaskOrCtrl === null || itaskOrCtrl instanceof MgControlBase) && (codeOrDisplayLine === null || codeOrDisplayLine.constructor === Number)) {
      this.addInternalEvent_1(itaskOrCtrl, codeOrDisplayLine);
      return;
    }
    if (arguments.length === 3 && (itaskOrCtrl === null || itaskOrCtrl instanceof MgControlBase) && (codeOrDisplayLine === null || codeOrDisplayLine.constructor === Number) && (codeOrPriority === null || codeOrPriority.constructor === Number)) {
      this.addInternalEvent_2(itaskOrCtrl, codeOrDisplayLine, codeOrPriority);
      return;
    }
    this.addInternalEvent_3(itaskOrCtrl, codeOrDisplayLine, codeOrPriority);
  }
  ...
  private addInternalEvent_0(itask: ITask, code: number): void {
    this.addGuiTriggeredEvent(itask, code, false);
  }

  addGuiTriggeredEvent(itask: ITask, code: number, onMultiMark: boolean): void;
  addGuiTriggeredEvent(ctrl: MgControlBase, code: number, line: number, modifiers: Modifiers): void;
  addGuiTriggeredEvent(ctrl: MgControlBase, code: number, line: number): void;
  addGuiTriggeredEvent(ctrl: MgControlBase, line: number, code: number, isProduceClick: boolean): void;
  addGuiTriggeredEvent(ctrl: MgControlBase, code: number, list: List<MgControlBase>): void;
  ...
  addGuiTriggeredEvent(itaskOrCtrlOrTask: any, codeOrLine: number, onMultiMarkOrLineOrCodeOrListOrRaisedBy?: any, modifiersOrIsProduceClickOrDotNetArgs?: any, onMultimarkOrRaisedBy?: any, onMultimark?: boolean): void {
    if (arguments.length === 3 && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Boolean)) {
      this.addGuiTriggeredEvent_0(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy);
      return;
    }
    if (arguments.length === 4 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Number) && (modifiersOrIsProduceClickOrDotNetArgs === null || modifiersOrIsProduceClickOrDotNetArgs.constructor === Number)) {
      this.addGuiTriggeredEvent_1(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy, modifiersOrIsProduceClickOrDotNetArgs);
      return;
    }
    if (arguments.length === 3 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Number)) {
      this.addGuiTriggeredEvent_2(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy);
      return;
    }
    if (arguments.length === 4 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Number) && (modifiersOrIsProduceClickOrDotNetArgs === null || modifiersOrIsProduceClickOrDotNetArgs.constructor === Boolean)) {
      this.addGuiTriggeredEvent_3(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy, modifiersOrIsProduceClickOrDotNetArgs);
      return;
    }
    if (arguments.length === 3 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy instanceof List)) {
      this.addGuiTriggeredEvent_4(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy);
      return;
    }
    if (arguments.length === 5 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Number) && (modifiersOrIsProduceClickOrDotNetArgs === null || modifiersOrIsProduceClickOrDotNetArgs instanceof Array) && (onMultimarkOrRaisedBy === null || onMultimarkOrRaisedBy.constructor === Boolean)) {
      this.addGuiTriggeredEvent_5(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy, modifiersOrIsProduceClickOrDotNetArgs, onMultimarkOrRaisedBy);
      return;
    }
    if (arguments.length === 6 && (itaskOrCtrlOrTask === null || itaskOrCtrlOrTask instanceof MgControlBase) && (codeOrLine === null || codeOrLine.constructor === Number) && (onMultiMarkOrLineOrCodeOrListOrRaisedBy === null || onMultiMarkOrLineOrCodeOrListOrRaisedBy.constructor === Number) && (modifiersOrIsProduceClickOrDotNetArgs === null || modifiersOrIsProduceClickOrDotNetArgs instanceof Array) && (onMultimarkOrRaisedBy === null || onMultimarkOrRaisedBy.constructor === Number) && (onMultimark === null || onMultimark.constructor === Boolean)) {
      this.addGuiTriggeredEvent_6(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy, modifiersOrIsProduceClickOrDotNetArgs, onMultimarkOrRaisedBy, onMultimark);
      return;
    }
    if (arguments.length === 2 && (codeOrLine === null || codeOrLine.constructor === Number)) {
      this.addGuiTriggeredEvent_7(itaskOrCtrlOrTask, codeOrLine);
      return;
    }
    this.addGuiTriggeredEvent_8(itaskOrCtrlOrTask, codeOrLine, onMultiMarkOrLineOrCodeOrListOrRaisedBy);
  }
  ...
  private addGuiTriggeredEvent_0(itask: ITask, code: number, onMultiMark: boolean): void {
    let task: Task = <Task>itask;
  ...
  /// <param name = "code"></param>
  /// <returns></returns>
  private addInternalEvent_1(ctrl: MgControlBase, code: number): void {
    let rtEvt: RunTimeEvent = new RunTimeEvent(<MgControl>ctrl);
    rtEvt.setInternal(code);
    this.addToTail(rtEvt);
  }

  private addInternalEvent_2(ctrl: MgControlBase, DisplayLine: number, code: number): void {
  ...
  private addInternalEvent_3(ctrl: MgControlBase, code: number, priority: Priority): void {
    let rtEvt: RunTimeEvent = new RunTimeEvent(<MgControl>ctrl);
    rtEvt.setInternal(code);
  ...
  private addGuiTriggeredEvent_1(ctrl: MgControlBase, code: number, line: number, modifiers: Modifiers): void {
  ...
  private addGuiTriggeredEvent_2(ctrl: MgControlBase, code: number, line: number): void {
  ...
  /// <param name = "code">code of internal event </param>
  private addGuiTriggeredEvent_3(ctrl: MgControlBase, line: number, code: number, isProduceClick: boolean): void {
    let rtEvt: RunTimeEvent = new RunTimeEvent(<MgControl>ctrl, line, true);
    rtEvt.setInternal(code);
  ...
  private addGuiTriggeredEvent_4(ctrl: MgControlBase, code: number, list: List<MgControlBase>): void {
  ...
  private addGuiTriggeredEvent_5(ctrl: MgControlBase, code: number, line: number, dotNetArgs: any[], onMultimark: boolean): void {
    let rtEvt: RunTimeEvent = new RunTimeEvent(<MgControl>ctrl, true);
    rtEvt.setInternal(code);
    this.addToTail(rtEvt);
  }
  ...
  private addGuiTriggeredEvent_6(ctrl: MgControlBase, code: number, line: number, dotNetArgs: any[], raisedBy: RaisedBy, onMultimark: boolean): void {
    let rtEvt: RunTimeEvent = new RunTimeEvent(<MgControl>ctrl, true);
    rtEvt.setInternal(code);
    this.addToTail(rtEvt);
  }
  ...
  /// <param name = "code">code of internal event </param>
  private addGuiTriggeredEvent_7(task: ITask, code: number): void {
  ...
  private addGuiTriggeredEvent_8(task: ITask, code: number, raisedBy: RaisedBy): void {
    this.addGuiTriggeredEvent(task, code);
  }
  ...
  NonInteractiveEventsLoop(mgData: MGData, task: Task): void {
    ...
    this.pushNewExecStacks();
      ...
      if (task.getMode() === Constants.TASK_MODE_DELETE)
        this.handleInternalEvent(task, InternalInterface.MG_ACT_CYCLE_NEXT_DELETE_REC);
      else
        this.handleInternalEvent(task, 1017);
        ...
        case InternalInterface.MG_ACT_CTRL_PREFIX:
          ...
          // only for the first time
          {
            ...
            if (!GUIManager.Instance.confirm(<MgForm>task.getForm(), MsgInterface.CONFIRM_STR_CANCEL)) {
              ...
              this.handleInternalEvent(prevCtrl, InternalInterface.MG_ACT_CTRL_PREFIX);
            }
            endTaskDueToVerifyError = true;
          }
          else {
            ...
            if (nextCtrl !== null) {
              ...
              this.handleInternalEvent(nextCtrl, InternalInterface.MG_ACT_CTRL_PREFIX);
            }
            else
        ...
        case InternalInterface.MG_ACT_CTRL_VERIFICATION:
          if (currCtrl !== GUIManager.getLastFocusedControl() && task.getCurrVerifyCtrl() !== currCtrl) {
            ...
            if (ClientManager.Instance.MoveByTab && !currCtrl.checkProp(PropInterface.PROP_TYPE_TAB_IN, true) &&
              lastParkedCtrl != null)
              this.handleInternalEvent(lastParkedCtrl, InternalInterface.MG_ACT_CTRL_PREFIX);
            else
              this.handleInternalEvent(currCtrl, InternalInterface.MG_ACT_CTRL_PREFIX);
            this.setStopExecution(true);
          }
          break;
      }
  ...
  private handleForceExitBefore(evt: Event, rtEvt: RunTimeEvent): boolean {
    ...
    if (forceExit !== ForceExit.Editing && task.getLevel() === Constants.TASK_LEVEL_CONTROL) {
      let ctrl: MgControl = <MgControl>task.getLastParkedCtrl();
      if (ctrl !== null)
        this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_CTRL_SUFFIX);
    }
  ...
  private handleForceExitAfter(rtEvt: RunTimeEvent): void {
    ...
    if (ctrl !== null && forceExit !== ForceExit.Editing) {
      this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_CTRL_PREFIX);
    }
  ...
  private handleSelection(ctrl: MgControl, line: number, val: string): void {
  ...
  private handleCtrlPrefix(evt: RunTimeEvent): boolean {
    ...
    if (lastParkedCtrl !== null && (<Task>lastParkedCtrl.getForm().getTask()).getMGData().GetId() === (<Task>ctrl.getForm().getTask()).getMGData().GetId() && lastParkedCtrl.getForm().getTask().getLevel() === Constants.TASK_LEVEL_CONTROL) {
      ...
      // QCR #970806
      {
        this.handleInternalEvent(lastParkedCtrl, InternalInterface.MG_ACT_CTRL_SUFFIX);
        if (this._stopExecution) {
  ...
  private commonHandlerBefore(evt: RunTimeEvent): boolean {
    ...
    if (evt.getType() === ConstInterface.EVENT_TYPE_SYSTEM) {
      ...
      // && actId != MG_ACT_SELECT)
      {
        ...
        evt.setArgList(null);
        let mgControl: MgControl = this.exitWide(evt);
    ...
    if (evt.getType() === ConstInterface.EVENT_TYPE_INTERNAL) {
      ...
      // note that the method exitWide() is change the event (ctrl & internal action)
      let ctrl: MgControl = this.exitWide(evt);
      ...
      switch (intEvtCode) {
          ...
          if (task.IsSubForm && task.PerformParentRecordPrefix) {
            ...
            while (parentTask.IsSubForm) {
              if (parentTask.getLevel() === Constants.TASK_LEVEL_TASK) {
                this.handleInternalEvent(parentTask, InternalInterface.MG_ACT_REC_PREFIX);
        ...
        case InternalInterface.MG_ACT_REC_SUFFIX:
          ...
          if (task.getLevel() === Constants.TASK_LEVEL_CONTROL) {
            ...
            task.setDirection(Task_Direction.FORE);
            this.handleInternalEvent(task.getLastParkedCtrl(), InternalInterface.MG_ACT_CTRL_SUFFIX);
            task.setDirection(Task_Direction.NONE);
        ...
        switch (intEvtCode) {
          ...
          case InternalInterface.MG_ACT_CTRL_HIT:
            ...
            if (ctrl.Type === MgControlType.CTRL_TYPE_BUTTON &&
              ...
              this.addInternalEvent(ctrl, InternalInterface.MG_ACT_ZOOM);
            }
          ...
          case InternalInterface.MG_ACT_EDT_ENDLINE:
            ...
            if (form != null && task.getLastParkedCtrl() != null) {
              ...
              let ret: EditReturnCode = Manager.TextMaskEditor.ProcessAction(mgControl, intEvtCode,
              ...
              if (ret === EditReturnCode.EDIT_AUTOSKIP)
                this.handleInternalEvent(task, InternalInterface.MG_ACT_TBL_NXTFLD);
          ...
          case InternalInterface.MG_ACT_WEB_ON_DBLICK: {
            ...
            if (ctrlType !== MgControlType.CTRL_TYPE_TABLE && ctrlType !== MgControlType.CTRL_TYPE_LABEL &&
              ...
              if (putAction !== InternalInterface.MG_ACT_NONE)
                this.addInternalEvent(ctrl, putAction);
            }
          }
            break;
          ...
          case InternalInterface.MG_ACT_RT_QUIT:
            this.handleInternalEvent(task, InternalInterface.MG_ACT_RT_QUIT);
            break;
          ...
          case InternalInterface.MG_ACT_RTO_QUERY:
            ...
            if (!this.GetStopExecutionFlag()) {
              ...
              if (oldMode !== Constants.TASK_MODE_CREATE) {
                ...
                this.handleInternalEvent(task, InternalInterface.MG_ACT_RT_REFRESH_VIEW);
                if (ctrl != null)
                  ctrl.invoke();
          ...
          case InternalInterface.MG_ACT_WIDE:
            ...
            let winId: number = task.getMGData().GetId();
          ...
          case InternalInterface.MG_ACT_COL_SORT:
            ...
            if ((intEvtCode === InternalInterface.MG_ACT_INDEX_CHANGE && task.DataviewManager.HasRemoteData) ||
              ...
              if (focusedCtrl != null)
                this.handleInternalEvent(focusedCtrl, InternalInterface.MG_ACT_CTRL_SUFFIX);
              ...
              if (!this.GetStopExecutionFlag()) {
                ...
                if (task === ClientManager.Instance.getLastFocusedTask() || !task.pathContains(ClientManager.Instance.getLastFocusedTask())) {
                  this.handleInternalEvent(task, InternalInterface.MG_ACT_TBL_BEGLINE);
                }
                else {
                  this.handleInternalEvent(task, InternalInterface.MG_ACT_REC_PREFIX);
          ...
          if (!this.GetStopExecutionFlag()) {
            ...
            if (startupMgDataClosed) {
              ...
              // something went wrong with the opening of the new task (which is supposed to be our new startup task). close all.
                this.handleInternalEvent(creatorTask, InternalInterface.MG_ACT_EXIT);
            }
          }
        }
        else {
  ...
  private HandleActCancel(evt: RunTimeEvent, task: Task, form: MgForm, ctrl: MgControl, rec: Record, isMG_ACT_CANCEL: boolean): void {
    ...
    if (evt.isQuit())
      this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_EXIT);
  ...
  private addLocateCharacter(evt: RunTimeEvent): void {
    ...
    task.locateQuery.Timer = new Timer(function () {
      this.incLocateTimerCB();
    }, task, ConstInterface.INCREMENTAL_LOCATE_TIMEOUT, EventsManager.Timer_Timeout_Infinite);
  }
          ...
          if (this._stopExecution) {
            ...
            if (task.getLevel() !== Constants.TASK_LEVEL_CONTROL) {
              ...
              if (last != null) {
                ...
                task.InCtrlPrefix = false;
                this.handleInternalEvent(last, InternalInterface.MG_ACT_CTRL_PREFIX);
  ...
  private handleInternalEvent_3(task: Task, eventCode: number, eventSubType: EventSubType): void {
    ...
    if (lastParkedCtrl !== null)
      this.handleInternalEvent(lastParkedCtrl, eventCode, eventSubType);

    else {
      if (eventCode === InternalInterface.MG_ACT_RT_QUIT)
        this.handleInternalEvent(task, InternalInterface.MG_ACT_CANCEL, EventSubType.CancelIsQuit);
      else {
  ...
  /// <param name = "eventCode">the code of the event</param>
  /// <param name = "isQuit">indicates whether MG_ACT_CANCEL is called from MG_ACT_RT_QUIT</param>
  private handleInternalEvent_4(ctrl: MgControl, eventCode: number, eventSubType: EventSubType): void {
    let rtEvt: RunTimeEvent;

    if (eventCode === InternalInterface.MG_ACT_RT_QUIT)
      this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_CANCEL, EventSubType.CancelIsQuit);
    else {
  ...
  setStopExecution(stop: boolean, clearSrvrEvents?: boolean): void {
    if (arguments.length === 1 && (stop === null || stop.constructor === Boolean)) {
  ...
  private setStopExecution_1(stop: boolean, clearSrvrEvents: boolean): void {
    if (stop)
      if (clearSrvrEvents)
        ...
        for (i = 0; i < tmpVec.Count; i++)
          this._eventsQueue.put(tmpVec[i]);
      }
      ...
      if (ctrl.IsParkable(false)) {
        ...
        if (prompt) {
          if (selectMode === Constants.SELPRG_MODE_PROMPT) {
            ...
            this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_ZOOM);
            progExecuted = true;
          ...
          if (!ctrl.validateAndSetValue(value, false))
            ...
            if (selectMode === Constants.SELPRG_MODE_AFTER)
              this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_TBL_NXTFLD);
            progExecuted = true;
          }
        }
      }
  ...
  private executeVerifyHandlerSubforms(task: Task, destCtrl: MgControl): boolean {
    ...
    // in subformctrl's form.
    for (let index: number = subformPathList.Count - 1; bRc && index >= 0; index--) {
  ...
  private execSubformRecPrefix(subformCtrl: MgControl, destCtrl: MgControl): void {
    ...
    this.handleInternalEvent(subformCtrlTask, InternalInterface.MG_ACT_REC_PREFIX);
  }
  ...
  private executeAllRecordPrefix(parentTask: Task, childTask: Task, destCtrl: MgControl, syncParent: boolean): void {
    ...
    if (task != null) {
      ...
      for (let index: number = taskPathList.Count - 1; index >= 0 && bRc; index--) {
        let pTask: Task = taskPathList[index];

        if (syncParent)
          EventsManager.doSyncForSubformParent(pTask);

        this.handleInternalEvent(pTask, InternalInterface.MG_ACT_REC_PREFIX);
        if (this.GetStopExecutionFlag())
          break;
  ...
  private executeVerifyHandler(ctrl: MgControl): boolean {
    let bRc: boolean = true;
    this.handleInternalEvent(ctrl, InternalInterface.MG_ACT_CTRL_VERIFICATION);
    if (this.GetStopExecutionFlag())
      return false;
  ...
  public getMatchingAction(task: Task, kbItm: KeyboardItem, checkAccessKeyInMenus: boolean): number {
    ...
    if (kbItm.getAction() === InternalInterface.MG_ACT_CHAR)
      ...
      if (kbItemArray != null) {
        for (let i: number = 0; i < kbItemArray.Count; i++) {
          let kbdItem: KeyboardItem = kbItemArray[i];
  ...
  getNextOperIdx(oper: Operation, clearWhenFound: boolean): number {
    ...
    if (this._serverExecStack.count() !== 0 && this._serverExecStack.peek() !== null && !(<ExecutionStack>this._serverExecStack.peek()).empty()) {
      ...
      if ((<ExecutionStack>this._serverExecStack.peek()).size() > (<ExecutionStack>this._execStack.peek()).size()) {
        ...
        if (NObject.GenericEquals(this._execStack.peek(), tmpServerExecStack) && tmpServerExecStackEntry.TaskId === oper.getTaskTag() && tmpServerExecStackEntry.HandlerId === oper.getHandlerId() && tmpServerExecStackEntry.OperIdx >= oper.getServerId()) {
  ...
  /// <param name = "evt"> </param>
  /// <returns> </returns>
  private exitWide(evt: RunTimeEvent): MgControl {
    let intEvtCode: number = evt.getInternalCode();
    let mgControl: MgControl = evt.Control;

    if (mgControl !== null) {
      let exitWide: boolean;
      switch (intEvtCode) {
      ...
      let topMostForm = <MgForm>mgControl.getTopMostForm();
      let wideParentControl = <MgControl>topMostForm.getWideParentControl();
        ...
        do
        {
          ...
          if (childTsk != null && !childTsk.isAborting()) {
            if (childTsk.IsSubForm) {
              ...
              this.handleInternalEvent(childTsk, InternalInterface.MG_ACT_EXIT);
