...
export class EventHandlerPosition {
  ...
  getNext(evt?: RunTimeEvent): EventHandler {
    if (arguments.length === 1 && (evt === null || evt instanceof RunTimeEvent)) {
  ...
  private getNext_1(): EventHandler {
    ...
    while (this.setNextHandlerIdx()) {
      ...
      switch (this._phase) {
        ...
        default:
          Logger.Instance.WriteExceptionToLog("in EventHandlerPosition.getNext() invalid phase: " + this._phase);
          break;
      }
    }
  ...
  private goUpTaskChain(): boolean {
    ...
    switch (this._phase) {
      ...
      default:
        Logger.Instance.WriteExceptionToLog("in EventHandlerPosition.goUpTaskChain() invalid phase: " + this._phase);
        break;
    }
    if (this._task === null)
      return false;
