...
export class UserEventsTable {
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, tsk: Task): boolean {
    ...
    switch (foundTagName) {
      ...
      default:
        Logger.Instance.WriteExceptionToLog("There is no such tag in UserEventsTable.initInnerObjects : " + foundTagName);
        return false;
    }
    return true;
  }
  ...
  getEvent(idx: number): Event {
    ...
    if (idx < 0 || this._userEventsTab === null || idx >= this._userEventsTab.Count)
      return null;
  ...
  getSize(): number {
    ...
    if (this._userEventsTab === null)
      return 0;
    return this._userEventsTab.Count;
  }
}
