...
import {Constants, Logger, OSEnvironment, UtilDateJpn, UtilImeJpn, XMLConstants, XmlParser} from "@magic/utils";
...
export class Environment implements IEnvironment {
  ...
  private _useWindowsXpThemes: boolean = false; // hould we use XP themes
  ...
  public fillData_0(): void {
    ...
    else Logger.Instance.WriteExceptionToLog("in Environment.FillData() out of string bounds");
  }
  ...
  private fillData_1(tokensVector: List<string>): void {
    ...
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      ...
      switch (attribute) {
        ...
        case ConstInterface.MG_ATTR_USE_WINDOWS_XP_THEAMS:
          this._useWindowsXpThemes = XmlParser.getBoolean(valueStr);
          break;
    ...
    hashKey = env.CompIdx;
    this._environments[hashKey] = env;
  ...
  fillFromUrl(tagName: string): void {
    ...
    if (endContext !== -1 && endContext < XMLdata.length) {
      ...
      let cachedFileUrl: string = (tokensVector[1]);
      ...
      // in which case the current error message will be matched with an error in the xpa server's log file (GeneralErrorLog=)).
        ...
        try {
          ...
          switch (tagName) {
            case ConstInterface.MG_TAG_COLORTABLE_URL:
              Logger.Instance.WriteExceptionToLog(NString.Format("Colors Table: '{0}'{1}{2}", cachedFileUrl, OSEnvironment.EolSeq, ex.Message));
              break;
            case ConstInterface.MG_TAG_FONTTABLE_URL:
              Logger.Instance.WriteExceptionToLog(NString.Format("Fonts Table: '{0}'{1}{2}", cachedFileUrl, OSEnvironment.EolSeq, ex.Message));
              break;
            case ConstInterface.MG_TAG_KBDMAP_URL:
              Logger.Instance.WriteExceptionToLog(NString.Format("Keyboard Mapping: '{0}'{1}{2}", cachedFileUrl, OSEnvironment.EolSeq, ex.Message));
              break;
            case ConstInterface.MG_TAG_ENV_PARAM_URL:
              Logger.Instance.WriteExceptionToLog(NString.Format("Env Params Table: '{0}'{1}{2}", cachedFileUrl, OSEnvironment.EolSeq, ex.Message));
          }
        }
  ...
  GetLocalFlag(f: string): boolean {
    return this._localFlags !== null && NString.IndexOf(this._localFlags, f) >= 0;
  }
