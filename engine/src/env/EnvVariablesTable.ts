...
import {ParamParseResult} from "../util/PrmMap";
import {
  Debug, NNumber, NObject,
  NString, List, StringBuilder
} from "@magic/mscorelib";

import {DisplayConvertor} from "@magic/gui";
import {MirrorPrmMap} from "../../index";
...
export class MirrorString extends NObject implements IMirrorXML {
...
export class EnvParamsTable extends MirrorPrmMap<MirrorString> {
  ...
  get(s: string): string {
    ...
    try {
      let resultObject: any = super.getvalue(s);
  ...
  private getAllowedSeparators(name: string): number {
    ...
    let section: string = NString.Substring(name, 1, NString.IndexOf(name, ']') /*']'*/ - 1);
    let param: string = NString.Substring(name, NString.IndexOf(name, ']') /*']'*/ + 1);
