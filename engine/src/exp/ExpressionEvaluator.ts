...
import {HttpUtility} from "@magic/httpClient";
...
export class ExpressionEvaluator extends GuiExpressionEvaluator {
  ...
  private execOperation(opCode: number, expStrTracker: ExpStrTracker, valStack: Stack<any>, addedOpers: List<DynamicOperation>, expectedType: StorageAttribute): void {
    ...
    switch (opCode) {
      ...
      case ExpressionInterface.EXP_OP_RIGHTS:
        ...
        this.eval_op_rights(resVal, val1);
        break;
      ...
      case ExpressionInterface.EXP_OP_COLOR_SET:
        ...
        val2 = <ExpVal>valStack.pop();
        val1 = <ExpVal>valStack.pop();
        this.eval_op_ColorSet(val1, val2, val3, resVal);
        break;
      ...
      case ExpressionInterface.EXP_OP_FONT_SET:
        ...
        val2 = <ExpVal>valStack.pop();
        val1 = <ExpVal>valStack.pop();
        this.eval_op_FontSet(val1, val2, val3, val4, val5, val6, val7, val8, val9, resVal);
        break;
  ...
  private execExpression(expStrTracker: ExpStrTracker, valStack: Stack<any>, expectedType: StorageAttribute): void {
    ...
    let nDynOpers: number = addedOpers.Count;
    if (nDynOpers > 0) {
    ...
    switch (mode) {
      case 0:
        ...
        for (pos = val1.StrVal.length; pos >= 1; pos--) {
          c_str = NString.Substring(val1.StrVal, pos - 1);
      ...
      case 1:
        for (pos = val1.StrVal.length; pos >= 1; pos--) {
          ...
          let c_char: string = NChar.ToUpper(val1.StrVal[pos - 1]);
          c_str = NString.Substring(val1.StrVal, pos - 1);
  ...
  private eval_op_soundx(resVal: ExpVal, val1: ExpVal): void {
    ...
    for (let inpos: number = 0; inpos < val1.StrVal.length; inpos++) {
      let inc: string = NChar.ToUpper(val1.StrVal.charAt(inpos));
      let outc: string = NChar.IsUpper(inc) ? soundx_vals[(inc.charCodeAt(0) - 'A'.charCodeAt(0))] : '0';
      if (inpos === 0)
  ...
  private hex_2_long(str: String, strCount: number, len: number): number {
    ...
    for (pos = strCount; pos < strCount + len; pos++) {
      let digit: string = NChar.ToUpper(str.charAt(pos));
  ...
  private eval_op_chr(resVal: ExpVal, val1: ExpVal): void {
    ...
    let c = Encoding.Default.GetChars(BitConverter.GetBytes(val1.MgNumVal.NUM_2_LONG()))[0];
    resVal.StrVal = "" + c;
  }
  ...
  private eval_op_asc(resVal: ExpVal, val1: ExpVal): void {
    ...
    if (val1.StrVal.length !== 0) {
      let chararray: string[] = [val1.StrVal[0]];
  ...
  private eval_op_addDateTime(resVal: ExpVal, dateVal: ExpVal, timeVal: ExpVal, yearsVal: ExpVal,
    ...
    let date: number = time / (60 * 60 * 24); // if diff is more then 24 hours, remove the full date changes
  ...
  private eval_op_idle(resVal: ExpVal): void {
    ...
    if (idleTime > 0) {
      ...
      // act_idle ():
      n = (<number>(CurrTimeMilli - ClientManager.Instance.LastActionTime) / 1000) / idleTime;
    }
    resVal.MgNumVal = new NUM_TYPE();
    resVal.Attr = StorageAttribute.NUMERIC;
      ...
      if (idx > 0) {
        ...
        if (delim.length === 0) {
          ...
          for (i = 0; i < idx; i++) {
            ...
            ret_str = NString.Substring(tmp_str, 0, tmp_str.length - ret_str.length);
            tmp_str = NString.Substring(tmp_str, ret_str.length + delim.length);
          }
        }
      }
    }
  ...
  private exp_get_var(resValOrVal1: ExpVal, val1OrResVal: ExpVal, is_previous?: boolean): void {
    if (arguments.length === 3 && (resValOrVal1 === null || resValOrVal1 instanceof ExpVal) && (val1OrResVal === null || val1OrResVal instanceof ExpVal) && (is_previous === null || is_previous.constructor === Boolean)) {
  ...
  private eval_op_user(resVal: ExpVal, numVal: ExpVal): void {
    let option: number = numVal.MgNumVal.NUM_2_LONG();
    resVal.Attr = StorageAttribute.ALPHA;
  ...
  private eval_op_stat(resVal: ExpVal, Parent: ExpVal, Modes: ExpVal): void {
    ...
    if ((iParent >= 0 && iParent < (<Task>this.ExpTask).getTaskDepth(false)) || iParent === ExpressionEvaluator.TRIGGER_TASK) {
      ...
      if (Tsk !== null) {
        let tskMode: string = NChar.ToUpper(Tsk.getMode());
        for (let i: number = 0; i < Modes.StrVal.length; i = i + 1) {
          let mode: string = NChar.ToUpper(Modes.StrVal.charAt(i));
  ...
  private eval_op_like(source: ExpVal, maskOrg: ExpVal, resVal: ExpVal): void {
    ...
    // 2. In the case: there is not any '*'
      ...
      Source = NString.Substring(Source, i);
  ...
  private static arrToStr(arr: any, from?: number, to?: number): string {
    if (arguments.length === 1 && (arr === null || arr instanceof Array)) {
  ...
  private static op_like_map(source: StringBuilder, mask: string, end: boolean): boolean {
    ...
    for (j = 0; j < source.Length && !same; j++) {
      same = true;
      if (end) {
        same = ExpressionEvaluator.op_like_cmp(NString.Substring(ptr, j), mask);
      }
      else {
    ...
    if (same) {
      ...
      source.Append(NString.Substring(ptr, j + i - 1));
    }
    return same;
  }
  ...
  private static trimStr(s: string, type: string): string {
    ...
    return r > l ? NString.Substring(s, l, (r) - (l)) : "";
  }
  ...
  private eval_op_astr(source: ExpVal, format: ExpVal, resVal: ExpVal): void {
    ...
    if (format.StrVal.length > 0 && source.StrVal.length > 0) {
      ...
      resVal.StrVal = DisplayConvertor.Instance.mg2disp(source.ToMgVal(), null, pic, false, (<Task>this.ExpTask).getCompIdx(), true, false);
    }
  }
  ...
  private eval_op_strTokenIdx(sourceString: ExpVal, token: ExpVal, delimiter: ExpVal, resVal: ExpVal): void {
    ...
    if (!sourceString.IsNull && sourceString.StrVal !== null && !token.IsNull && token.StrVal !== null && NString.TrimEnd(sourceString.StrVal, this._charsToTrim).length !== 0 && NString.Trim(token.StrVal).length > 0) {
  ...
  private strTokenIndex(source: string, delimiter: string, token: string): number {
    ...
    let tokenOffset: number = NString.IndexOf(trimSource, trimToken);
    ...
    if (tokenOffset === -1)
      ...
      let tokenIndex: number = this.strTokenCount(NString.Substring(trimSource, 0, tokenOffset + trimDelim.length), trimDelim);
      return tokenIndex;
    }
  }
  ...
  private strTokenCount(source: string, delimiter: string): number {
    ...
    delimiterOffset = NString.IndexOf(data, trimDelim, fromOffset);
  ...
  private eval_op_StrBuild(valStack: Stack<ExpVal>, resVal: ExpVal, nArgs: number): void {
    ...
    for (let i: number = 1; i < nArgs; i++) {
      ...
      while (indexFrom !== -1) {
        let nextIndex: number = NString.IndexOf(resultStr.ToString(), toReplace, indexFrom);
        if (nextIndex === -1)
          break;

        let precededBySlash: boolean = false;
        let shashIndex: number = NString.IndexOf(resultStr.ToString(), "\\" + toReplace, indexFrom);
        if (shashIndex !== -1)
        ...
        if ((precededBySlash && nextIndex !== shashIndex + 1) || !precededBySlash) {
          resultStr.Replace(resultStr.ToString(nextIndex, nextIndex + toReplace.length - nextIndex), NString.Trim(Exp_parms[i].StrVal), nextIndex, nextIndex + toReplace.length - nextIndex);
          indexFrom = nextIndex + NString.Trim(Exp_parms[i].StrVal).length;
        }
        else
  ...
  private cst_code_trans_buf(opr: string, intStr: String, code: string, strId: string): string {
    ...
    for (i = 0; i < intStr.length && tokens.MoveNext(); i++) {
      ...
      let ofs: number = NString.IndexOf(token, '&');

      // if '&' is found, move to character next to '&'
      // else move to first character
      ofs++;
      let currCode: string = NChar.ToUpper(token.charAt(ofs));
      if (opr === 'I') {
        if (code === currCode) {
  ...
  /// right index
  private eval_op_rights(resVal: ExpVal, valIdx: ExpVal): void {
    resVal.Attr = StorageAttribute.BOOLEAN;
    resVal.BoolVal = ClientManager.Instance.getUserRights().getRight((<Task>this.ExpTask).getCtlIdx(), valIdx.MgNumVal.NUM_2_LONG());
  }
  ...
  private eval_op_expcalc(resVal: ExpVal, expVal: ExpVal): void {
    ...
    if (ExpressionEvaluator._recursiveExpCalcCount < 50) {
      ...
      let expVal2: ExpVal = (<Task>this.ExpTask).getExpById(expVal.MgNumVal.NUM_2_LONG()).evaluate(StorageAttribute.NONE);
      ExpressionEvaluator._recursiveExpCalcCount--;
  ...
  private eval_op_ClientRedirect(resVal: ExpVal, val1: ExpVal, val2: ExpVal, val3: ExpVal, val4: ExpVal): void {
    let userID: string = val1.StrVal;
    let password: string = val2.StrVal;
    let execProperties: string = val3.StrVal;
    let parallelExecution: boolean = val4.BoolVal;
    ...
    let executionProps: MgProperties = ClientManager.Instance.copyExecutionProps();
    executionProps.loadFromXML(execProperties);
    ...
    executionProps.set_Item(ConstInterface.MG_TAG_USERNAME, userID);
    executionProps.set_Item(ConstInterface.MG_TAG_PASSWORD, password);
    if (NString.IsNullOrEmpty(userID))
      executionProps.Remove(ConstInterface.REQ_SKIP_AUTHENTICATION);

    // arrange new execution properties
    let executionPropertiesXML: StringBuilder = new StringBuilder();
    executionProps.storeToXML(executionPropertiesXML);
    let executionPropertiesEncoded: string = HttpUtility.UrlEncode(executionPropertiesXML.ToString(), Encoding.UTF8);
  ...
  private eval_op_taskType(resVal: ExpVal, Parent: ExpVal): void {
    ...
    if ((iParent >= 0 && iParent < (<Task>this.ExpTask).getTaskDepth(false)) || iParent === ExpressionEvaluator.TRIGGER_TASK) {
      let task: Task = <Task>super.GetContextTask(iParent);
      if (task !== null) {
        resVal.StrVal = (task.isMainProg() ? "MC" : "C");
      }
      else
        resVal.StrVal = " ";
    }
  ...
  private eval_op_dataview_to_datasource(resVal: ExpVal, val1: ExpVal, val2: ExpVal, val3: ExpVal, val4: ExpVal, val5: ExpVal): void {
    ...
    if (iParent >= 0) {
      ...
      if (task !== null) {
        ...
        let taskVarName: string = NString.Trim(val2.StrVal);
        ...
        let destinationColumns: string = NString.Trim(val5.StrVal);
  ...
  eval_op_control_select_program(expVal: ExpVal, resVal: ExpVal): void {
    ...
    if (mgControl !== null && mgControl.HasSelectProgram()) {
      let selectProgProp: Property = mgControl.getProp(58);
...
/// </summary>
export class ExpStrTracker extends NObject {
...
class DynamicOperation extends NObject {
  argCount_: number = 0;
