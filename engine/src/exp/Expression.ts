import {List, NObject, NString, NNumber, NumberStyles} from "@magic/mscorelib";
...
///   data for <exp ...> tag
/// </summary>
export class Expression extends NObject implements IResultValue {
  ...
  evaluate(resType: StorageAttribute, length: number): string;
  evaluate(resType: StorageAttribute): ExpVal;
  evaluate(length: number): Expression_ReturnValue;
  evaluate(resTypeOrLength: any, length?: number): string | ExpVal | Expression_ReturnValue {
    if (arguments.length === 2 && (resTypeOrLength === null || resTypeOrLength.constructor === Number) && (length === null || length.constructor === Number)) {
      return this.evaluate_0(resTypeOrLength, length);
    }
    if (arguments.length === 1 && (resTypeOrLength === null || resTypeOrLength.constructor === Number)) {
      return this.evaluate_1(resTypeOrLength);
    }
    return this.evaluate_2(resTypeOrLength);
  }
  ...
  private evaluate_0(resType: StorageAttribute, length: number): string {
    let retVal: string;
  ...
  /// <param name="resType"></param>
  /// <returns></returns>
  private evaluate_1(resType: StorageAttribute): ExpVal {
  ...
  /// </summary
  private evaluate_2(length: number): Expression_ReturnValue {
    ...
    if (this.computedByClient()) {
      ...
      val = this.evaluate(resType, length);
      resType = this._type;
    }
  ...
  private initElements(tokensVector: List<String>): void {
    ...
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      ...
      switch (attribute) {
        ...
        default:
          Logger.Instance.WriteExceptionToLog("There is no such tag in Expression.initElements class. Insert case to Expression.initElements for " + attribute);
          break;
      }
    }
    if (!this.computedByClient())
  ...
  private buildByteArray(expStr: string): void {
    ...
    for (let i: number = 0; i < expStr.length; i = i + 2) {
      let twoHexDigits: string = NString.Substring(expStr, i, 2);
