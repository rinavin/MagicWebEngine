...
import {UserRights} from "./security/UserRights";
import {TasksTable} from "./tasks/TasksTable";
import {TableCacheManager} from "./rt/TableCacheManager";
import {MgProperties} from "./util/MgProperties";
import {EventsManager, EventsAllowedType} from "./event/EventsManager";
...
import {HttpClientEvents} from "@magic/httpClient";
...
import {
  CommandsProcessorBase, CommandsProcessorBase_SendingInstruction,
...
export class ClientManager {
  ...
  private _unframedCmds: MgArrayList<IClientCommand> = null;
  private _CommandsExecutedAfterTaskStarted: MgArrayList<IClientTargetedCommand> = null;
  ...
  // TODO: Remove this becaues webclient won't be started using /INI=
  LoadedExecutionPropertiesFileName: string = null;  // the full name (including the path) of the execution properties file, only if the client was started with /INI=, otherwise null.
  ...
  getUnframedCmds(): MgArrayList<IClientCommand> {
  ...
  addCommandsExecutedAfterTaskStarted(cmd: IClientTargetedCommand): void {
    this._CommandsExecutedAfterTaskStarted.Add(cmd);
  }

  /// <summary> get commands which are to be executed after task started</summary>
  getCommandsExecutedAfterTaskStarted(): MgArrayList<IClientTargetedCommand> {
  ...
  parseLogLevel(strLogLevel: string): Logger_LogLevels {
    ...
    if (!NString.IsNullOrEmpty(strLogLevel)) {
      if (NString.StartsWith(strLogLevel, "SERVER", StringComparison.CurrentCultureIgnoreCase))
        logLevel = strLogLevel.endsWith("#") ? Logger_LogLevels.ServerMessages : Logger_LogLevels.Server; // only requests to the server
      else if (NString.StartsWith(strLogLevel, "S", StringComparison.CurrentCultureIgnoreCase) || NString.StartsWith(strLogLevel, "Q", StringComparison.CurrentCultureIgnoreCase))
        logLevel = Logger_LogLevels.Support;
      else if (NString.StartsWith(strLogLevel, "G", StringComparison.CurrentCultureIgnoreCase))
        logLevel = Logger_LogLevels.Gui;
      else if (NString.StartsWith(strLogLevel, "D", StringComparison.CurrentCultureIgnoreCase))
        logLevel = Logger_LogLevels.Development;
      else if (NString.StartsWith(strLogLevel, "B", StringComparison.CurrentCultureIgnoreCase))
  ...
  ProcessResponse(response: string, currMgdID: number, openingTaskDetails: OpeningTaskDetails, res: IResultValue): void {
    ...
    if (response === null || NString.Trim(response).length === 0)
      return;
  ...
  WorkThreadExecution(): void {
    if (Logger.Instance.ShouldLog()) {
      ...
      Logger.Instance.WriteToLog("Started on " + DateTimeUtils.ToString(DateTime.Now, "dd/MM/yyyy"), true);
  ...
  ProcessAbortingError(ex: ApplicationException): void {
    ...
    // TODO : handle errors
    // let flag: boolean = ex instanceof ServerError;
    // let text: string;
    // if (flag) {
    //   text = (<ServerError>ex).GetMessage();
    // }
    // else {
    //   text = ((ex.Message !== null) ? ex.Message : ex.GetType().FullName);
    // }
    // let flag2: boolean = ex.InnerException !== null;
    // if (flag2) {
    //   Logger.Instance.WriteExceptionToLog(ex.InnerException, ex.Message);
    // }
    // else {
    //   let flag3: boolean = !(ex instanceof ServerError) || Logger.Instance.LogLevel !== Logger_LogLevels.Basic;
    //   if (flag3) {
    //     Logger.Instance.WriteExceptionToLog(text);
    //   }
    // }
    // let num: number = 7;
    ...
    // if (flag6) {
    //   text = text + OSEnvironment.EolSeq + OSEnvironment.EolSeq + ClientManager.Instance.getMessageString(MsgInterface.STR_RESTART_SESSION);
    //   num2 = (num2 | 4);
    // }
    // let flag7: boolean = NString.StartsWith(ex.Message, "<HTML", StringComparison.CurrentCultureIgnoreCase);
    // if (flag7) {
    //   this.processHTMLContent(ex.Message);
    // }
    // else {
    //   num = Commands.messageBox(null, messageString, text, num2);
    // }
  ...
  WriteExecutionPropertiesToLog(): void {
    if (Logger.Instance.ShouldLogServerRelatedMessages()) {
      ...
      this._executionProps.Keys.forEach((key: string) => {
        let line: string;
        if (NString.Equals(key, ConstInterface.MG_TAG_PASSWORD, StringComparison.CurrentCultureIgnoreCase))
          line = NString.Format("{0,-35} : ***", key);
        else {
          let value: string = this._executionProps.get_Item(key);
          if (NString.Equals(key, ConstInterface.REQ_PRG_NAME, StringComparison.CurrentCultureIgnoreCase)) {
  ...
  constructor() {
    ...
    this._unframedCmds = new MgArrayList(); // client commands which do not belong to any mgdata yet
    this._CommandsExecutedAfterTaskStarted = new MgArrayList();
  ...
  static Main(args: string[]): void {
    ...
    if (args.length !== 0 && NString.StartsWith(args[0], "/spawnahead")) {
      ...
      // TODO : WebClient change
      // StudioLauncher.Init(args);
    }
    else
      ClientManager.StartExecution(args);

    Logger.Instance.Flush();
  }

  static StartExecution(args: string[]): void {
    let loaded: boolean = false;
    if (args.length > 0)
      loaded = ClientManager.Instance.LoadExecutionProps(args);

    ClientManager.Instance.InitGuiManager();

    if (!loaded) {
      let loadedExecutionPropertiesFileName: string = ClientManager.Instance.LoadedExecutionPropertiesFileName;
      if (loadedExecutionPropertiesFileName !== null && loadedExecutionPropertiesFileName.length > 0) {
        let title: string = null;
        Commands.messageBox(null, title, NString.Format("{0} not found or invalid file", loadedExecutionPropertiesFileName), 0);
      }
    }
    else {
  ...
  LoadExecutionProps(args: string[]): boolean {
    // TODO: Load execution properties --- maybe from assets
    // return true, if successfully loaded.
    let xMLdata2: string = "";
    this._executionProps.loadFromXML(xMLdata2);
  ...
  IsLogonRTL(): boolean {
    ...
    return text !== null && NString .CompareTo(text, "Y") === 0;
  }
    ...
    for (let i: number = 0; i < mgDataTab.getSize(); i = i + 1) {
      ...
      for (let j: number = 0; j < tasksCount; j = j + 1) {
        ...
        if (task.IsSubForm && task.DoSubformPrefixSuffix) {
          ...
          this.EventsManager.pushNewExecStacks();
          this.EventsManager.handleInternalEvent(task, InternalInterface.MG_ACT_REC_PREFIX, true);
  ...
  /// </summary>
  private UserRights_RightsChanged(): void {
  ...
  getRightsHashKey(): string {
    return this._userRights.RightsHashKey;
  }
  ...
  fillUserRights(): void {
    ...
    if (endContext !== -1 && endContext < ClientManager.Instance.RuntimeCtx.Parser.getXMLdata().length) {
      ...
      let tmp: string = ClientManager.Instance.RuntimeCtx.Parser.getXMLsubstring(endContext);
      // skip to after the "val =" part, and pass it on to user rights function
      tmp = tmp.substr(tmp.indexOf(XMLConstants.MG_ATTR_VALUE) + 5);
      tmp.substr(0, tmp.length - 1);
      this._userRights.fillUserRights(tmp);
  ...
  RegisterDelegates(): void {
    HttpClientEvents.GetExecutionProperty_Event = this.GetExecutionProperty;
    HttpClientEvents.GetRuntimeCtxID_Event = this.GetRuntimeCtxID;
    HttpClientEvents.GetGlobalUniqueSessionID_Event = this.GetGlobalUniqueSessionID;
    HttpClientEvents.ShouldDisplayGenericError_Event = this.ShouldDisplayGenericError;
    UserRights.RightsChanged = this.UserRights_RightsChanged;
  }
