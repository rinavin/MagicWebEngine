...
///
/// </author>
export class MgControl extends MgControlBase implements IComparable, PropParentInterface {
  ...
  CompareTo(obj: any): number {
    let otherCtrl: MgControl = <MgControl>obj;
    let otherOrderProp: Property = otherCtrl._propTab.getPropById(PropInterface.PROP_TYPE_TAB_ORDER);
    let thisOrderProp: Property = this._propTab.getPropById(PropInterface.PROP_TYPE_TAB_ORDER);
    ...
    let thisOrder: number = thisOrderProp.getValueInt();
    let otherOrder: number = otherOrderProp.getValueInt();
    ...
    if (super.getDitIdx() > otherCtrl.getDitIdx())
      return 1;
    else
      return -1;
  }
    ...
    if (!isTagProcessed) {
      switch (attribute) {
        ...
        case ConstInterface.MG_ATTR_REFRESHON:
          this.refreshOnString = NString.Trim(valueStr);
          break;
        default:
  ...
  GetSelectMode(): string {
    ...
    let smTmp: string = super.getProp(PropInterface.PROP_TYPE_SELECT_MODE).getValue();
    if (smTmp !== null)
  ...
  allowedParkRelatedToDirection(): boolean {
    ...
    let allowDirection: number = super.getProp(PropInterface.PROP_TYPE_ALLOWED_DIRECTION).getValueInt();
      ...
      if (super.getForm().getTask() === lastparkedCtrl.getForm().getTask()) {
        ...
        if (lastparkedCtrlTabOrder === -1) {
          if (super.getForm().isSubForm())
            subformPathList.Add(<MgControl>((<MgFormBase>form).getSubFormCtrl()));
          else
        ...
        for (let i: number = 0; i < subformPathList.Count; i++) {
  ...
  private invoke_1(moveByTab: boolean, checkSubFormTabIntoProperty: boolean): boolean {
    ...
    if (this.isParkable(true, moveByTab, checkSubFormTabIntoProperty) && (this.IgnoreDirectionWhileParking || this.allowedParkRelatedToDirection())) {
  ...
  IsParkable(moveByTab: boolean): boolean {
    return this.isParkable(true, moveByTab, true);
  }

  isParkable(checkEnabledAndVisible: boolean, moveByTab: boolean, checkSubFormTabIntoProperty: boolean): boolean;
  isParkable(checkEnabledAndVisible: boolean, moveByTab: boolean): boolean;
  isParkable(checkEnabledAndVisible: boolean, moveByTab: boolean, checkSubFormTabIntoProperty?: boolean): boolean {
    if (arguments.length === 3 && (checkEnabledAndVisible === null || checkEnabledAndVisible.constructor === Boolean) && (moveByTab === null || moveByTab.constructor === Boolean) && (checkSubFormTabIntoProperty === null || checkSubFormTabIntoProperty.constructor === Boolean)) {
      return this.isParkable_0(checkEnabledAndVisible, moveByTab, checkSubFormTabIntoProperty);
    }
    return this.isParkable_1(checkEnabledAndVisible, moveByTab);
  }

  private isParkable_0(checkEnabledAndVisible: boolean, moveByTab: boolean, checkSubFormTabIntoProperty: boolean): boolean {
  ...
  /// </summary>
  isModifiableCtrlDB(): boolean {
    let dbModifable: boolean = true;
    if (this._field !== null)
      dbModifable = this._field.DbModifiable;
    return dbModifable && super.isModifiable();
  }
  ...
  isPartOfDataGroup(): boolean {
    return this._siblingVec !== null && this._siblingVec.Count > 0;
  }
  ...
  getNextSiblingControl(): MgControl {
    ...
    for (let i: number = 0; i < this._siblingVec.Count && nextSiblingCtrl === null; i++) {
  ...
  getPrevSiblingControl(): MgControl {
    ...
    for (i = this._siblingVec.Count - 1; i >= 0 && prevSiblingCtrl === null; i--) {
      let ctrl: MgControl = <MgControl>this._siblingVec.get_Item(i);
      if (this.Form.getControlIdx(ctrl) < myIndex)
  ...
  enableTextAction(enable: boolean): void {
    if (super.isTextControl()) {
      ...
      task.ActionManager.enable(InternalInterface.MG_ACT_WIDE, enable && super.IsWideAllowed());
  ...
  HasSelectProgram(): boolean {
    ...
    let selectProgProp: Property = super.getProp(PropInterface.PROP_TYPE_SELECT_PROGRAM);
  ...
  private ShouldSetSubformInvisible(): boolean {
    ...
    let subformType: SubformType = <SubformType>super.getProp(PropInterface.PROP_TYPE_SUBFORM_TYPE).getValueInt();
  ...
  validateAndSetValue(NewValue: string, updateCtrl: boolean): boolean {
    ...
    if (this.ModifiedByUser) {
      ...
      if (updateCtrl) {
        ...
        if (this._field !== null && this._field.getType() === StorageAttribute.NUMERIC && NString.Trim(NewValue).length > 0)
  ...
  private setValue(vd: ValidationDetails): void {
    ...
    let newValue: string = vd.getDispValue();
    let mgVal: string;
  ...
  IsTableWithAbsoluteScrollbar(): boolean {
    ...
    if (this._propTab.propExists(PropInterface.PROP_TYPE_SCROLL_BAR_THUMB))
      tableWithAbsoluteScrollbar = (this._propTab.getPropById(PropInterface.PROP_TYPE_SCROLL_BAR_THUMB).getValueInt() === <number>ScrollBarThumbType.Absolute);
