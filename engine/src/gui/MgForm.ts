import {MgFormBase, Property, PropInterface, MgControlBase, Manager, Row, TaskBase, RuntimeContextBase,
        DataModificationTypes, ControlTable, NUM_TYPE, CommandType, Commands, LastFocusedVal} from "@magic/gui";
import {Logger, Misc, MgControlType, WindowType, InternalInterface,
        Constants, Priority, XmlParser, XMLConstants} from "@magic/utils";
import {List, Debug, Exception, Int32, NString, RefParam, ApplicationException,
        NObject,
        StringBuilder} from "@magic/mscorelib";
...
import {GuiConstants} from "../../../gui/src/GuiConstants";
...
export class MgForm extends MgFormBase {
  ...
  private _wideActBegNxtLineWasEnabled: boolean = false; // before the wide, need to save if action MG_ACT_EDT_BEGNXTLINE
  ...
  moveInView(unit: string, direction: string, returnToCtrl?: boolean): void {
    if (arguments.length === 2 && (unit === null || unit.constructor === Number) && (direction === null || direction.constructor === Number)) {
  ...
  private moveInView_1(unit: string, direction: string, returnToCtrl: boolean): void {
    ...
    try {
            ...
            try {
              ...
              if (ex instanceof RecordOutOfDataViewException) {
                ...
                if (this.GetDataview().getTopRecIdx() < 0) {
                  ...
                  // QCR 758639
                  {
                    ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_REC_SUFFIX);
                    if (ClientManager.Instance.EventsManager.GetStopExecutionFlag()) {
                      (<DataView>this.GetDataview()).restoreTopRecIdx();
                      this.restoreOldDisplayLine(oldDisplayLine);
                      return;
                  ...
                  if (unit === Constants.MOVE_UNIT_ROW && direction === Constants.MOVE_DIRECTION_NEXT) {
                    ...
                    if (!this._task.IsInteractive && this._task.getMode() !== Constants.TASK_MODE_CREATE)
                      ...
                      ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_CRELINE);
                  }
                  return;
                }
                Logger.Instance.WriteExceptionToLog(ex);
      ...
      ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_REC_PREFIX);
  ...
  moveInRow(ctrl: MgControl, direction: string): boolean {
    ...
    switch (direction) {
      ...
      default:
        Logger.Instance.WriteExceptionToLog(NString.Format("in Form.moveInRow() illigal move direction: {0}", direction));
        this._task.setLastMoveInRowDirection(Constants.MOVE_DIRECTION_NONE);
        return cursorMoved;
    }
      ...
      if (this.isLineMode()) {
        ...
        try {
          ...
          if (ex instanceof RecordOutOfDataViewException) {
            ...
            Logger.Instance.WriteExceptionToLog(NString.Format("in Form.addRec() {0}", ex.Message));
          }
        }
  ...
  delCurrRec(): void {
    ...
    if (this.GetDataview().isEmpty() && !(<Task>this._task).IsTryingToStop) {
      if (this._task.DataView.isEmptyDataview()) {
        ...
        if (this._task.checkProp(202, true)) {
        ...
        // exit from the ownerTask
          ...
          ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_EXIT);
      }
    }
      ...
      if (super.isLineMode()) {
        try {
          ...
          if (ex instanceof RecordOutOfDataViewException) {
            // no exception should really be caught here.
            if (row !== 0)
              Logger.Instance.WriteExceptionToLog(NString.Format("in Form.cancelEdit() {0}", ex.Message));
          }
          else
            throw ex;
        }
  ...
  getHelpUrlProp(): Property {
    ...
    let helpProp: Property = this._propTab.getPropById(54);
    if (helpProp === null) {
      if (super.isSubForm()) {
  ...
  refreshOnExpressions(): void {
    for (let i: number = 0; i < this.CtrlTab.getSize(); i = i + 1) {
      let mgControl: MgControl = this.CtrlTab.getCtrl(i) as MgControl;
  ...
  getFirstParkableCtrl(IncludingSubForm: boolean): MgControl;
  getFirstParkableCtrl(IncludingSubForm?: boolean): MgControl {
    if (arguments.length === 0) {
      return this.getFirstParkableCtrl_0();
    }
    return this.getFirstParkableCtrl_1(IncludingSubForm);
  }
  ...
  /// </summary>
  /// <returns></returns>
  private getFirstParkableCtrl_0(): MgControl {
    return this.getFirstParkableCtrl(true);
  }
  ...
  /// </summary>
  private getFirstParkableCtrl_1(IncludingSubForm: boolean): MgControl {
  ...
  getCtrlByName(ctrlName: string): MgControl {
    ...
    for (guiParentTask = <Task>this._task;
      ...
      if (destSubForm == null) {
        if (!guiParentTask.IsSubForm ||
          NString.Compare(guiParentTask.getForm().getSubFormCtrl().Name, ctrlName, true))
          break;
      }
    }
  ...
  getSubFormCtrlForTask(taskId: string): MgControl {
    ...
    for (guiParentForm = this; guiParentForm != null; guiParentForm = guiParentForm.ParentForm) {
      ...
      for (let i: number = 0; i < parentCtrlTab.getSize(); i++) {
        curr = <MgControl>parentCtrlTab.getCtrl(i);
  ...
  applyTabOrderChangeAfterTableReorder(list: List<MgControlBase>): void {
    ...
    for (let i: number = 0; i < list.Count; i = i + 1) {
  ...
  private checkAndCreateRowsEvent(): void {
    if (this._tableMgControl !== null && this.Opened && this.RefreshRepeatableAllowed) {
      ...
      if (this._lastRowSent >= this.Rows.Count) {
        // when we decrease size of rows array adjust lastRowSent
        this._lastRowSent = this.Rows.Count - 1;
      }
      if (this._lastRowSent < size - 1 && !this._transferingData) {
        ...
        ClientManager.Instance.EventsManager.addInternalEvent(this._tableMgControl, InternalInterface.MG_ACT_DV_TO_GUI, Priority.LOWEST);
  ...
  private refreshRows(desiredTopIndex: number, sendAll: boolean, saveRowIdx: number, diffRef: RefParam<number>): boolean {
    ...
    for (let i: number = 0; i < this._rowsInPage + 2; i++) {
  ...
  /// <returns></returns>
  getWideForm(): MgForm {
    let wideForm: MgForm = null;
    if (this.wideIsOpen())
      wideForm = <MgForm>this._wideControl.getForm();
  ...
  private isTabbingCycleMoveToParentTask(): boolean {
    let prop: Property = this._task.getProp(PropInterface.PROP_TYPE_TABBING_CYCLE);
    return prop !== null && prop.getValue().charAt(0) === GuiConstants.TABBING_CYCLE_MOVE_TO_PARENT_TASK;
  }
  ...
  private isTabbingCycleRemainInCurrentRecord(): boolean {
    let prop: Property = this._task.getProp(PropInterface.PROP_TYPE_TABBING_CYCLE);
    return prop !== null && prop.getValue().charAt(0) === GuiConstants.TABBING_CYCLE_REMAIN_IN_CURRENT_RECORD;
  }
  ...
  private isTabbingCycleMoveToNextRecord(): boolean {
    let prop: Property = this._task.getProp(PropInterface.PROP_TYPE_TABBING_CYCLE);
    return prop !== null && prop.getValue().charAt(0) === GuiConstants.TABBING_CYCLE_MOVE_TO_NEXT_RECORD;
  }
  ...
  RefreshDisplay(refreshType: string): boolean {
    ...
    try {
      ...
      if (ex instanceof Exception) {
        Logger.Instance.WriteExceptionToLog(NString.Format("{0} : {1} in task {2}", ex.GetType(), ex.Message, (<Task>super.getTask()).PreviouslyActiveTaskId));
      }
      else
        throw ex;
    }
  ...
  private refreshTable(): void {
    ...
    if (!this._tableRefreshed && currRecIdx >= 0) {
      ...
      if (oldCurrRow + 1 > this._rowsInPage)
        this.GetDataview().setTopRecIdx(oldCurrRow - this._rowsInPage / 2 + 1);
  ...
  buildTabbingOrder(): void {
    ...
    if (this._recomputeTabOrder !== 'N') {
      ...
      for (let i: number = 0; i < ctrlArray.Count; i++) {
  ...
  private getSortedControls(ctrlArray: List<MgControlBase>): void {
    for (let i: number = 0; i < this.CtrlTab.getSize(); i = i + 1) {
      ...
      for (let sortArrayIdx: number = 0; sortArrayIdx < ctrlArray.Count; sortArrayIdx = sortArrayIdx++) {
        let sortedCtrl: MgControlBase = ctrlArray.get_Item(sortArrayIdx);
        let sortedCtrlProp: Property = sortedCtrl.getProp(314);
  ...
  /// <param name="parentControl"></param>
  openWide(parentControl: MgControlBase): void {
    // enable the MLE actions & state (the parent wide can't be MLE, so we are not checking the parent MLE property)
    this._wideActBegNxtLineWasEnabled = this._task.ActionManager.isEnabled(InternalInterface.MG_ACT_EDT_BEGNXTLINE);
  ...
  closeWide(): void {
    ...
    this._task.ActionManager.enable(InternalInterface.MG_ACT_EDT_BEGNXTLINE, this._wideActBegNxtLineWasEnabled);
  ...
  private InitTableControl_01(size: number, removeAll: boolean): void {
    if (this._tableMgControl !== null) {
      if (removeAll) {
        Commands.addAsync(CommandType.SET_TABLE_INCLUDES_FIRST, this._tableMgControl, 0, true);
        Commands.addAsync(CommandType.SET_TABLE_INCLUDES_LAST, this._tableMgControl, 0, true);
      }
      else {
        Commands.addAsync(CommandType.SET_TABLE_INCLUDES_FIRST, this._tableMgControl, 0, this.GetDataview().IncludesFirst() || this.GetDataview().IsOneWayKey);
        Commands.addAsync(CommandType.SET_TABLE_INCLUDES_LAST, this._tableMgControl, 0, this.GetDataview().IncludesLast());
      }
  ...
  SetTableTopIndex(): void {
    if (this._tableMgControl !== null) {
      ...
      if (index !== this._prevTopIndex) {
        Commands.addAsync(CommandType.SET_TABLE_TOP_INDEX, this._tableMgControl, 0, index);
  ...
  ToString(): string {
    return NString.Concat([
      "{", this.GetType().Name, ": task=", this._task, ", Id=", this.UserStateId, "}"
    ]);
  }
  ...
  GetHiddenControlListXML(): string {
    ...
    if (this.hiddenControlsIsnsList !== null && this.hiddenControlsIsnsList.Count > 0) {
      // write the id
...
class ParkableControlInfo extends NObject {
