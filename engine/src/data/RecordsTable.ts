...
import {ObjectReferencesCollection, DataModificationTypes} from "@magic/gui";
import {} from "@magic/gui/src/GuiEnums";
import {FieldsTable} from "./FieldsTable";
import {Hashtable, List, IDisposable, StringBuilder, NObject, ApplicationException, Int32} from "@magic/mscorelib";
import {Logger} from "@magic/utils";

///   an object of this class holds a collection of records
export class RecordsTable extends NObject implements IRecordsTable {
  ...
  constructor(withLinkedList: boolean) {
    ...
    this._useLinkedList = withLinkedList;
  ...
  buildXML(fieldsTab: FieldsTable, message: StringBuilder, skipCurrRec: boolean, currRecId: number): void {
    for (let i: number = 0; i < this._records.Count; i = i + 1) {
      ...
      while (prevRec !== null && (prevRec.getMode() === DataModificationTypes.Insert || !prevRec.SendToServer) && record.getMode() === DataModificationTypes.Insert && ((recIdx = this.getRecIdx(prevRec.getId())) > i || recIdx === RecordsTable.REC_NOT_FOUND || !prevRec.SendToServer)) {
  ...
  getRecByIdx(idx: number): Record {
    if (idx < 0 || idx >= this._records.Count) {
  ...
  private addRec(rec: Record): void {
    if (this._useLinkedList && this._records.Count > 0) {
      let record: Record = this._records.get_Item(this._records.Count - 1);
  ...
  private removeRecord_0(recIdx: number): void {
    if (recIdx >= 0 && recIdx < this._records.Count) {
      ...
      if (useLinkedList) {
        ...
        if (recIdx + 1 < this._records.Count) {
          let record2: Record = this._records.get_Item(recIdx + 1);
  ...
  replicate(): RecordsTable {
    let recordsTable: RecordsTable = <RecordsTable>super.MemberwiseClone();
    ...
    for (let i: number = 0; i < this._records.Count; i = i + 1) {
      ...
      recordsTable._records.Add(record);
