    ...
    for (let j: number = 0; j < tokensVector.Count; j = j + 2) {
      ...
      switch (attribute) {
        case XMLConstants.MG_ATTR_ID:
          this.Columns.Add(this._table.FldsTab.getField(XmlParser.getInt(valueStr)));
          break;
  ...
  private InitInnerObjects(parser: XmlParser, foundTagName: string): boolean {
    ...
    if (foundTagName === ConstInterface.MG_TAG_COLUMN) {
      ...
      if (endContext !== -1 && endContext < parser.getXMLdata().length) {
        ...
        parser.add2CurrIndex(NString.IndexOf(tag, ConstInterface.MG_TAG_COLUMN) + ConstInterface.MG_TAG_COLUMN.length);
