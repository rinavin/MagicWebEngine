...
export class MGDataCollection implements IMGDataTable {
  ...
  getAvailableIdx(): number {
    ...
    for (i = 0; i < this._mgDataTab.Count; i = i + 1) {
  ...
  GetMainProgByCtlIdx(contextIDOrCtlIdx: any, ctlIdx?: number): Task {
    if (arguments.length === 2 && (contextIDOrCtlIdx === null || contextIDOrCtlIdx.constructor === Number) && (ctlIdx === null || ctlIdx.constructor === Number)) {
  ...
  GetMGDataForStartupProgram(): MGData {
    Debug.Assert(this._mgDataTab.Count > 0, "The main program must be processed before invoking this method.");
