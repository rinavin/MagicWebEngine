...
export class GUIManager {
  ...
  abort(): void;
  abort(form?: MgForm): void {
    if (arguments.length === 1 && (form === null || form instanceof MgForm)) {
      this.abort_0(form);
      return;
    }
    this.abort_1();
  ...
  confirm(form: MgForm, msgId: string, style?: number): any {
    if (arguments.length === 2 && (form === null || form instanceof MgForm) && (msgId === null || msgId.constructor === String)) {
  ...
  private confirm_0(form: MgForm, msgId: string): boolean {
    let retResult: number = this.confirm(form, msgId, Styles.MSGBOX_ICON_QUESTION | Styles.MSGBOX_BUTTON_YES_NO |
  ...
  /// </summary>
  static getLastFocusedControl(): MgControl;
  static getLastFocusedControl(): MgControl;
