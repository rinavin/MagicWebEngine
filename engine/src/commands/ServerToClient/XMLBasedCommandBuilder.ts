...
export class XMLBasedCommandBuilder {
  ...
  private initElements(tokensVector: List<string>): IClientTargetedCommand {
    ...
    let commandType: ClientTargetedCommandType = this.getType(tokensVector[1].toLowerCase());
    let clientTargetedCommand: IClientTargetedCommand = this.CreateCommand(commandType);

    for (let i: number = 2; i < tokensVector.Count; i = i + 2) {
      let attribute: string = tokensVector[i];
      let value: string = tokensVector[i + 1];
  ...
  private getType(valueAttr: String): ClientTargetedCommandType {
    ...
    switch (valueAttr) {
      ...
      default:
        ...
        Logger.Instance.WriteExceptionToLog("Command.getType there is no such SERVER to CLIENT command : " + valueAttr);
        break;
    }
    return type;
  }
