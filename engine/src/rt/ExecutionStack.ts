import {NNumber, NObject, Stack, StringBuilder} from "@magic/mscorelib";
...
/// </summary>
export class ExecutionStack extends NObject {
  ...
  push(execEntryOrTaskId: any, handlerId?: string, operIdx?: number): void {
    if (arguments.length === 1 && (execEntryOrTaskId === null || execEntryOrTaskId instanceof ExecutionStackEntry)) {
  ...
  Equals(execStackCmp: any): boolean {
    ...
    if (tmpExecStack.size() === tmpExecStackCmp.size()) {
      ...
      while (!tmpExecStack.empty() && equalStacks) {
        if (!NObject.GenericEquals(tmpExecStack.pop(), tmpExecStackCmp.pop()))
          equalStacks = false;
      }
    }
    return equalStacks;
