...
export class Recompute {
  ...
  RcmpMode: Recompute_RcmpBy;  // Who (and when) performs rcmp (by client,
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, dataView: DataView): boolean {
    switch (foundTagName) {
      ...
      default:
        Logger.Instance.WriteExceptionToLog("There is no such tag in Recompute. Insert else if to Recompute.initInnerObjects for " + foundTagName);
        return false;
    }
    return true;
  }
  ...
  private fillLink(parser: XmlParser, dataView: DataView): void {
    ...
    if (endContext !== -1 && endContext < parser.getXMLdata().length) {
      ...
      for (let j: number = 0; j < tokensVector.Count; j += 2) {
        ...
        else Logger.Instance.WriteExceptionToLog("There is no such tag in Recompute class. Insert case to Recompute.fillLink for " + attribute);
      }
  ...
  private initElements(tokensVector: List<String>, dataView: DataView): void {
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      ...
      switch (attribute) {
        ...
        default:
          Logger.Instance.WriteExceptionToLog("There is no such tag in Recompute class. Insert case to Recompute.initElements for " + attribute);
          break;
      }
    }
  }
    ...
    try {
      ...
      // CLIENT
        ...
        try {
          ...
          if (this._rcmpOrder != null) {
            for (i = 0; i < this._rcmpOrder.Count; i++) {
              if (this._rcmpOrder[i] instanceof Field) {
                fld = <Field>this._rcmpOrder[i];
                this.fldRcmp(fld, true);
              }
              else if (this._rcmpOrder[i] instanceof DataviewHeaderBase) {
                let curLnk = <DataviewHeaderBase>this._rcmpOrder[i];
                ...
                for (let j: number = 0; j < linkFields.Count; j++) {
                  this.fldRcmp(linkFields[j], false);
  ...
  private RefreshSubforms(): void {
    if (this.Task.getForm().AllowedSubformRecompute) {
      ...
      subformsToRefresh.forEach(subformTask => this.Task.SubformRefresh(subformTask, true));
    }
  }
  ...
  AddSubform(subformTask: Task): void {
    ...
    if (!this._subForms.Contains(subformTask))
      this._subForms.Add(subformTask);
  }
  ...
  private recSuffixSubForms(): boolean {
    ...
    for (i = 0; successful && i < this._subForms.Count; i++) {
      subForm = this._subForms[i];
