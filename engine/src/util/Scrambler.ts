...
export class Scrambler {
  ...
  static Scramble(inVal: string): string {
    ...
    let key: number = <number>Math.sqrt(<number>length) + random;
  ...
  private static LocateScramble(inVal: string, from: number): number {
    let i: number = from;
    while (i < inVal.length && NChar.IsWhiteSpace(inVal.charAt(i).charCodeAt(0))) {
      i = i + 1;
    }
    return i + 1;
  }
  ...
  static UnScramble(inVal: string, beginOffSet: number, endOffSet: number): string {
    if (!Scrambler.ScramblingEnabled) {
      let outVal: string = NString.Substring(inVal, beginOffSet, endOffSet - beginOffSet + 1);
      return outVal;
    }
    else {
      ...
      key = (randomChr.charCodeAt(0) - 81) + Math.sqrt(length);
