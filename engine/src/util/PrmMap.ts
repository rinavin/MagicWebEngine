import {Dictionary, List, NObject, StringBuilder} from "@magic/mscorelib";
...
/// </summary>
export class PrmMap<TValue extends IMirrorXML> extends NObject {
...
export class MirrorPrmMap<TValue extends IMirrorXML> extends PrmMap<TValue> {
  ...
  setValue(s: string, v: TValue, addToChanges?: boolean): void {
    if (arguments.length === 3 && (s === null || s.constructor === String) && (v === null) && (addToChanges === null || addToChanges.constructor === Boolean)) {
    ...
    if (this.changes.Count > 0) {
      ...
      this.changes.forEach(change => {
        ...
        // call the variable to write it's own data
          xml.Append(this.values[change].mirrorToXML());
        else
