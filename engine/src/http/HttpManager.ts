import {HttpClient, HttpClientEvents, ICommunicationsFailureHandler} from "@magic/httpClient";
import {ApplicationException, Debug, Environment, NString, RefParam} from "@magic/mscorelib";
...
export class HttpManager {
  ...
  private RegisterBasicDelegates(): void {
    HttpClientEvents.GetHttpCommunicationTimeout_Event = this.GetHttpCommunicationTimeoutMS;
  }
  ...
  async GetContent(requestedURL: string, requestContent: string, isError: RefParam<boolean>): Promise<string> {
    let response: string;
    ...
    try {
      ...
      response = await this._httpClient.GetContent(requestedURL, requestContent);
      ...
      throw new ApplicationException(ex.Message + Environment.NewLine + url.origin + url.pathname, ex);
    }
    finally {
  ...
  private static LogAccessToServer(msg: string, requestContent: string): void {
    if (Logger.Instance.ShouldLogServerRelatedMessages()) {
      ...
      if (requestContent === null) {
        ...
        msg = NString.Concat([
          msg, "uploading ", requestContent.length, " bytes"
        ]);
        Logger.Instance.WriteServerToLog(msg);
      }
    }
  ...
  AddOutgoingHeader(name: string, value: string): void {
    this._httpClient.AddRequestHeader(name, value);
  }
}
