...
/// </summary>
import {IDisposable, NString, StackFrame} from "@magic/mscorelib";
import {HelpCommand, UtilImeJpn, UtilStrByteMode, WindowType} from "@magic/utils";
import {FontsTable} from "./env/FontsTable";
import {ColorsTable} from "./env/ColorsTable";
...
export class Manager {
  private static _delayWait: Object = null; // this object is used for execution and interrupting delay command
  private static _fontsTable: FontsTable = null;
  private static _colorsTable: ColorsTable = null;
  private static _clipboardData: string = null; // Data to be written to the clipboard.
  static TextMaskEditor: TextMaskEditor = null;
  ...
  static UtilImeJpn: UtilImeJpn = null; // JPN: IME support
  ...
  static Manager_cctor(): void {
    Manager._delayWait = new Object();
    Manager._clipboardData = "";
    Manager._fontsTable = new FontsTable();
    Manager._colorsTable = new ColorsTable();
    Manager.MenuManager = new MenuManager();
    Manager.TextMaskEditor = new TextMaskEditor();
  ...
  /// </summary>
  static SetProcessDPIAwareness(): void {
  }

  static Init(startupImageFileName: string): void;
  static Init(useWindowsXPThemes: boolean, startupImageFileName: string): void;
  static Init(startupImageFileNameOrUseWindowsXPThemes: any, startupImageFileName?: string): void {
    if (arguments.length === 1 && (startupImageFileNameOrUseWindowsXPThemes === null || startupImageFileNameOrUseWindowsXPThemes.constructor === String)) {
      Manager.Init_0(startupImageFileNameOrUseWindowsXPThemes);
      return;
    }
    Manager.Init_1(startupImageFileNameOrUseWindowsXPThemes, startupImageFileName);
  }

  private static Init_0(startupImageFileName: string): void {
  }

  private static Init_1(useWindowsXPThemes: boolean, startupImageFileName: string): void {
  }
  ...
  static MarkedTextGet(ctrl: MgControlBase): string {
    ...
    if (ctrl.isTextControl()) {
      ...
      if (selection.x !== selection.y) {
        ...
        markedText = NString.Substring(text, selection.x, selection.y - selection.x);
      }
    }
    return markedText;
  }
  ...
  static Abort(form: MgFormBase, mainPrgTask: TaskBase): void {
    ...
    let topMostForm: MgFormBase = form.getTopMostForm();
  ...
  /// </summary>
  static GetFontsTable(): FontsTable {
    return Manager._fontsTable;
  }

  /// <summary>
  ///   get the application ColorTable object from ColorTableList
  /// </summary>
  static GetColorsTable(): ColorsTable {
    return Manager._colorsTable;
  }
  ...
  private static SetFocus_1(itask: ITask, ctrl: MgControlBase, line: number, activateForm: boolean): void {
    ...
    if (ctrl !== null) {
      if (!ctrl.isParkable(true, false))
        return;
      Commands.addAsync(CommandType.SET_FOCUS, ctrl, (line >= 0) ? line : ctrl.getDisplayLine(false), activateForm);
    }
    else {
  ...
  static GetMessage(msg: string): string {
    if (msg === null)
      ...
      let idxOfCarriage: number = NString.IndexOf(msg, "\r");
      if (idxOfCarriage !== -1)
        msg = NString.Substring(msg, 0, idxOfCarriage);

      let idxOfNewLine: number = NString.IndexOf(msg, "\n");
      if (idxOfNewLine !== -1)
        msg = NString.Substring(msg, 0, idxOfNewLine);
    }
  ...
  static GetDefaultTimeFormat(): string {
    ...
    let timeFormat: string = NString.Concat([
...
/// </summary>
export class ContextIDGuard implements IDisposable {
