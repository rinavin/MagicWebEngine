...
export  abstract class EventsProcessor {
  ...
  private RegisterHandlers(): void {
    Events.KeyDownEvent = this.processKeyDown;
  ...
  processFormClose(guiMgForm: GuiMgForm): boolean {
    ...
    if (mgFormBase.IsMDIFrame) {
      Manager.EventsManager.addGuiTriggeredEvent(mgFormBase.getTask(), InternalInterface.MG_ACT_EXIT_SYSTEM);
    }
    else {
      if (!mgFormBase.IsHelpWindow) {
        if (MgFormBase.ShouldPutActOnFormClose())
          Manager.EventsManager.addGuiTriggeredEvent(mgFormBase.getTask(), InternalInterface.MG_ACT_HIT, RaisedBy.CLOSE_SYSTEM_MENU);
        Manager.EventsManager.addGuiTriggeredEvent(mgFormBase.getTask(), InternalInterface.MG_ACT_CLOSE);
      }
      else {
  ...
  private processWindowResize(guiMgForm: GuiMgForm): void {
    let mgFormBase: MgFormBase = <MgFormBase>guiMgForm;
    Manager.EventsManager.addGuiTriggeredEvent(mgFormBase.getTask(), InternalInterface.MG_ACT_WINSIZE);
  }
  ...
  private processWindowMove(guiMgForm: GuiMgForm): void {
    let mgFormBase: MgFormBase = <MgFormBase>guiMgForm;
    Manager.EventsManager.addGuiTriggeredEvent(mgFormBase.getTask(), InternalInterface.MG_ACT_WINMOVE);
  }
  ...
  private ProcessColumnFilter(columnCtrl: GuiMgControl, columnHeader: string, x: number, y: number, width: number, height: number): void {
    ...
    x = <number>(<number>x / resolution);
    y = <number>(<number>y / resolution);
    width = <number>(<number>width / resolution);
    height = <number>(<number>height / resolution);
