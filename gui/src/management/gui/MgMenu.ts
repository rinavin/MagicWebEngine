...
import {
  Hashtable, ICloneable, Array_Enumerator, List, NConsole,
  NString, IEnumerable
...
/// </summary>
export class MgMenu extends GuiMgMenu implements ICloneable {
  ...
  /// <returns></returns>
  Clone(): any {
    let mgMenu: MgMenu = <MgMenu>super.MemberwiseClone();
  ...
  private getDeepCopyOfMenuEntries(menuEntries: List<MenuEntry>): List<MenuEntry> {
    ...
    for (let i: number = 0; i < menuEntries.Count; i++) {
      clonedMenuEntries.Add(<MenuEntry>menuEntries.get_Item(i).Clone());
  ...
  addSubMenu(newEntry: MenuEntry): void {
    this._menuEntries.Add(newEntry);
  }
  ...
  removeAll(form: MgFormBase): void {
    for (let idx: number = 0; idx < this._menuEntries.Count; idx = idx + 1) {
      this._menuEntries.get_Item(idx).deleteMenuEntryObject(form, MenuStyle.MENU_STYLE_PULLDOWN);
    }
  ...
  createMenu(form: MgFormBase, menuStyle: MenuStyle): void {
    ...
    if (menuStyle === MenuStyle.MENU_STYLE_CONTEXT) {
      for (let i: number = 0; i < this._menuEntries.Count; i = i + 1) {
  ...
  private createToolbar(form: MgFormBase): void {
    if (!this._instantiatedToolbar.ContainsKey(form)) {
      if (!form.isSubForm()) {
        ...
        NConsole.Out.WriteLine(NString.Concat([
          "MgMenu toolbar(", this.GetHashCode(), ") menuReference (", newToolbar, ") "
        ]));
  ...
  rebuild(formsToRefresh: List<MgFormBase>): void {
    ...
    if (formsToRefresh !== null) {
      for (let i: number = 0; i < formsToRefresh.Count; i = i + 1) {
        let formToRefresh: MgFormBase = formsToRefresh.get_Item(i);
        let prop: Property = formToRefresh.getProp(442);
        if (prop !== null)
        ...
        prop = formToRefresh.getProp(441);
  ...
  enableInternalEvent(form: MgFormBase, action: number, enable: boolean, mdiChildForm: MgFormBase): void {
    ...
    if (entries !== null) {
      for (let i: number = 0; i < entries.Count; i = i + 1) {
