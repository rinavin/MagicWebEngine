...
export class MenuEntryMenu extends MenuEntry {
  ...
  addSubMenu(newEntry: MenuEntry, index?: number): void {
    if (arguments.length === 1 && (newEntry === null || newEntry instanceof MenuEntry)) {
  ...
  dispose(): void {
    for (let i: number = 0; i < this.subMenus.Count; i = i + 1) {
  ...
  protected OnSetEnabled(enabled: RefParam<boolean>, checkValidation: boolean, checkEnableSystemAction: boolean, checkForName: string, IsChildOfCheckForName: boolean, refresh: boolean): void {
    ...
    let NewIsChildOfCheckForName: boolean = IsChildOfCheckForName || checkForName === null || (this._name !== null && NString.CompareTo(this._name, checkForName) === 0);
    for (let i: number = 0; i < (<MenuEntryMenu>this).subMenus.Count; i = i + 1) {
