...
import {ICloneable, Debug, NString, Array_Enumerator, RefParam, IEnumerable} from "@magic/mscorelib";
...
/// </summary>
export class MenuEntry extends GuiMenuEntry implements ICloneable {
  ...
  /// <returns></returns>
  Clone(): any {
    let menuentry: MenuEntry = <MenuEntry>super.MemberwiseClone();
  ...
  setVisible(visible: boolean, setPropOnly: boolean, pullDownMenu: boolean, topMostTask: TaskBase, sendForm?: MgFormBase): void {
    if (arguments.length === 4 && (visible === null || visible.constructor === Boolean) && (setPropOnly === null || setPropOnly.constructor === Boolean) && (pullDownMenu === null || pullDownMenu.constructor === Boolean) && (topMostTask === null || topMostTask instanceof TaskBase)) {
  ...
  setEnabled(enabled: boolean, checkValidation: boolean, checkEnableSystemAction: boolean, checkForName?: string, refreshOrIsChildOfCheckForName?: boolean, refresh?: boolean): void {
    if (arguments.length === 3 && (enabled === null || enabled.constructor === Boolean) && (checkValidation === null || checkValidation.constructor === Boolean) && (checkEnableSystemAction === null || checkEnableSystemAction.constructor === Boolean)) {
      this.setEnabled_0(enabled, checkValidation, checkEnableSystemAction);
      return;
    }
    if (arguments.length === 5 && (enabled === null || enabled.constructor === Boolean) && (checkValidation === null || checkValidation.constructor === Boolean) && (checkEnableSystemAction === null || checkEnableSystemAction.constructor === Boolean) && (checkForName === null || checkForName.constructor === String) && (refreshOrIsChildOfCheckForName === null || refreshOrIsChildOfCheckForName.constructor === Boolean)) {
  ...
  private setEnabled_2(enabled: boolean, checkValidation: boolean, checkEnableSystemAction: boolean, checkForName: string, IsChildOfCheckForName: boolean, refresh: boolean): void {
    ...
    let activateEanble: boolean = super.menuType() === GuiMenuEntry_MenuType.MENU || checkForName === null || (this._name !== null && NString.CompareTo(this._name, checkForName) === 0);
  ...
  getGroupCount(form: MgFormBase, toolGroup: number, forMenuEntry: MenuEntry, found: RefParam<boolean>): number {
    ...
    if (this === forMenuEntry) {
      ...
      let isMenuEntryMenu: boolean = this instanceof MenuEntryMenu;
      if (isMenuEntryMenu) {
  ...
  /// <returns></returns>
  CreateMenuEntryItem(mgFormBase: MgFormBase, menuType: GuiMenuEntry_MenuType, guiMgForm: GuiMgForm, menuStyle: MenuStyle, bChecked: boolean): MenuEntry {
