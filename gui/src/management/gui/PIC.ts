...
export class PIC {
  ...
  private static _environment: IEnvironment = null;
  ...
  static PIC_cctor(): void {
    PIC._environment = Manager.Environment;
  }
  ...
  constructor(picStr: string, attr: StorageAttribute, compIdx: number) {
    ...
    if (this._picAttr === StorageAttribute.BLOB ||
      ...
      if (NString.Trim(picStr).length > 0) {
            ...
            if (this._format[tmp] === '#') {
              ...
              if (first && (tmp_c > 0 || count > 1)) {
                ...
                if (this._comma && !this._decimal)
                  this.pik_drv_fill(PICInterface.PIC_N, (tmp_c - 1) / 3);
                first = false;
              }
            }
            // add commas befor places
            else if (this._comma && !this._decimal)
              this.pik_drv_fill(PICInterface.PIC_N, (count - 1) / 3);
...
/// <returns></returns>
  ...
  getDirectiveLen(posOrMskPos: number, bufPos?: number, buf?: string): number {
    if (arguments.length === 1 && (posOrMskPos === null || posOrMskPos.constructor === Number)) {
