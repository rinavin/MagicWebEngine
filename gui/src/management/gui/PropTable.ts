...
import {XmlParser, XMLConstants, MgArrayList} from "@magic/utils";
...
export class PropTable {
  ...
  addProp(prop: Property, checkExistence: boolean): void;
  addProp(prop: Property, key: number): void;
  addProp(prop: Property, checkExistenceOrKey: any): void {
    if (arguments.length === 2 && (prop === null || prop instanceof Property) && (checkExistenceOrKey === null || checkExistenceOrKey.constructor === Boolean)) {
      this.addProp_0(prop, checkExistenceOrKey);
      return;
    }
    this.addProp_1(prop, checkExistenceOrKey);
  }
  ...
  /// </param>
  private addProp_0(prop: Property, checkExistence: boolean): void {
    if (checkExistence) {
      let existingProp: Property = <Property>this._hashTab.get_Item(prop.getID());
      if (existingProp === null)
        this.addProp(prop, prop.getID());
      else {
  ...
  private addProp_1(prop: Property, key: number): void {
  ...
  private fillExistInnerObjects(nameOfFound: string, task: TaskBase): boolean {
    ...
    if (nameOfFound === XMLConstants.MG_TAG_CONTROL)
      endContext = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex());
    else if (nameOfFound === XMLConstants.MG_TAG_PROP)
      endContext = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_TERM, parser.getCurrIndex());
    ...
    if (endContext !== -1 && endContext < parser.getXMLdata().length) {
      ...
      parser.add2CurrIndex(NString.IndexOf(tag, nameOfFound) + nameOfFound.length);
  ...
  private fillName(tokensVector: List<string>): string {
    ...
    for (let j: number = 0; j < tokensVector.Count; j = j + 2) {
  ...
  getProp(idx: number): Property {
    if (idx < 0 || idx >= this._props.Count)
      return null;
  ...
  RefreshDisplay(forceRefresh: boolean, onlyRepeatableProps: boolean): boolean {
    ...
    let visibleProps: MgArrayList<Property> = new MgArrayList();
  ...
  updatePrevValueArray(newSize: number): void {
    for (let i: number = 0; i < this._props.Count; i = i + 1) {
