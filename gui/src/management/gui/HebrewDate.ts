...
export class HebrewDate {
  ...
  private static DAYS_PER_NORM_YEAR: number = (HebrewDate.PARTS_PER_NORM_YEAR / HebrewDate.PARTS_PER_DAY);
  private static PARTS_PER_LEAP_YEAR: number = (HebrewDate.MONTHS_PER_LEAP_YEAR * HebrewDate.PARTS_PER_MONTH);
  private static DAYS_PER_LEAP_YEAR: number = (HebrewDate.PARTS_PER_LEAP_YEAR / HebrewDate.PARTS_PER_DAY);
  private static PARTS_PER_CYCLE: number = (HebrewDate.MONTHS_PER_CYCLE * HebrewDate.PARTS_PER_MONTH);
  private static DAYS_PER_CYCLE: number = (HebrewDate.PARTS_PER_CYCLE / HebrewDate.PARTS_PER_DAY);
  ...
  private static dateheb_year_start(year: number): number {
    ...
    /*---------------------------*/
    year = year - 1;
    let cycle: number = year / HebrewDate.YEARS_PER_CYCLE;
    year = year % HebrewDate.YEARS_PER_CYCLE;
    ...
    days = days + parts / HebrewDate.PARTS_PER_DAY;
    parts = parts % HebrewDate.PARTS_PER_DAY;

    /*---------------------------*/
    /* Reminder > 3/4 of a day ? */
    /*---------------------------*/
    if (<number>parts >= HebrewDate.PARTS_PER_DAY * 3 / 4) {
      parts = 0;
      days = days + 1;
    }
  ...
  static dateheb_4_d(date: number, day: RefParam<number>, month: RefParam<number>, year: RefParam<number>): boolean {
    ...
    /*--------------------------------*/
    let cycle: number = date / (HebrewDate.DAYS_PER_CYCLE + 1);
    let days: number = date % (HebrewDate.DAYS_PER_CYCLE + 1);
    year.value = cycle * HebrewDate.YEARS_PER_CYCLE;
    year.value = year.value + days / (HebrewDate.DAYS_PER_LEAP_YEAR + 1);
    ...
    if (HebrewDate._localMonths[0] === null) {
      ...
      if (monthStr !== null) {
        ...
        for (let i: number = 1; i < HebrewDate._localMonths.length; i = i + 1)
          HebrewDate._localMonths[i] = NString.Substring(monthStr, (i - 1) * monthLen, i * monthLen - (i - 1) * monthLen);
      }
    }
    ...
    if (HebrewDate._localDows[0] === null) {
      ...
      if (dowsStr !== null) {
        ...
        for (let i: number = 1; i < HebrewDate._localDows.length; i = i + 1)
          HebrewDate._localDows[i] = NString.Substring(dowsStr, (i - 1) * dowLen, i * dowLen - (i - 1) * dowLen);
      }
    }
