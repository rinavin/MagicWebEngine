...
export class TextMaskEditor {
  ...
  /// <returns></returns>
  ProcessAction(ctrl: MgControlBase, internalEventCode: number, value: string): EditReturnCode;
  ProcessAction(ctrl: MgControlBase, internalEventCode: number, value: string, start: number, end: number): EditReturnCode;
  ProcessAction(ctrl: MgControlBase, internalEventCode: number, value: string, start?: number, end?: number): EditReturnCode {
    if (arguments.length === 3 && (ctrl === null || ctrl instanceof MgControlBase) && (internalEventCode === null || internalEventCode.constructor === Number) && (value === null || value.constructor === String)) {
      return this.ProcessAction_0(ctrl, internalEventCode, value);
    }
    return this.ProcessAction_1(ctrl, internalEventCode, value, start, end);
  }

  private ProcessAction_0(ctrl: MgControlBase, internalEventCode: number, value: string): EditReturnCode {
    return this.ProcessAction(ctrl, internalEventCode, value, -1, -1);
  }

  private ProcessAction_1(ctrl: MgControlBase, internalEventCode: number, value: string, start: number, end: number): EditReturnCode {
            ...
            if (endOfSeq >= editParams.CtrlText.Length) {
              ...
              if (len > 0) {
                ...
                for (; startPos < endOfSeq; startPos++, endPos++) {
                  ...
                  if (endPos < endOfSeq)
                    editParams.CtrlText[startPos] = editParams.CtrlText[endPos];
                  else {
                    editParams.CtrlText[startPos] = ' ';
      ...
      // char is inserted before the end of existing text
        ...
        if (!isFullWidth) {
          ...
          // writing on the last blank in seq, nothing is deleted, only blank is replaced.
            ...
            editParams.CtrlText[editParams.StartPos] = charToWrite;
          }
        }
        else {
  ...
  private actionCut(ctrl: MgControlBase, editParams: EditParams): EditReturnCode {
    ...
    if (editParams.StartPos !== editParams.EndPos)
      Manager.EventsManager.addInternalEvent(ctrl, InternalInterface.MG_ACT_EDT_DELCURCH, Priority.HIGH);
      ...
      // for blob, copywhatever in the clipboard
        ...
        lenToPaste = sizeLimit -
          (UtilStrByteMode.lenB(strCtrlText) -
            (UtilStrByteMode.lenB(strCtrlText) - UtilStrByteMode.lenB(NString.Substring(strCtrlText, editParams.StartPos,
              editParams.EndPos - editParams.StartPos))));
        ...
        if (UtilStrByteMode.isLocaleDefLangDBCS() && pic.isAttrAlpha()) {
          ...
          this.guiSetText(ctrl,
                    editParams.CtrlText.ToString(0, editParams.StartPos) + strPasteText +
                          NString.Trim(editParams.CtrlText.ToString(editParams.EndPos, editParams.CtrlText.Length - editParams.EndPos), removeChars));
        }
        else {
          ...
          this.guiSetText(ctrl,
                    editParams.CtrlText.ToString(0, editParams.StartPos) + NString.Substring(content, 0, lenToPaste) +
                         NString.Trim(editParams.CtrlText.ToString(editParams.EndPos, editParams.CtrlText.Length - editParams.EndPos), removeChars));
        }
  ...
  private editPasteWithMask(ctrl: MgControlBase, editParams: EditParams, pasteStr: string): boolean {
    ...
    if (pasteStrText !== null && pasteStrText.Length > 0 && editParams.StartPos !== editParams.EndPos)
      if (!this.overWriteChar(pasteStrText[posOnPasteStr], ctrl, editParams, true))
        pasteSuccess = false;
    ...
    if (pasteSuccess) {
      ...
      while (posOnPasteStr < pasteStrLen && maskPos < maskLen) {
        ...
        if (maskPos < maskLen) {
          ...
          insertChar = pasteStrText[posOnPasteStr];
          posOnPasteStr++;
  ...
  private edtPasteSkipMaskCheck(pic: PIC, pos: number, pasteStrText: StringBuilder, posOnPasteStr: number): number {
    ...
    for (iSkip = 0; pos < pic.getMaskLength() && (posOnPasteStr + iSkip) < pasteStrText.Length; iSkip++, pos++) {
      ...
      if (pic.isMaskPicEq(pos, pasteStrText[posOnPasteStr + iSkip]))
        continue;
...
/// <returns></returns>
  ...
  private convPosBufToMsk(ctrl: MgControlBase, posOrEditParams: any, pos ?: number): number {
    if (arguments.length === 2 && (ctrl === null || ctrl instanceof MgControlBase) && (posOrEditParams === null || posOrEditParams.constructor === Number)) {
