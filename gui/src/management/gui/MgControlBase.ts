import {
  ApplicationException, Debug, Exception, NNumber,
  NString, StringBuilder, List
...
import {MgArrayList, StorageAttribute, ChoiceUtils, UtilStrByteMode, XMLConstants, MgControlType,  TableBehaviour,
  StrUtil, XmlParser, Misc, SEQ_2_STR, AutoFit, FramesetStyle, AlignmentTypeHori, ListboxSelectionMode, MultilineHorizontalScrollBar,
  StorageAttributeCheck, Constants, InternalInterface} from "@magic/utils";
...
import {CommandType, MenuStyle, MgCheckState} from "../../GuiEnums";
...
export abstract class MgControlBase extends GuiMgControl implements PropParentInterface {
  ...
  _choiceDisps: MgArrayList<string[]> = null; // for SELECT controls, holds the
  _choiceLayerList: MgArrayList<string[]> = null; // the order on which tabs to be displayed
  _choiceLinks: MgArrayList<string[]> = null; // for SELECT controls, holds the
  _choiceNums: MgArrayList<NUM_TYPE[]> = null;  // for SELECT controls, with numeric
  private _containerDitIdx: number = -1;
  _currReadOnly: MgArrayList<boolean> = null; // a history array for readonly param
  private _dataCtrl: boolean = false; // The data control flag states that this
  _dcTableRefs: MgArrayList<number> = null; // references to dcVals for each line of a control is a data control
  ...
  _orgChoiceDisps: MgArrayList<string[]> = null; // for TAB Control save the org option (with the &)
  _parentTable: MgControlBase = null; // reference to parent Magic Table for
  _pic: PIC = null;
  private _picExpExists: boolean = false;
  _prevIsNulls: MgArrayList<boolean> = null; // for NULL
  private _prevPicExpResult: string = null;
  _prevValues: MgArrayList<string> = null; // references to previous values of the
  ...
  getProp(propId: number): Property {
    ...
    if (this._propTab !== null) {
      ...
      if (prop === null) {
        prop = PropDefaults.getDefaultProp(propId, 'C', /*'C'*/this);
  ...
  fillData(mgForm: MgFormBase, ditIdx: number): void {
    ...
    if (this._picStr !== null && this.DataType > StorageAttribute.NONE) {
  ...
  private ParseSourceTable(): void {
    ...
    let sourceTableReference: ObjectReference = ObjectReference.FromXML(NString.Trim(objRefStr));
  ...
  private parseAttributes(): void {
    let parser: XmlParser = Manager.GetCurrentRuntimeContext().Parser;
    let endContext: number = NString.IndexOf(parser.getXMLdata(), ">", parser.getCurrIndex());

    if (endContext !== -1 && endContext < parser.getXMLdata().length) {

      // last position of its tag
      let tag: string = parser.getXMLsubstring(endContext);
      parser.add2CurrIndex(NString.IndexOf(tag, XMLConstants.MG_TAG_CONTROL.length) + XMLConstants.MG_TAG_CONTROL.length);

      let tokens: List<string> = XmlParser.getTokens(parser.getXMLsubstring(endContext), XMLConstants.XML_ATTR_DELIM)/*'"'*/;

      for (let i: number = 0; i < tokens.Count; i = i + 2) {
  ...
  setImage(fileName?: string): void {
    if (arguments.length === 1 && (fileName === null || fileName.constructor === String)) {
      this.setImage_0(fileName);
      return;
    }
    this.setImage_1();
  }

  private setImage_0(fileName: string): void {
    if (fileName === null) {
      fileName = "";
    }
    ...
    Commands.addAsync(CommandType.PROP_SET_IMAGE_FILE_NAME, this, this.getDisplayLine(false), fileName, imageStyleProp.getValueInt());
  ...
  getPrevIsNull(): boolean {
    ...
    let currObj: any = this._prevIsNulls[line];
    let curr: boolean;
    if (currObj === null) {
      curr = false;
      this._prevIsNulls[line] = curr;
    }
    else {
  ...
  setPrevIsNull(newValue: boolean): void {
    this._prevIsNulls[this.getDisplayLine(true)] = newValue;
  }
  ...
  setPIC(picStr: string): void {
    ...
    if (this._dataCtrl && NString.IndexOf(this._picStr, 'H') > -1) {
      this._pic.setHebrew();
    }
  }
  ...
  getCurrentIndexOfChoice(): number[] {
    ...
    if (!NString.IsNullOrEmpty(val)) {
      let array2: string[] = NString.Split(val, [
        ',']/*','*/);
  ...
  getIndexOfChoice(mgVal: string, line: number, isNull: boolean): number[] {
    ...
    return this.getDcVals().getIndexOf(mgVal, isVectorValue, isNull, this._choiceLinks[line], this._choiceNums[line], splitCommaSeperatedVals);
  }
  ...
  getDispVals(line: number, execComputeChoice: boolean): string[] {
    ...
    if (super.isTabControl()) {
      return this._orgChoiceDisps[line];
    }
    else {
      return  this._choiceDisps[line];
    }
  ...
  getMaxDisplayItems(): number {
    let maxLayer: number = this._choiceLayerList[0].length;

    if (maxLayer === 0) {
      let dispValsSrcData: string[] = this.getDcVals().getDispVals();

      maxLayer = this._choiceDisps[0].length;
  ...
  private emptyChoice(line: number): void {
    ...
    this._choiceDisps[line] = value;
    this._orgChoiceDisps[line] = value;
    this._choiceLinks[line] = value;
    this._choiceLayerList[line] = value;
  }
  ...
  private computeChoice(line: number): void {
    ...
    try {
      ...
      if (this._choiceLinks[line] === null || (this.isDataCtrl() && currDcId !== this._dcValId)) {
        ...
        this._choiceDisps[line] = choiceDisp;
        ...
        {
          nextPos = currPos;
          nextPos = NString.IndexOf(helpStr, ',', nextPos);

          if (nextPos === currPos)
            token = helpToken = "";
          else if (nextPos === -1)
          {
            token = NString.Substring(choiceLinkStr, currPos);
            helpToken = NString.Substring(helpStr, currPos);
          }
          else
          {
            token = NString.Substring(choiceLinkStr, currPos, (nextPos) - (currPos));
            helpToken = NString.Substring(helpStr, currPos, (nextPos) - (currPos));
          }
          ...
          switch (dataType)
          {
            ...
            case StorageAttribute.BLOB_VECTOR:
              ...
              if (NString.IndexOf(helpToken, '\\') >= 0)
              {
              ...
              token = StrUtil.makePrintableTokens(token, SEQ_2_STR.toString());
              ...
              else
              {
                if (trimToLength !== -1 && token.length > trimToLength)
                  token = NString.Substring(token, 0, trimToLength);
              }
            ...
            case StorageAttribute.TIME:
              ...
              choiceLink[i] = DisplayConvertor.Instance.disp2mg(NString.Trim(token), choiceLinkStr, picture,
                task.getCompIdx(),
                BlobType.CONTENT_TYPE_UNKNOWN);
              ((this._choiceNums[line]))[i] = new NUM_TYPE(choiceLink[i]);
              break;
  ...
  getMgValue(dispVal: string): string {
  ...
  getLinkValue(selectedIndice: string, line: number): string {
    ...
    let choiceLink: string[] = this._choiceLinks[line];
    ...
    for (let i: number = 0; i < array2.length; i = i + 1)
    {
      ...
      if (idx >= size) {
        ...
        temp.Add(dcVals.getLinkValue(idx - size));
      }
      else {
    ...
    if (this.DataType === StorageAttribute.BLOB_VECTOR) {
      ...
      for (let indx: number = 0; indx < temp.Count; indx = indx + 1) {
  ...
  private setText(): void {
    ...
    let mlsTranslatedValue: string = this.Value;
    if (super.isButton()) {
  ...
  private setRadioChecked(radioLine: number): void {
    if (this._siblingVec !== null && radioLine >= 0) {
      for (let i: number = 0; i < this._siblingVec.Count; i = i + 1) {
  ...
  setDcRef(dcId: number): void {
    this._dcTableRefs[this.getDcLineNum()] = dcId;
  }
  ...
  updateChildrenPropValue(propId: number, commandType: CommandType, val: boolean): void {
    ...
    if (!super.isTableControl()) {
      ...
      for (let i: number = 0; i < this._linkedControls.Count; i = i + 1) {
  ...
  SetCurrReadOnly(newVal: boolean): void {
    this._currReadOnly[this.getDisplayLine(true)] = newVal;
  }
  ...
  GetCurrReadOnly(): boolean {
    ...
    let currObj: any = this._currReadOnly[line];
    let curr: boolean;
    if (currObj === null) {
      curr = false;
      this._currReadOnly[line] = curr;
    }
    else {
      curr = <boolean>currObj;
    }
  ...
  getDcRef(): number {
    ...
    let obj: any = this._dcTableRefs[this.getDcLineNum()];
    let dcId: number;
    if (obj === null) {
      dcId = -2;
      this._dcTableRefs[line] = dcId;
    }
    else {
      dcId = <number>obj;
    }
  ...
  private optionIsValid(option: string): boolean {
    ...
    if (this.DataType === StorageAttribute.NUMERIC && option.length > 0 && NString.IndexOf(option, '0') === -1) {
  ...
  private getOrderedDispList(choiceDispList: string[], line: number): string[] {
    ...
    if (choiceLayerStr.length > 0 && choiceDispList.length > 0) {
      let layerList: string[] = NString.Split(choiceLayerStr, ",");
      let posCopyIndex: number = -1;
      for (let index: number = 0; index < layerList.length; index = index + 1) {
        let currLayerListVal: string = NString.Trim(layerList[index]);
        if (!(layerArryList.IndexOf(currLayerListVal) !== -1)) {
          ...
          if (isNumber && posCopyIndex > 0 && posCopyIndex <= choiceDispList.length) {
            dispArryList.Add(NString.Trim(choiceDispList[posCopyIndex - 1]));
    ...
    if (dispArryList.Count === 0) {
      ...
      this._choiceLayerList[line] = new Array<string>(0);
    }
    else {
      result = new Array<string>(dispArryList.Count);
      for (let _ai: number = 0; _ai < result.length; ++_ai)
        result[_ai] = null;

      result = dispArryList.ToArray();
      this._choiceLayerList[line] = layerArryList.ToArray();
    }
    return result;
  }
  ...
  setPrevIsNull_ToNull(): void {
    this._prevIsNulls[this.getDisplayLine(true)] = null;
  }
  ...
  clearRange(line: number): void {
    this._orgChoiceDisps[line] = null;
    this._choiceDisps[line] = null;
    this._choiceLinks[line] = null;
    this._choiceLayerList[line] = null;
  }
  ...
  checkProp(propId: number, defaultRetVal: boolean, line: number): boolean;
  checkProp(propId: number, defaultRetVal: boolean): boolean;
  checkProp(propId: number, defaultRetVal: boolean, line?: number): boolean {
    if (arguments.length === 2 && (propId === null || propId.constructor === Number) && (defaultRetVal === null || defaultRetVal.constructor === Boolean)) {
      return this.checkProp_0(propId, defaultRetVal);
    }
    return this.checkProp_1(propId, defaultRetVal, line);
  }
  ...
  /// <param name = "propId">the id of the property</param>
  /// <param name = "defaultRetVal">the default return value (when the property does not exist)</param>
  private checkProp_0(propId: number, defaultRetVal: boolean): boolean {
    let result: boolean = defaultRetVal;
  ...
  /// <param name = "defaultRetVal">the default return value (when the property does not exist)</param>
  /// <param name = "line">the line number in table in which the control is located</param>
  private checkProp_1(propId: number, defaultRetVal: boolean, line: number): boolean {
    let result: boolean = defaultRetVal;
  ...
  copyValFrom(fromControl: MgControlBase): void {
    ...
    let savePrevValue: string = this._prevValues[line];
  ...
  setValueForEditSet(savedValue: string, savePrevValue: string, savedIsNull: boolean, savePrevIsNull: boolean): void {
    ...
    this._prevValues[this.getDisplayLine(true)] = savePrevValue;
    ...
    if (dcRangeIsEmpty) {
      if (!super.isCheckBox() && (this._field === null || !isNull || !this._field.hasNullDisplayValue() || fromEditSet)) {
        try {
          if ((super.isImageControl() && this.DataType === StorageAttribute.BLOB) || super.IsImageButton()) {
            ...
            if ((this._field === null && isNull) && (this.DataType === StorageAttribute.NUMERIC || this.DataType === StorageAttribute.DATE || this.DataType === StorageAttribute.TIME || this.DataType === StorageAttribute.BOOLEAN)) {
              ...
              this.Value = DisplayConvertor.Instance.mg2disp(mgVal, this._range, this._pic, super.isSelectionCtrl(), this.getForm().getTask().getCompIdx(), inControl);
            }
          }
          ...
          if (!this._pic.isAttrNumeric() || this._pic.isLeft()) {
            this.rtrimValue();
          }
        ...
        if (isNull && !this._pic.isAttrBlob() && mgVal.length > this._pic.getMaskLength()) {
          mgVal = NString.Substring(mgVal, 0, this._pic.getMaskLength());
        }
        this.Value = mgVal;
      }
    }
  ...
  ValidateValue(newValue: string, vd: ValidationDetails[]): boolean {
    vd[0] = null;
    let ctrlCurrValue: string = this.Value;

    if (this.getPIC() === null)
      return true;

    vd[0] = this.buildPicture(ctrlCurrValue, newValue);
    vd[0].evaluate();

    return !vd[0].ValidationFailed;
  }
  ...
  private rtrimValue(): void {
    if ((super.isImageControl() && this.DataType === StorageAttribute.BLOB) || super.IsImageButton()) {
      ...
      if (this.Value.length > minimumValueLength) {
        let str: string = NString.Substring(this.Value, minimumValueLength);

        // copy the minimum length value as is
        this.Value = NString.Substring(this.Value, 0, minimumValueLength);
  ...
  RefreshDisplayValue(mgVal: string): void {
    ...
    try {
      ...
      let prevValue: string = this._prevValues[line];
  ...
  RefreshDisplay(onlyRepeatableProps?: boolean): void {
    if (arguments.length === 0) {
      this.RefreshDisplay_0();
      return;
    }
    this.RefreshDisplay_1(onlyRepeatableProps);
  }

  private RefreshDisplay_0(): void {
  ...
  refreshAndSetItemsListForRadioButton(line: number, execComputeChoice: boolean): void {
    ...
    if (!this.IsRepeatable) {
      ...
      if (this.getProp(PropInterface.PROP_TYPE_IMAGE_FILENAME) !== null) {
        this.getProp(PropInterface.PROP_TYPE_IMAGE_FILENAME).RefreshDisplay(true);
      }
  ...
  /// <param name="val"></param>
  /// <returns></returns>
  buildPicture(oldVal: string, val: string): ValidationDetails {
  ...
  /// <returns></returns>
  isDifferentValue(NewValue: string, isNull: boolean, checkNullValue: boolean): boolean {
    let ctrlCurrValue: string = this.Value;
    let ctrlCurrIsNull: boolean = this.IsNull;
    let valChanged: boolean = !(StrUtil.rtrim(NewValue) === StrUtil.rtrim(ctrlCurrValue));
  ...
  getRect(): MgRectangle {
    ...
    mgRectangle.x = NNumber.Parse(this.getProp(21).getValue());
    mgRectangle.y = NNumber.Parse(this.getProp(22).getValue());
    mgRectangle.width = NNumber.Parse(this.getProp(23).getValue());
    mgRectangle.height = NNumber.Parse(this.getProp(24).getValue());
  ...
  private SetRowPlacement(): void {
    Debug.Assert(super.isTableControl());
    let hasRowPlacement: boolean = this.getProp(PropInterface.PROP_TYPE_ROW_PLACEMENT).getValueBoolean();
  ...
  private SetTableOrgRowHeight(): void {
    Debug.Assert(super.isTableControl());

    // get coordinates that were defined on the controls originally
    let rowHeight: number = NNumber.Parse(this.getProp(PropInterface.PROP_TYPE_ROW_HEIGHT).getOrgValue());
    rowHeight = this.Form.uom2pix(<number>rowHeight, false);
  ...
  /// </summary>
  private setColumnOrgWidth(): void {

    // get coordinates that were defined on the controls originally,
    // i.e. before expression executed
    let width: number = NNumber.Parse(this.getProp(PropInterface.PROP_TYPE_WIDTH).getOrgValue());
    width = this.Form.uom2pix(<number>width, true);
  ...
  /// <returns></returns>
  getOrgWidth(): number {
    return NNumber.Parse(this.getProp(23).getOrgValue());
  }
  /// <summary>
  /// </summary>
  /// <returns>true if the current control is wide control</returns>
  isWideControl(): boolean {
  ...
  IsWideAllowed(): boolean {
    ...
    if (super.isTextControl()) {
      switch (this.DataType)
      {
        ...
        case StorageAttribute.MEMO:
        case StorageAttribute.UNICODE:
          if ((!this._propTab.propExists(PropInterface.PROP_TYPE_PASSWORD) || !this.checkProp(PropInterface.PROP_TYPE_PASSWORD, false)) && !this.isMultiline()
            && (this._pic.getSize() === this._pic.getMaskSize()))
            allow = true;
          break;
      }
    }
  ...
  ///   open\close the wide
  /// </summary>
  onWide(forceClose: boolean): boolean {
    let wideToggled: boolean = true;
    let topMostForm: MgFormBase = this.getTopMostForm();
  ...
  setWideProperties(parentForm: MgFormBase, parentControl: MgControlBase, rectWide: MgRectangle): void {
    this.IsWideControl = true;
    ...
    this.setField(parentControl.getField());
    this.setControlToFocus();
    ...
    this._pic = parentControl.getPIC();
    this.DataType = parentControl.DataType;

    let num: NUM_TYPE = new NUM_TYPE();

    // set properties of wide Control
    num.NUM_4_LONG(rectWide.x);
    this.setProp(PropInterface.PROP_TYPE_LEFT, num.toXMLrecord());

    num.NUM_4_LONG(rectWide.y);
    this.setProp(PropInterface.PROP_TYPE_TOP, num.toXMLrecord());

    num.NUM_4_LONG(rectWide.width);
    this.setProp(PropInterface.PROP_TYPE_WIDTH, num.toXMLrecord());

    num.NUM_4_LONG(rectWide.height);
    this.setProp(PropInterface.PROP_TYPE_HEIGHT, num.toXMLrecord());

    this.setProp(PropInterface.PROP_TYPE_MULTILINE, "0");
    this.setProp(PropInterface.PROP_TYPE_VISIBLE, "1");

    let val: string = parentControl.isModifiable() ? "1" : "0";

    this.setProp(PropInterface.PROP_TYPE_MODIFIABLE, val);
    this.setProp(PropInterface.PROP_TYPE_BORDER, "1");

    let val2: string = this._pic.isHebrew() ? "1" : "0";

    this.setProp(PropInterface.PROP_TYPE_HEBREW, val2);

    num.NUM_4_LONG(MultilineHorizontalScrollBar.WordWrap);
    this.setProp(PropInterface.PROP_TYPE_MULTILINE_WORDWRAP_SCROLL, num.toXMLrecord());
    ...
    this.setProp(PropInterface.PROP_TYPE_MULTILINE, "1");
    num.NUM_4_LONG(parentControl.getProp(PropInterface.PROP_TYPE_COLOR).getValueInt());
    this.setProp(PropInterface.PROP_TYPE_COLOR, num.toXMLrecord());

    num.NUM_4_LONG(parentControl.getProp(PropInterface.PROP_TYPE_FONT).getValueInt());
    this.setProp(PropInterface.PROP_TYPE_FONT, num.toXMLrecord());

    num.NUM_4_LONG(parentControl.getProp(PropInterface.PROP_TYPE_TRANSLATOR).getValueInt());
    this.setProp(PropInterface.PROP_TYPE_TRANSLATOR, num.toXMLrecord());

    let horAligment: number = parentControl.getProp(PropInterface.PROP_TYPE_HORIZONTAL_ALIGNMENT).getValueInt();
    let specialEditLeftAlign: boolean = Manager.GetSpecialEditLeftAlign();
    if (specialEditLeftAlign) {
      horAligment = AlignmentTypeHori.Left;
    }
    num.NUM_4_LONG(horAligment);
    this.setProp(PropInterface.PROP_TYPE_HORIZONTAL_ALIGNMENT, num.toXMLrecord());
  }
  ...
  AutoWideModeCheck(act: number): void {


    if (this.checkProp(PropInterface.PROP_TYPE_AUTO_WIDE, false))
    {
      let lenCheck: boolean = false;
      let CheckAutoWide: boolean = true;
      ...
      if (CheckAutoWide)
      {
        Commands.checkAutoWide(this, this.getDisplayLine(true), lenCheck);
      }
    }
  }
  ...
  SetKeyboardLanguage(restoreLang: boolean): void {
    ...
    if (super.isChoiceControl()) {
      setKeyBordLan = this.getProp(130).getValueBoolean();
    }
    if (setKeyBordLan) {
  ...
  refreshDefaultButton(): void {
    let prop: Property = this.Form.getProp(408);
    prop.RefreshDisplay(true);
  }
  ...
  /// </summary>
  OnSubformClick(): void {
    Manager.EventsManager.addGuiTriggeredEvent(this.getForm().getTask(), InternalInterface.MG_ACT_HIT);
  }
  ...
  ToString(): string {
    return NString.Concat([
      "{", super.GetType().Name, ": ", MgControlType[this.Type], "}"
    ]);
  }
