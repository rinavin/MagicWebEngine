...
export abstract class TaskBase implements ITask, PropParentInterface {
  ...
  setProp(propId: number, val: string): void;
  setProp(propId: number, val: any): void {
    if (arguments.length === 2 && (propId === null || propId.constructor === Number) && (val === null || val.constructor === String)) {
      this.setProp_0(propId, val);
      return;
    }
    this.setProp_1(propId, val);
  }
  ...
  private setProp_0(propId: number, val: string): void {
  ...
  /// <param name = "propId">the id of the property</param>
  /// <param name = "val">the value of the property</param>
  private setProp_1(propId: number, val: string): void {
    let checkPaste: boolean = false;
    if (PropInterface.PROP_TYPE_TASK_MODE && this.getMode() !== val) {
      checkPaste = true;

      if (this._flowMonitor !== null)
        this._flowMonitor.addTaskCngMode(this.ContextID, val);
    }
    this.setProp(propId, val + "");
  ...
  getFieldByValueStr(valueStr: string, parent?: RefParam<number>, vee?: RefParam<number>): FieldDef {
    if (arguments.length === 1 && (valueStr === null || valueStr.constructor === String)) {
  ...
  private getFieldByValueStr_1(valueStr: string, parent: RefParam<number>, vee: RefParam<number>): FieldDef {
    ...
    let comma: number = NString.IndexOf(valueStr, ",");
    if (comma > 0) {
      let parentId: number = NNumber.Parse(NString.Substring(valueStr, 0, comma));
      let fldIdx: number = NNumber.Parse(NString.Substring(valueStr, comma + 1));
  ...
  getFieldDef(fldIdOrParent: number, fldIdx?: number): FieldDef {
    if (arguments.length === 1 && (fldIdOrParent === null || fldIdOrParent.constructor === Number)) {
  ...
  fillAttributes(): void {
    ...
    let endContext: number = NString.IndexOf(parser.getXMLdata(), XMLConstants.MG_TAG_TASK, parser.getCurrIndex());
    if (endContext !== -1 && endContext < parser.getXMLdata().length) {
      // last position of its tag
      let tag: string = parser.getXMLsubstring(endContext);
      parser.add2CurrIndex(NString.IndexOf(tag, XMLConstants.MG_TAG_TASK) + XMLConstants.MG_TAG_TASK.length);

      let tokens: List<string> = XmlParser.getTokens(parser.getXMLsubstring(endContext), XMLConstants.XML_ATTR_DELIM);

      for (let j: number = 0; j < tokens.Count; j = j + 2) {
      ...
      parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex()) + 1); // to delete ">" too
    }
  }
  ...
  initInnerObjects(foundTagName: string, parentForm: MgFormBase): boolean {
    ...
    switch (foundTagName) {
      ...
      case XMLConstants.MG_ATTR_MENU_CONTENT:
        ...
        parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), XMLConstants.MG_ATTR_MENU_CONTENT, parser.getCurrIndex()) +
          XMLConstants.MG_ATTR_MENU_CONTENT.length + 1);
        let endContext: number = NString.IndexOf(parser.getXMLdata(), "</" + XMLConstants.MG_ATTR_MENU_CONTENT, parser.getCurrIndex());
      ...
      case XMLConstants.MG_TAG_ASSEMBLIES:
        ...
        endContext = NString.IndexOf(parser.getXMLdata(), '/' + XMLConstants.MG_TAG_ASSEMBLIES,
          parser.getCurrIndex());
        parser.setCurrIndex(endContext);
        parser.setCurrIndex2EndOfTag();
        break;
  ...
  private setCtlAndCompIdx(valueStr: string): void {
    let i: number = NString.IndexOf(valueStr, ",");
    if (i > -1) {
      this._ctlIdx = NNumber.Parse(NString.Substring(valueStr, 0, i));
      this._compIdx = NNumber.Parse(NString.Substring(valueStr, i + 1));
    }
  }
