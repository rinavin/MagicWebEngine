...
export class UtilDateJpn {
  ...
  private date_jpn_yr_4_a(ucp_str: string, s_len: number): number {
    ...
    if (i > 0) {
      ...
      const text: string = NString.Substring(ucp_str, 0, 1);
      if (text === " " || text === "?") {
        result = 1;
      }
      else {
  ...
  private static getEraNameStrInDate(strDate: string, strPicture: string, intStartPos: number): string {
    ...
    while (i + intPicIdxOfs < strPicture.length) {
      ...
      result = NString.Substring(strDate, i, intLetters);
      break; // exit loop
    }
    return result;
  }
      ...
      if (array[i].length > 0) {
        ...
        if (array2.length === 2 && array2[0].length > 0 && array2[1].length > 0 && array2[2].length > 0) {
          ...
          if (array3.length === 2 && array3[0].length > 0 && array3[1].length > 0 && array3[2].length > 0) {
            UtilDateJpn.GengoStr[5 + i][0] = NString.Substring(array2[2], 0, 1); // symbol name (upper case): A
            UtilDateJpn.GengoStr[5 + i][1] = NString.Substring(array2[2], 1, 1); // symbol name (lower case): a
            UtilDateJpn.GengoStr[5 + i][2] = NString.Substring(array2[2], 2);    // gengo name: ABCD
