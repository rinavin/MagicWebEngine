import {Stack, Encoding, NChar, NString, NNumber, StringBuilder, Hashtable} from "@magic/mscorelib";
...
export class Rtf {
  ...
  static isRtf(str: string): boolean {
    ...
    if (str !== null && NString.StartsWith(str, this.RTF_PREFIX)) {
      isRtf = true;
    }
    return isRtf;
  }
  ...
  public toTxt(rtfTxt: string, outputTxt: StringBuilder): Rtf_ErrorRtf {
    ...
    if (currPos >= 3 && (outputTxt[currPos - 3] === Rtf_RtfChar.CR && outputTxt[currPos - 2] === Rtf_RtfChar.LF && outputTxt[currPos - 1] === Rtf_RtfChar.CR || outputTxt[currPos - 3] === Rtf_RtfChar.LF && outputTxt[currPos - 2] === Rtf_RtfChar.CR && outputTxt[currPos - 1] === Rtf_RtfChar.CR))
  ...
  private ParseKeyword(rtfTxt: string, outputTxt: StringBuilder): Rtf_ErrorRtf {
    ...
    if (NString.CompareOrdinal(szKeyword, Rtf.CHAR_PAR) === 0) {
  ...
  private TranslateKeyword(szKeyword: string, outputTxt: StringBuilder): Rtf_ErrorRtf {
    ...
    for (isym = 0; isym < Rtf.rgsymRtf.length; isym++) {
      if (NString.CompareOrdinal(szKeyword, Rtf.rgsymRtf[isym].szKeyword) === 0) {
        break;
      }
    }
