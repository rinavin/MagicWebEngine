...
export class UtilStrByteMode  {
  private static _bLocaleDefLangJPN: boolean = false;
  private static _bLocaleDefLangCHN: boolean = false;
  private static _bLocaleDefLangKOR: boolean = false;

  /// <summary> static block to initialize static members </summary>
  static UtilStrByteMode_cctor(): void {
    // TODO : need to check what to do with CultureInfo
    // let twoLetterISOLanguageName: string = " "; //CultureInfo.CurrentCulture.TwoLetterISOLanguageName;
    let twoLetterISOLanguageName: string = " ";
    UtilStrByteMode._bLocaleDefLangJPN = (twoLetterISOLanguageName === "ja");
    UtilStrByteMode._bLocaleDefLangCHN = (twoLetterISOLanguageName === "zh");
    UtilStrByteMode._bLocaleDefLangKOR = (twoLetterISOLanguageName === "ko");
  }
    ...
    if (len <= 0) {
      ...
      if (intByteLength < ofs) {
        ...
        for (let intIndex: number = 0; intIndex < strVal.length; intIndex = intIndex + 1) {
          let s: string = NString.Substring(strVal, intIndex, 1);
          intByteLength = intByteLength + Encoding.Default.GetByteCount(s);
        ...
        // execute Mid
        if (bHeadSpace) {
          strbufAddingBuf.Append(32)/*' '*/;
        }

        if (intValidMinIndex <= intValidMaxIndex) {
          strbufAddingBuf.Append(NString.Substring(strVal, intValidMinIndex, intValidMaxIndex + 1 - intValidMinIndex));
        }
  ...
  static instrB(strTarget: string, strSearch: string): number {
    ...
    let ofs: number = NString.IndexOf(strTarget, strSearch);
    ...
    return UtilStrByteMode.lenB(NString.Substring(strTarget, 0, ofs)) + 1;
  }
  ...
  static repC(strTarget: string, strOrigin: string, ofs: number, len: number): string {
    ...
    strbufAddingBuf.Append(NString.Substring(strTarget, 0, ofs));
    ...
    strbufAddingBuf.Append(NString.Substring(strOrigin, 0, len));

    let intRightLen: number = strTarget.length - (ofs + len);

    if (intRightLen > 0) {
      strbufAddingBuf.Append(NString.Substring(strTarget, ofs + len));
    }
  ...
  static convPos(strSource: string, strDest: string, pos: number, isAdvance: boolean): number {
    ...
    if (pos > strSource.length)
      ...
      let byteSource: Uint8Array = Encoding.Default.GetBytes(NString.Substring(strSource, 0, pos));
  ...
  static strcmp(str1: string, str2: string): number {
    let array1: Uint8Array = Encoding.Default.GetBytes(str1);
    let array2: Uint8Array = Encoding.Default.GetBytes(str2);
