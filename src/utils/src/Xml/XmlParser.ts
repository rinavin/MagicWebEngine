...
import {MgArrayList} from "../MgArrayList";
...
export class XmlParser {
  ...
  private _history: MgArrayList<number | string> = new MgArrayList(); // In order to allow recursive parsing we save prev data
  ...
  static getTokens(str: string, delimiter: string, isMagicXML: boolean = false): List<string> {
  ...
  push(): void {
    this._history.Add(this._currIndex);
    this._history.Add(this._xmLdata);
  }

  /// <summary> restore the previous parsing information from the history stack</summary>
  pop(): void {
    let count: number = this._history.Count;

    this._xmLdata = <string>this._history[count - 1];
    this._currIndex = <number>this._history[count - 2];
