...
import {
  DateTime, DateTimeFormatInfo, List, NNumber, NObject, NString, StringBuilder,
  Type
} from "FlowMonitorQueue";
import {InternalInterface, XMLConstants, Constants, XmlParser, DateTimeUtils, Queue, Logger} from "@magic/utils";
...
export class FlowMonitorQueue implements IFlowMonitorQueue {
  ...
  _formatter: DateTimeFormatInfo = null;
  ...
  constructor() {
    this._formatter = new DateTimeFormatInfo();
    this._formatter.LongTimePattern = "HH:mm:ss:fff";
    this._formatter.DateSeparator = "";
    this._formatter.LongDatePattern = "";
    this._formatter.ShortDatePattern = "";
  }
  ...
  fillData(): void {
    ...
    if (endContext !== -1 && endContext < parser.getXMLdata().length) {
      ...
      parser.add2CurrIndex(NString.IndexOf(tag, "flwmtr_config") + "flwmtr_config".length);
...
class ActivityItem {
  ...
  private _type: string; // ACTY_TASK|ACT_TASK_FLW|ACT_DATAVIEW|ACT_RECOMPUTE|ACT_FLW_OPER
  ...
  private setTime(): void {
    this._time = DateTimeUtils.ToString(DateTime.Now, this._enclosingInstance._formatter);
  }
