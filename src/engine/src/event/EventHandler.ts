import {ApplicationException, Exception, List, NObject, NString, RefParam} from "@magic/mscorelib";
import {
  Constants, XMLConstants, XmlParser, Logger, StorageAttributeCheck,
  BrkScope, Logger_LogLevels, StorageAttribute, InternalInterface
} from "@magic/utils";
...
export class EventHandler {
  ...
  private _scope: BrkScope; // Task|Subtask|Global
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string): boolean {
    ...
    if (this._evt.getType() === 'ConstInterface.EVENT_TYPE_TIMER || (this._evt.getType() === ConstInterface.EVENT_TYPE_USER && this._evt.getUserEventType() === ConstInterface.EVENT_TYPE_TIMER)')
  ...
  private initInnerElements(parser: XmlParser, taskRef: Task): void {
    ...
    if (endContext !== -1 && endContext < parser.getXMLdata().length) {
      ...
      parser.add2CurrIndex(NString.IndexOf(tag, ConstInterface.MG_TAG_HANDLER) + ConstInterface.MG_TAG_HANDLER.length);
    ...
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      ...
      switch (attribute) {
        ...
        case ConstInterface.MG_ATTR_DVPOS:
          ...
          if (i > -1) {
            this._dvLen = XmlParser.getInt(NString.Substring(valueStr, 0, i));
            this._dvPos = XmlParser.getInt(NString.Substring(valueStr, i + 1));
        ...
        default:
          Logger.Instance.WriteExceptionToLog("There is no such tag in EventHandler class. Insert case to EventHandler.initElements for " + attribute);
          break;
      }
    }
  }
...
/// <summary>
  ...
  calculateCtrlFromControlName(taskRef: Task): void {
    if (!NString.IsNullOrEmpty(this._ctrlName)) {
      this._ctrl = ((taskRef.getForm() !== null) ? (<MgForm>taskRef.getForm()).getCtrlByName(this._ctrlName) : null);
    }
  }
  ...
  isSpecificHandlerOf(rtEvt: RunTimeEvent): boolean {
    ...
    if (this._scope === BrkScope.Task && !NObject.GenericEquals(this._task, rtEvt.getTask()))
      return false;
    ...
    if (this._evt.equals(rtEvt)) {
      ...
      if (rtEvt.Control !== null && !this._isHandlerOnForm) {
        ...
        if (eventCtrlName !== null && this._ctrlName === eventCtrlName)
          return true;
      }
  ...
  isNonSpecificHandlerOf(rtEvt: RunTimeEvent): boolean {
    ...
    if (this._ctrl !== null || this._ctrlName !== null || this._handlerFld !== null || this._isHandlerOnForm)
      return false;
    // check the validity of the scope
    if (this._scope === BrkScope.Task && !NObject.GenericEquals(this._task, rtEvt.getTask()) && rtEvt.getType() !== ConstInterface.EVENT_TYPE_TIMER)
      return false;

    if (this._scope === BrkScope.Global && !NObject.GenericEquals(this._task, rtEvt.getTask()))
      return false;
  ...
  isGlobalSpecificHandlerOf(rtEvt: RunTimeEvent): boolean {
    ...
    return (this._ctrlName != null && this._evt.equals(rtEvt) &&
            this._ctrlName === rtEvt.Control.Name);
  }
...
/// </summary>
export class RetVals extends NObject {
  ...
  constructor(pOrOperIdx: any, eOrInReturnedFromServer: boolean, inBlockIfReturnedFromServer?: boolean) {
    super();
    if (arguments.length === 2 && (pOrOperIdx === null || pOrOperIdx.constructor === Boolean) && (eOrInReturnedFromServer === null || eOrInReturnedFromServer.constructor === Boolean)) {
