...
import {Property} from "@magic/gui";
import {Task} from "./Task";
import {PropInterface} from "../../../gui/src/management/gui/PropInterface";
import {MsgInterface} from "../../../utils/src/MsgInterface";
...
export abstract class TaskServiceBase {
  ...
  static PreparePropMainDisplay(task: Task): ReturnResult
  {
    ...
    let returnResult: ReturnResult = task.FormIsLegal() ? ReturnResult.SuccessfulResult : new ReturnResult(MsgInterface.BRKTAB_STR_ERR_FORM);
    if (returnResult.Success) {
      let mgForm: MgForm = <MgForm>task.getForm();
      if (mgForm !== null && mgForm.IsFrameSet)
        returnResult = new ReturnResult(MsgInterface.CHK_ERR_OFFLINE_NOT_SUPPORT_FRAME_INTERFACE);
    }
