...
import {Logger, MgArrayList, XMLConstants, XmlParser} from "@magic/utils";
import {RecomputeTable} from "../rt/RecomputeTable";
import {List, NString, StringBuilder} from "@magic/mscorelib";
...
import {NNumber} from "../../../mscorelib/src/NNumber";
...
export class MGData {
  ...
  constructor(id: number, parent: MGData, isModal: boolean, forceModal?: boolean) {
    if (arguments.length === 3 && (id === null || id.constructor === Number) && (parent === null || parent instanceof MGData) && (isModal === null || isModal.constructor === Boolean)) {
  ...
  fillData(openingTaskDetails: OpeningTaskDetails): void {
    ...
    do {
      ...
      flag = !this.initInnerObjects(parser, nextTag, openingTaskDetails);
    }
    while (!flag);
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, openingTaskDetails: OpeningTaskDetails): boolean {
    ...
    switch (foundTagName) {
      case XMLConstants.MG_TAG_XML:
        parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex()) + 1);// do
        // nothing
        break;
      case ConstInterface.MG_TAG_DATAVIEW:
        if (!this.insertDataView(parser)) {
          ...
          let endContext: number = NString.IndexOf(parser.getXMLdata(), ('/' + ConstInterface.MG_TAG_DATAVIEW,
            parser.getCurrIndex()));
      ...
      case ConstInterface.MG_TAG_COMPLETE_DV: {
        ...
        let index: number = NString.IndexOf(parser.getXMLdata(), ConstInterface.MG_TAG_COMPLETE_DV, parser.getCurrIndex()) + ConstInterface.MG_TAG_COMPLETE_DV.length;
        let tokensVector: List<String> = XmlParser.getTokens(NString.Substring(parser.getXMLdata(), index, endContext - index), '"');
        let attribute: string = (tokensVector[0]);
        let valueStr: string;
        ...
        endContext = NString.IndexOf(parser.getXMLdata(), XMLConstants.END_TAG + ConstInterface.MG_TAG_COMPLETE_DV,
          parser.getCurrIndex());
        let dataViewContent: string = NString.Substring(parser.getXMLdata(), start, endContext - start);
      ...
      case ConstInterface.MG_TAG_CONTEXT: {
        let endContext: number = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_TERM, parser.getCurrIndex());
        if (endContext !== -1 && endContext < parser.getXMLdata().length) {
          let tag: string = parser.getXMLsubstring(endContext);
          parser.add2CurrIndex(NString.IndexOf(tag, ConstInterface.MG_TAG_CONTEXT) + ConstInterface.MG_TAG_CONTEXT.length);
      ...
      case ConstInterface.MG_TAG_KBDMAP:
        ...
        let kbdEndIdx: number = NString.IndexOf(parser.getXMLdata(), '/' + ConstInterface.MG_TAG_KBDMAP,
          parser.getCurrIndex());
        parser.setCurrIndex(kbdEndIdx);
        parser.setCurrIndex2EndOfTag();
        let kbdData: string = NString.Substring(parser.getXMLdata(), kbdStartIdx, parser.getCurrIndex() - kbdStartIdx);
      ...
      case XMLConstants.MG_TAG_COLORTABLE: {
        Logger.Instance.WriteDevToLog("goes to color");
        let colorStartIdx: number = parser.getCurrIndex();
        let colorEndIdx: number = NString.IndexOf(parser.getXMLdata(), '/' + XMLConstants.MG_TAG_COLORTABLE,
          parser.getCurrIndex());
        parser.setCurrIndex(colorEndIdx);
        parser.setCurrIndex2EndOfTag();
        let colorData: string = NString.Substring(parser.getXMLdata(), colorStartIdx,
          parser.getCurrIndex() - colorStartIdx);
        Manager.GetColorsTable().FillFrom(colorData);
        break;
      }
      ...
      case ConstInterface.MG_TAG_FONTTABLE:
        Logger.Instance.WriteDevToLog("goes to font");
        let startIdx: number = parser.getCurrIndex();
        let endIdx: number = NString.IndexOf(parser.getXMLdata(), '/' + ConstInterface.MG_TAG_FONTTABLE,
          parser.getCurrIndex());
        parser.setCurrIndex(endIdx);
        parser.setCurrIndex2EndOfTag();
        let fontData: string = NString.Substring(parser.getXMLdata(), startIdx, parser.getCurrIndex() - startIdx);
        Manager.GetFontsTable().FillFrom(fontData);
        break;
      ...
      case ConstInterface.MG_TAG_CONTEXT_ID: {
        ...
        parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex()) + 1);
        ctxEndIdx = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_OPEN, parser.getCurrIndex());
        ctxId = NString.Trim(parser.getXMLsubstring(ctxEndIdx));
      ...
      case ConstInterface.MG_TAG_HTTP_COMMUNICATION_TIMEOUT: {
        ...
        parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex()) + 1);
        httpCommunicationTimeoutIdx = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_OPEN, parser.getCurrIndex());
      ...
      case ConstInterface.MG_TAG_DBHS_END:
        ...
        break;
      default:
        Logger.Instance.WriteExceptionToLog("There is no such tag name in MGData, add case to MGData.initInnerObjects and case to while of MGData.FillData  " + foundTagName);
        return false;
    }
    return true;
  }
  ...
  private insertDataView(parser: XmlParser): boolean {
    let task: Task = null;
    let endContext: number = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, parser.getCurrIndex());
    let index: number = NString.IndexOf(parser.getXMLdata(), ConstInterface.MG_TAG_DATAVIEW, parser.getCurrIndex()) + ConstInterface.MG_TAG_DATAVIEW.length;
    let tokensVector: List<string> = XmlParser.getTokens(NString.Substring(parser.getXMLdata(), index, endContext - index), '"')/*'"'*/;
  ...
  getTask(taskIdOrIdx: any): Task {
    if (arguments.length === 1 && (taskIdOrIdx === null || taskIdOrIdx.constructor === String)) {
  ...
  changeTimers(oldTimers: List<number>, newTimers: List<number>): void {
    ...
    while (i < oldTimers.Count) {
      guiManager.stopTimer(this, oldTimers.get_Item(i), false);
      i = i + 1;
    }
    while (j < newTimers.Count) {
      guiManager.startTimer(this, newTimers.get_Item(j), false);
      j = j + 1;
    }
  }
    ...
    if (foundTagName === null) {
      ...
      if (foundTagName === ConstInterface.MG_TAG_EVENT) {
        ...
        if (foundTagName === ConstInterface.MG_TAG_EVENTS_QUEUE) {
          parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), ">", parser.getCurrIndex()) + 1);
        }
        else {
          if (foundTagName === "/" + ConstInterface.MG_TAG_EVENTS_QUEUE) {
            parser.setCurrIndex2EndOfTag();
  ...
  copyUnframedCmds(): void {
    let unframedCmds: MgArrayList<IClientCommand> = ClientManager.Instance.getUnframedCmds();
    for (let i: number = 0; i < unframedCmds.Count; i = i + 1) {
      this.CmdsToClient.Add(unframedCmds[i]);
    }
