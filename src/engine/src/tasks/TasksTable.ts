...
import {MgArrayList, XMLConstants} from "@magic/utils";
import {MGData} from "./MGData";
import {OpeningTaskDetails, Task} from "./Task";
import {ClientManager} from "../ClientManager";

export class TasksTable {
  private readonly _tasks: MgArrayList<Task> = null;

  constructor() {
    this._tasks = new MgArrayList();
  }
  ...
  getTask(tasktagOrIdx: any): Task {
    if (arguments.length === 1 && (tasktagOrIdx === null || tasktagOrIdx.constructor === String)) {
  ...
  private getTask_0(tasktag: string): Task {
    let result: Task;
    for (let i: number = 0; i < this._tasks.Count; i = i + 1) {
  ...
  private getTask_1(idx: number): Task {
    let result: Task;
    if (idx >= 0 && idx < this._tasks.Count) {
      result = this._tasks[idx];
    }
    else {
      result = null;
    }
