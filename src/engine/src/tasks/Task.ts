import {Debug, Encoding,
  Int32, List, NNumber, NObject, NString, Stack, StringBuilder,
  Timer, RefParam
} from "@magic/mscorelib";
import {StorageAttribute, UniqueTskSort, WindowType, Logger,
  XmlParser, StrUtil, XMLConstants, InternalInterface, Constants, MgArrayList
} from "@magic/utils";
import {TaskBase, Property, MgControlBase, MgFormBase, DisplayConvertor, PropInterface, PIC, FieldDef, ExpVal, Manager, NUM_TYPE} from "@magic/gui";
...
import {CommandsProcessorBase_SendingInstruction, CommandsProcessorBase} from "../CommandsProcessorBase";
...
import {TaskDefinitionId} from "../../../gui/src/management/tasks/TaskDefinitionId";
...
import {DataModificationTypes} from "../../../gui/src/GuiEnums";
import {ITask} from "../../../gui/src/management/tasks/ITask";
import {Argument} from "../rt/Argument";
import {IClientTargetedCommand} from "../commands/IClientTargetedCommand";
import {Commands} from "../../../gui/src/Commands";
...
import {Styles} from "../../../gui/src/gui/low/Style";
...
export class Task extends TaskBase {
  ...
  public MenuUid: number;
  private _inCreateLine: boolean = false;               // is the task into processing CREATE LINE
  public InSelect: boolean;                            // true when the task is executing a SELECT internal event
  public InStartProcess: boolean;
  ...
  public ContextTask: Task;                             // under context of which task this task is currently running
  /// <summary>
  ...
  public UniqueSort: UniqueTskSort;
  ...
  /// </summary>
  fillAttributes(): void {
    let endContext: number = NString.IndexOf(ClientManager.Instance.RuntimeCtx.Parser.getXMLdata(), XMLConstants.TAG_CLOSE, ClientManager.Instance.RuntimeCtx.Parser.getCurrIndex());
  ...
  RecomputeFillData(): void {
    ...
    recomputeTable.fillData(<DataView>this.DataView, this);
  }
  ...
  private FillTaskTables_1(): void {
    ...
    let endContext: number = NString.IndexOf(parser.getXMLdata(), ConstInterface.MG_TAG_TASK_TABLES_END, parser.getCurrIndex());
    endContext = NString.IndexOf(parser.getXMLdata(), XMLConstants.TAG_CLOSE, endContext) + XMLConstants.TAG_CLOSE.length;
  ...
  public SetRefreshOnVars(variables: string): void {
    ...
    this.RefreshOnVars = new Array(intVals.Count);
  ...
  getModeString(): string {
    let taskMode: string = this.getMode();
  ...
  CalculateExpression(expId: number, resType: StorageAttribute, length: number, contentTypeUnicode: boolean, resCellType: StorageAttribute): string {
    ...
    if (expId > 0) {
      ...
      result = expById.evaluate(resType, length);
    }
    return result;
  }
  ...
  buildXMLForRngs(message: StringBuilder, UserRanges: List<UserRange>, locate: boolean): void {
    ...
    for (i = 0; i < UserRanges.Count; i++) {
      ...
      let rng: UserRange = UserRanges[i];
  ...
  buildXMLForSorts(message: StringBuilder): void {
    if (this.UserSorts != null && this.UserSorts.Count > 0) {
      let i: number;
      for (i = 0; i < this.UserSorts.Count; i++) {
        message.Append(XMLConstants.TAG_CLOSE);
        let srt: Sort = this.UserSorts[i];
    ...
    if (!this.HasMDIFrame) {
      ...
      if (this.checkProp(PropInterface.PROP_TYPE_SELECTION, false)) {
        if ((this.Form.getProp(PropInterface.PROP_TYPE_DEFAULT_BUTTON).getValue() !== "")) {
          ...
          let ctrl: MgControl = (<MgForm>this.Form).getCtrlByName(defaultButtonName);
  ...
  private Setup(): ReturnResult {
    ...
    if (result.Success) {
      ...
      let commands: MgArrayList<IClientTargetedCommand> = ClientManager.Instance.getCommandsExecutedAfterTaskStarted();

      for (let cmdIdx: number = 0; cmdIdx < commands.Count; cmdIdx++) {
        let command: IClientTargetedCommand = (commands[cmdIdx]);
  ...
  doFirstRecordCycle(): void {
    if (!this._isMainPrg) {
      ...
      ClientManager.Instance.EventsManager.handleInternalEvent(this, InternalInterface.MG_ACT_REC_PREFIX, this.IsSubForm);
  ...
  private GetSubTasks(): List<Task> {
    ...
    if (this.hasSubTasks()) {
      for (let i: number = 0; i < this.SubTasks.getSize(); i = i + 1)
        subTasks.Add(this.SubTasks.getTask(i));
    }
  ...
  RefreshSubforms(): void {
    ...
    subformsToRefresh.forEach(subFormTask => {
  ...
  evalEndCond(mode: string): boolean {
    ...
    if (evalEndCondProp != null && (NString.Compare(evalEndCondProp.getValue(), ConstInterface.END_COND_EVAL_IMMIDIATE,
        true) || NString.Compare(evalEndCondProp.getValue(), mode, true))) {
  ...
  endTask(reversibleExit: boolean, onlyDescs: boolean, dueToVerifyErrorOrSubformDestination: boolean, dueToVerifyError?: boolean): boolean {
    if (arguments.length === 3 && (reversibleExit === null || reversibleExit.constructor === Boolean) && (onlyDescs === null || onlyDescs.constructor === Boolean) && (dueToVerifyErrorOrSubformDestination === null || dueToVerifyErrorOrSubformDestination.constructor === Boolean)) {
  ...
  handleEventOnSlaveTasks(internalEvtCode: number): Task {
    ...
    for (; isOwner && i < orphans.Count && !ClientManager.Instance.EventsManager.GetStopExecutionFlag(); i++) {
      task = orphans[i];
  ...
  evaluateDescriptor(): number {
    ...
    if (this.DvPosDescriptor == null)
      return NUM_TYPE.get_hash_code(Encoding.Default.GetBytes(result));

    let first: boolean = true;

    // go over the list of the descriptor
    for (let i: number = 0; i < this.DvPosDescriptor.Count; i++) {
      if (!first)
        result += ";";

      let currentCell: string[] = this.DvPosDescriptor[i];
      ...
      switch (currentField.getType()) {
        ...
        case StorageAttribute.TIME:
          ...
          if (dispVal !== "") {
            ...
            result += conv.mg2disp(dispVal, "", pic, this.getCompIdx(), false);
                        }
                        break;
  ...
  setOriginalTaskMode(originalTaskMode: string): void;
  setOriginalTaskMode(originalTaskMode: any): void {
    if (arguments.length === 1 && (originalTaskMode === null || originalTaskMode.constructor === String)) {
      this.setOriginalTaskMode_0(originalTaskMode);
      return;
    }
    this.setOriginalTaskMode_1(originalTaskMode);
  }
  ...
  private setOriginalTaskMode_0(originalTaskMode: string): void {
  ...
  /// </summary>
  /// <param name = "originalTaskMode">mode of task</param>
  private setOriginalTaskMode_1(originalTaskMode: string): void {
    this._originalTaskMode = originalTaskMode;
  }
  ...
  isAllowEvents(): boolean {
    let prop: Property = this.getProp(592);
  ...
  execSubformRecSuffix(newTask: Task): number {
    let subfTask: Task;
    let i: number, idx = 0;
    for (i = this._taskPath.Count; i > 0; i--) {
      subfTask = this._taskPath[i - 1];
  ...
  getMenusFileURL(): string {
    let menusFileURL: string = this._menusFileName;
    ...
    if (menusFileURL != null) {
      ...
      if (ClientManager.Instance.getRightsHashKey().length > 0)
        NString.Insert(menusFileURL, menusFileURL.indexOf("Menus.xml"), ClientManager.Instance.getRightsHashKey() + "_");
    }
  ...
  static isMainProgramField(fieldStr: string): boolean {
    ...
    if (Field.Count === 2) {
      let parent: number = NNumber.Parse(Field[0]);
      if (parent === TaskBase.MAIN_PRG_PARENT_ID)
        mainProgField = true;
    }
  ...
  CheckAllowTaskMode(taskModeValue: string): boolean {
    let initModeRelevantYNExp: number = 0;
    let allowedTaskMode: boolean = false;
    ...
    if (initModeRelevantYNExp !== 0) {
      let propTaskMode: Property = this.getProp(initModeRelevantYNExp);
...
export class Task_LocateQuery extends NObject {
...
export class UserRange {
  public veeIdx: number;
  public min: string;
  public max: string;
  public nullMax: boolean;
  public nullMin: boolean;
  public discardMax: boolean;
  public discardMin: boolean;
}

/// <summary>
/// Class holding additional information regarding a task being opened.
/// </summary>
export class OpeningTaskDetails extends NObject {
