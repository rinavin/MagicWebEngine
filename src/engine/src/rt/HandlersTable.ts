...
export class HandlersTable {
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, taskRef: Task): boolean {
    ...
    if (foundTagName === ConstInterface.MG_TAG_EVENTHANDLERS) {
      ...
      if (foundTagName === ConstInterface.MG_TAG_HANDLER) {
        ...
        Logger.Instance.WriteExceptionToLog("There is no such tag in HandlersTable. Insert else if to HandlersTable.initInnerObjects for " + foundTagName);
        return false;
      }
    }
  ...
  getHandler(idx: number): EventHandler {
    if (idx < 0 || idx >= this._handlers.Count)
      return null;
  ...
  startTimers(mgd: MGData): void {
    ...
    if (this._handlers !== null) {
      ...
      for (let i: number = 0; i < timersVector.Count; i++)
  ...
  getTimersVector(): List<number> {
    ...
    // scan the timers
    for (let i: number = 0; i < this._handlers.Count; i++) {
      ...
      for (let j: number = 0; j < timers.Count; j++) {
