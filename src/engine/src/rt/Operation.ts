...
import {HttpUtility} from "../../../httpClient/src/HttpUtility";
...
import {RefParam} from "../../../mscorelib/src/RefParam";
import {InternalInterface} from "../../../utils/src/InternalInterface";
...
export class Operation {
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, taskRef: Task): boolean {
    ...
    if (foundTagName === ConstInterface.MG_TAG_EVENT) {
      ...
      Logger.Instance.WriteExceptionToLog("There is no such tag in Operation. Insert else if to Operation.initInnerObjects for " + foundTagName);
      return false;
    }
    return true;
  }
      ...
      switch (attribute) {
        ...
        case ConstInterface.MG_ATTR_UNDO:
          if (valueStr[0] === 'N')
            ...
            this._undo = true;
          else Logger.Instance.WriteExceptionToLog("in Operation.initElements(): No such value to the MG_ATTR_UNDO for " + valueStr);
          break;
        ...
        case ConstInterface.MG_ATTR_IS_GUI_THREAD_EXECUTION:
          ...
          break;
        default:
          Logger.Instance.WriteExceptionToLog("There is no such tag in Operation class. Insert case to Operation.initElements for " + attribute);
          break;
      }
    }
    if (this._serverId > -1)
  ...
  static InitField(valueStr: string, task: Task): Field {
    ...
    Logger.Instance.WriteExceptionToLog(NString.Format("Unknown field: '{0}'", valueStr));
    return null;
  }
    ...
    try {
      switch (this._type) {
        ...
        case ConstInterface.MG_OPER_CALL:
          ...
          break;
        default:
          Logger.Instance.WriteExceptionToLog("There is no such type of operation " + this._type);
          break;
      }
    }
    catch (ex) {
  ...
  private operVerify(): boolean {
    ...
    let textToDisplay: string = this._exp == null
      ? this._text
      : this._exp.evaluate(StorageAttribute.UNICODE, 255);

    let titleToDisplay: string = this._titleExp == null
      ? this._title
      : this._titleExp.evaluate(StorageAttribute.UNICODE, 255);
    ...
    // Message Box
      ...
      if (!NString.IsNullOrEmpty(textToDisplay)) {
        ...
        let tmpdefaultButton: number = this._defaultButtonExp == null
          ? this._defaultButton
          : (this._defaultButtonExp.evaluate(StorageAttribute.NUMERIC)).MgNumVal.NUM_2_LONG();
  ...
  private operEvaluate(): void {
    ...
    if (this._field !== null) {
      result = this._exp.evaluate(this._field.getType(), this._field.getSize());
      this._field.setValueAndStartRecompute(result, result === null, true, true, false);
      this._field.updateDisplay();
    }
    else
      result = this._exp.evaluate(StorageAttribute.BOOLEAN, 0);
  }
  ...
  private operUpdate(mprgCreator: Task): void {
      ...
      if (!this._field.IsLinkField) {
        ...
        if (this._task.getMode() !== Constants.TASK_MODE_CREATE) {
          ...
          oldVal = (this._field.isNull()
            ...
            : this._exp.evaluate(fieldType, this._field.getSize()));
        ...
        if (this._task.getMode() !== Constants.TASK_MODE_DELETE) {
          newVal = this._exp.evaluate(fieldType, this._field.getSize());
          if (newVal != null) {
    ...
    // end of  incremental update
      ...
      result = this._exp.evaluate(fieldType, this._field.getSize());
  ...
  private operInvokeOS(): void {
    ...
    if (this._exp == null)
      ...
      command = this._exp.evaluate(StorageAttribute.ALPHA, 2048);
    ...
    if (errMsg != null) {
      ...
      Logger.Instance.WriteExceptionToLog(errMsg.value);
    }
