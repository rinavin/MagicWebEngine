...
export class RemoteDataviewHeader extends DataviewHeaderBase {
  private _table: TableCache;
  private _lastFetchRecPos: string; // the db pos of the last fetched record from the table cache
  ...
  public getLinkedRecord(curRec: Record): boolean {
    ...
    if (this._cond.getVal()) {
      ...
      if (this.Loc == null) {
        ...
        for (let i: number = 0; i < lnkFlds.Count; i++) {
          let currLoc: Boundary = lnkFlds[i].Locate;
          if (currLoc != null) {
            this.Loc.Add(currLoc);
        ...
        for (let i: number = 0; i < this.Loc.Count; i++)
          this.Loc[i].compute(false);
      }
  ...
  private copyLinkFldToRec(curRec: Record, linkFlds: List<Field>, tableRec: Record, ret: boolean): void {
    ...
    for (let i: number = 0; i < linkFlds.Count; i++) {
      let curFld: Field = linkFlds[i];
