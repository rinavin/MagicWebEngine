...
export class DataviewHeaders {
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string): boolean {
    ...
    if (foundTagName === ConstInterface.MG_TAG_LINKS) {
      let endContext: number = NString.IndexOf(parser.getXMLdata(), ConstInterface.MG_TAG_LINKS_END, parser.getCurrIndex()) + ConstInterface.MG_TAG_LINKS_END.length + 1;
  ...
  buildDbPosString(): string {
    ...
    while (dataviewHeaders.MoveNext()) {
      let curr: RemoteDataviewHeader = <RemoteDataviewHeader>dataviewHeaders.Current;
