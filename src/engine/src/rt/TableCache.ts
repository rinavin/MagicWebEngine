...
export class TableCache {
  ...
  SetTableIdent(ident: string): void {
    if (this._tableIdent === null)
      ...
      Logger.Instance.WriteExceptionToLog("in TableCache.setTableIdent() already set and table identifier  does not match");
    }
  }
        ...
        switch (attribute) {
          ...
          case ConstInterface.MG_ATTR_IDENT:
            if (this._tableIdent !== valueStr) {
              ...
              Logger.Instance.WriteExceptionToLog("in TableCache.fillAttributes() table identifier id does not match");
            }
            break;
          default:
  ...
  GetKeyById(keyId: number): Key {
    for (let i: number = 0; i < this._keys.Count; i = i + 1) {
  ...
  Fetch(loc: List<Boundary>): Record {
    ...
    let checkLoc: boolean = loc.Count > 0;
    ...
    if (this._isLoaded) {
      ...
      if (!checkLoc) {
        ...
        return ((this._records.Count !== 0) ? this._records.get_Item(0) : null);
      }
  ...
  private static validateRec(currRec: Record, rangeCond: List<Boundary>): boolean {
    ...
    for (let i: number = 0; i < rangeCond.Count; i = i + 1) {
  ...
  protected ReverseVector(array: Object[]): void {
    ...
    for (let left: number = 0; left < (array.length - 1) / 2; left++ , right--) {
