...
export class TaskTransactionManager {
  ...
  checkAndCommit(reversibleExit: boolean, level: string, isAbort: boolean): boolean {
    ...
    let ret: boolean = (() => {
      ...
      let _r = this.checkAndCommitPerDataview(this.task.DataviewManager.RemoteDataviewManager, reversibleExit, level, oper, _p0);
      result = _p0[0];
      return _r;
    })();
    ...
    if (!ret) {
      ...
      // send it now.
      if (level === 'P' && (<DataView>this.task.DataView).modifiedRecordsNumber() > 0 && (<DataView>this.task.DataView).FlushUpdates) {
        try {
  ...
  /// <returns></returns>
  checkAndCommitPerDataview(dataviewmanager: DataviewManagerBase, reversibleExit: boolean, level: string, oper: string, result: ReturnResult[]): boolean {
    result[0] = new ReturnResult();
    ...
    if (currentTransaction !== null && !this.task.isAborting() && currentTransaction.isOpened() && currentTransaction.isOwner(this.task) && currentTransaction.getLevel() === level) {
      ...
      try {
        this.task.TryingToCommit = true;
        result[0] = dataviewmanager.Execute(cmd);
      }
      finally {
