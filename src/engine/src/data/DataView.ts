import {
  Int32, Int64, Debug, ApplicationException, List, NNumber, StringBuilder
} from "@magic/mscorelib";
import {XmlParser, Misc, Logger, Constants, InternalInterface, XMLConstants} from "@magic/utils";
import {DataViewBase, DcValues, IRecord, DataModificationTypes, Property, MgFormBase, PropInterface} from "@magic/gui";
...
import {RecordOutOfDataViewException} from "./RecordOutOfDataViewException";
...
import {RecordOutOfDataViewException_ExceptionType} from "./RecordOutOfDataViewException";
...
export class DataView extends DataViewBase {
      ...
      for (let j: number = 0; j < tokensVector.Count; j += 2) {
        ...
        switch (attribute) {
          ...
          case ConstInterface.MG_ATTR_CHUNK_SIZE_EXPRESSION:
            ...
            break;
          default:
            Logger.Instance.WriteExceptionToLog("Unknown attribute for <" + XMLConstants.MG_TAG_DVHEADER + "> tag: " +
      ...
      for (let j: number = 0; j < tokensVector.Count; j += 2) {
        ...
        switch (attribute) {
          ...
          case ConstInterface.MG_ATTR_VAL_RECORDS_BEFORE_CURRENT_VIEW:
            ...
            break;
          default:
            Logger.Instance.WriteExceptionToLog("Unknown attribute for <" + ConstInterface.MG_TAG_DATAVIEW + "> tag: " + attribute);
            break;
        }
      }
      parser.setCurrIndex(++endContext);
  ...
  private peekInvalidate(tokensVector: List<string>): boolean {
    ...
    for (let j: number = 0; j < tokensVector.Count; j += 2) {
      let attribute = tokensVector[j];
      if (attribute === (ConstInterface.MG_ATTR_INVALIDATE)) {
        let valueStr = tokensVector[j + 1];
  ...
  getCurrRec(): Record {
    if (this.CurrRec === null && this._currRecId > Int32.MinValue) {
      if (!this.setCurrRec(this._currRecId, true))
        Logger.Instance.WriteExceptionToLog("DataView.getCurrRec(): record " + this._currRecId + " not found");
    }
    return this.CurrRec;
  }
...
/// <summary>
  ...
  /// </summary>
  addFirstRecord(): void {
    let mode: DataModificationTypes = (this.getTask().getMode() === Constants.TASK_MODE_CREATE) ? DataModificationTypes.Insert : DataModificationTypes.None;
    this.addRecord(false, true, true, 0, mode);
  }
  ...
  addRecord(doSuffix: boolean, ignoreCurrRec: boolean, isFirstRecord?: boolean, newCurrRecId?: number, mode?: DataModificationTypes): number {
    if (arguments.length === 2 && (doSuffix === null || doSuffix.constructor === Boolean) && (ignoreCurrRec === null || ignoreCurrRec.constructor === Boolean)) {
  ...
  MoveToEmptyDataviewIfAllowed(): void {
    if (!super.isEmptyDataview() && (<Task>this._task).getMode() !== Constants.TASK_MODE_CREATE) {
      ...
      if ((<Task>this._task).getMode() === Constants.TASK_MODE_CREATE /*'C'*/ && super.isEmptyDataview())
  ...
  currEqualsPrev(fldList: number[]): boolean {
    if (this.CurrRec !== null && this._prevCurrRec !== null) {
      if (this.CurrRec.Equals(this._prevCurrRec))
        return true;
    ...
    else if (!this.CurrRec.Equals(this._prevCurrRec))
      return false;
  ...
  ParametersExist(iPos?: number, iLen?: number): boolean {
    if (arguments.length === 2 && (iPos === null || iPos.constructor === Number) && (iLen === null || iLen.constructor === Number)) {
      ...
      switch (orgAction) {
        case RECOVERY_ACT_CANCEL:
          ...
          if (this._task.getLastParkedCtrl() == null) {
            let FirstParkableCtrl: MgControl = (<MgForm>this._task.getForm()).getFirstParkableCtrl(false);
          ...
          ClientManager.Instance.EventsManager.handleInternalEvent(this.getTask(), InternalInterface.MG_ACT_CANCEL,
        ...
        case RECOVERY_ACT_BEGIN_SCREEN:
          ...
          ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_TBL_BEGPAGE);
        ...
        case RECOVERY_ACT_BEGIN_TABLE:
          ...
          ClientManager.Instance.EventsManager.handleInternalEvent(this._task, InternalInterface.MG_ACT_TBL_BEGTBL);
          // clear the cache
          this.getTask().getTaskCache().clearCache();
          break;
  ...
  /// </summary>
  replicate(): DataView {
    let rep: DataView = <DataView>super.MemberwiseClone();
  ...
  setSameAs(newDv: DataView): void {
    ...
    try {
      ...
      if (ex instanceof RecordOutOfDataViewException) {
        Logger.Instance.WriteExceptionToLog("wrong top record idx in DataView.setSameAs" + ex.Message);
      }
      else
        throw ex;
    }
