...
export class Field extends FieldBase {
  ...
  EvaluateInitExpression(result: RefParam<string>, isNullFld: RefParam<boolean>): void {
    ...
    try {
      ...
      result.value = this._initExp.evaluate(this._type, this._size);
    }
    finally {
  ...
  setValueAndStartRecompute(val: string, isNullFld: boolean, recompute: boolean, setRecordUpdated: boolean, isArgUpdate: boolean): void;
  ...
  setValueAndStartRecompute(val: string, isNullFld: boolean, recompute: boolean, setRecordUpdated: boolean, isArgUpdate: boolean, enforceVariableChange?: boolean): void {
    if (arguments.length === 5 && (val === null || val.constructor === String) && (isNullFld === null || isNullFld.constructor === Boolean) && (recompute === null || recompute.constructor === Boolean) && (setRecordUpdated === null || setRecordUpdated.constructor === Boolean) && (isArgUpdate === null || isArgUpdate.constructor === Boolean)) {
      this.setValueAndStartRecompute_0(val, isNullFld, recompute, setRecordUpdated, isArgUpdate);
      return;
    }
    this.setValueAndStartRecompute_1(val, isNullFld, recompute, setRecordUpdated, isArgUpdate, enforceVariableChange);
  }

  private setValueAndStartRecompute_0(val: string, isNullFld: boolean, recompute: boolean, setRecordUpdated: boolean, isArgUpdate: boolean): void {
    this.setValueAndStartRecompute(val, isNullFld, recompute, setRecordUpdated, isArgUpdate, false);
  }
  ...
  private setValue(newVal: string, isNullFld: boolean, recompute: boolean, setRecordModified: boolean, setCrsrModified: boolean, isArgUpdate: boolean, enforceValiableChange: boolean): boolean {
    ...
    if (this._type !== StorageAttribute.UNICODE && UtilStrByteMode.isLocaleDefLangDBCS()) {
      ...
      if (newVal != null && this._type !== StorageAttribute.BLOB && this._type !== StorageAttribute.BLOB_VECTOR &&
        ...
        this._val = newVal + new NString(this._spaces, 0, remainder);
