...
export class FieldsTable extends FieldsTableBase {
  ...
  initField(): FieldDef;
  initField(dataview?: DataViewBase): FieldDef {
    if (arguments.length === 1 && (dataview === null || dataview instanceof DataViewBase)) {
  ...
  private initField_0(dataview: DataViewBase): FieldDef {
    let field: Field = new Field(<DataView>dataview, this._fields.Count);
    field.fillData();
    return field;
  }
...
/// <summary>
  ...
  private initField_1(): FieldDef {
    let fieldDef: FieldDef = new FieldDef(this._fields.Count);
    fieldDef.fillData();
    return fieldDef;
  }
