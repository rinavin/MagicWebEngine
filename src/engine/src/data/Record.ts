...
/// </summary>
export class Record extends NObject implements IComparable, IRecord {
  ...
  constructor(dvOrTableCache: any);
  ...
  constructor(dvOrTableCacheOrCId: any, dataview?: DataView, clobberedOnly?: boolean, isFirstRecord?: boolean) {
    super();
    if (arguments.length === 1 && (dvOrTableCacheOrCId === null || dvOrTableCacheOrCId.constructor === Object)) {
      this.constructor_0(dvOrTableCacheOrCId);
      return;
    }
    if (arguments.length === 3 && (dvOrTableCacheOrCId === null || dvOrTableCacheOrCId.constructor === Number) && (dataview === null || dataview instanceof DataView) && (clobberedOnly === null || clobberedOnly.constructor === Boolean)) {
      this.constructor_1(dvOrTableCacheOrCId, dataview, clobberedOnly);
      return;
    }
    this.constructor_2(dvOrTableCacheOrCId, dataview, clobberedOnly, isFirstRecord);
  }
  ...
  public CompareTo(obj: Object): number {
    ...
    if (key != null) {
      ...
      for (let i: number = 0; i < key.Columns.Count; i++) {
        let currFld = key.Columns[i];
...
/// <summary>
  ...
  getFieldDataXML(fldIdx: number, getOldVal?: boolean): string | Uint8Array {
    if (arguments.length === 2 && (fldIdx === null || fldIdx.constructor === Number) && (getOldVal === null || getOldVal.constructor === Boolean)) {
  ...
  protected initElements(tokensVector: List<string>): boolean {
    ...
    if (this.ValueInBase64) {
      let fldValInBytes: Uint8Array = ISO_8859_1_Encoding.getInstance().GetBytes(recFieldsData);
  ...
  fillFieldsData(fldValInBytesOrFldsVal: any, recFlags: string, isCurrRec?: boolean): void {
    if (arguments.length === 2 && (fldValInBytesOrFldsVal === null || fldValInBytesOrFldsVal instanceof Array) && (recFlags === null || recFlags.constructor === String)) {
  ...
  buildFieldsData(message: StringBuilder, isCurrRec?: boolean, getOldVal?: boolean): void {
    if (arguments.length === 3 && (message === null || message instanceof StringBuilder) && (isCurrRec === null || isCurrRec.constructor === Boolean) && (getOldVal === null || getOldVal.constructor === Boolean)) {
  ...
  protected buildFieldsData_1(message: StringBuilder): void {
    ...
    for (i = 0; i < from + size; i++) {
      ...
      tmp.forEach(x => fieldValue.Add(x));
    }

    let fieldValuesInBytes: Uint8Array = new Uint8Array(fieldValue.Count);
    for (i = 0; i < fieldValue.Count; i++)
      fieldValuesInBytes[i] = fieldValue[i];
  ...
  buildXML(message: StringBuilder, isCurrRec: boolean, forceBuild?: boolean): void {
    if (arguments.length === 2 && (message === null || message instanceof StringBuilder) && (isCurrRec === null || isCurrRec.constructor === Boolean)) {
...
/// <summary>
  ...
  /// </summary>
  replicate(): Record {
    let record: Record = <Record>super.MemberwiseClone();
  ...
  setSameAs(rec: Record, realOnly: boolean, rId?: number): void {
    if (arguments.length === 3 && (rec === null || rec instanceof Record) && (realOnly === null || realOnly.constructor === Boolean) && (rId === null || rId.constructor === Number)) {
  ...
  compare(obj1: any, obj2: any): number {
    let rec1 = <Record>obj1;
    let rec2 = <Record>obj2;
    try {
...
export class DcValuesReference extends ObjectReferenceBase {
  ...
  static Parse(couple: string, dcValuesOwner: DataView): DcValuesReference {
    let commaPos: number = NString.IndexOf(couple, ",");

    let controlId: number = NNumber.Parse(NString.Substring(couple, 0, commaPos));
    let dcId: number = NNumber.Parse(NString.Substring(couple, commaPos + 1));
