...
export class ExpTable {
  ...
  private initInnerObjects(parser: XmlParser, foundTagName: string, task: Task): boolean {
    ...
    if (foundTagName === ConstInterface.MG_TAG_EXP) {
      ...
      this._exps.Add(expression);
    }
    else if (foundTagName === ConstInterface.MG_TAG_EXPTABLE) {
      parser.setCurrIndex(NString.IndexOf(parser.getXMLdata(), ">", parser.getCurrIndex()) + 1); // end of outer tad and its ">"
    }
    else if (foundTagName === ('/' + ConstInterface.MG_TAG_EXPTABLE)) {
      ...
      Logger.Instance.WriteExceptionToLog("There is no such tag in ExpTable.initInnerObjects(): " + foundTagName);
      return false;
    }
  ...
  getExpById(id: number): Expression {
    ...
    for (let i: number = 0; i < this._exps.Count; i++) {
