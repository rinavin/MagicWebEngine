...
import {Debug, Encoding, NString, RefParam, StringBuilder, StringComparison} from "@magic/mscorelib";
...
import {HttpClientEvents, HttpUtility} from "@magic/httpClient";
...
export class RemoteCommandsProcessor extends CommandsProcessorBase {
  ...
  private constructor() {
    ...
    if (serverURL.host.length > 0) {
      ...
      if (httpReq !== null && NString.StartsWith(httpReq, "http", StringComparison.CurrentCultureIgnoreCase))
  ...
  StartSession(): boolean {
    ...
    try {
      ...
      handshakeInitialTokens.Append("&" + ConstInterface.REQ_ARGS + "=" + ConstInterface.REQ_ARG_ALPHA +
        "<Richclient><RIAProtocolVersion=\"" + RemoteCommandsProcessor.RIA_COMMUNICATION_PROTOCOL_VERSION + "\"/></Richclient>");
  ...
  private ExecuteInitialRequest(): void {
    ...
    this.Execute(CommandsProcessorBase_SendingInstruction.TASKS_AND_COMMANDS, CommandsProcessorBase_SessionStage.INITIAL, null);
  }
  ...
  Execute(sendingInstruction: CommandsProcessorBase_SendingInstruction, sessionStage?: CommandsProcessorBase_SessionStage, res?: IResultValue): void {
    if (arguments.length === 3)
      this.Execute_00(sendingInstruction, sessionStage, res);
    else
      super.Execute(sendingInstruction);
  }

  private Execute_00(sendingInstruction: CommandsProcessorBase_SendingInstruction, sessionStage: CommandsProcessorBase_SessionStage, res: IResultValue): void {
    let reqBuf: string;
  ...
  private DispatchRequest(url: string, reqBuf: string, sessionStage: CommandsProcessorBase_SessionStage): string {
    ...
    if (!NString.StartsWith(response, "<HTML", StringComparison.CurrentCultureIgnoreCase)) {
  ...
  private HandleErrorResponse(response: string): void {
    ...
    else if (NString.StartsWith(response, "<HTML", StringComparison.CurrentCultureIgnoreCase)) {
  ...
  RegisterDelegates(): void {
    HttpClientEvents.GetSessionCounter_Event = this.GetSessionCounter;
    HttpClientEvents.CheckAndSetSessionCounter_Event = this.CheckAndSetSessionCounter;
  }
}
...
class HandshakeResponse {
  ...
  constructor(responseXML: string) {
    try {
      JSON_Utils.JSONFromXML(responseXML, this.FillFromJSON);
    }
    catch (ex) {
  ...
  private FillFromJSON (error, result): void {
    ...
    for (let elementName in response) {
      if (response.hasOwnProperty(elementName)) {
        ...
        {
          case "ContextID":
            this._contextId = response[elementName];
            break;
...
class ErrorMessageXml {
  ...
  constructor(xmlContent: string, lastRequestTime: number) {
    ...
    try {
      JSON_Utils.JSONFromXML(xmlContent, this.FillFromJSON);
    }
    catch (ex) {
  ...
  private FillFromJSON (error, result): void {
    ...
    for (let elementName in xmlerr) {
      if (xmlerr.hasOwnProperty(elementName)) {
        ...
        {
          case "errmsg":
            this._errorMessage = xmlerr[elementName];
            break;
          case "errcode":
            this._errorCode = +(xmlerr[elementName]);
            break;
          case "server":
            this._middlewareAddress = xmlerr[elementName];
            break;
          ...
          case "xmlerr": // end of the error message
            break;
          default:
            Logger.Instance.WriteExceptionToLog(NString.Format("Unknown element: '{0}'", elementName));
            break;
        }
      }
    }
        ...
        case ServerError.ERR_CTX_NOT_FOUND:
          if (this.InactivityTimeoutExpired())
            sb.Append(NString.Format("{0} ({1} {2})",
              ...
              ClientManager.Instance.getEnvironment().getContextInactivityTimeout() / 600, // conversion from 1/10 of seconds to minutes
