import {NString, Exception, StringBuilder, NNumber, BitConverter, Int32, NumberStyles} from "@magic/mscorelib";
...
export class NUM_TYPE {
  ...
  private constructor_1(recordHexStr: string): void {
    ...
    try {
      for (i = 0; i < this.SIGNIFICANT_NUM_SIZE; i = i + 1) {
        twoDigits = NString.Substring(recordHexStr, i * 2, 2);
  ...
  toXMLrecord(): string {
    ...
    for (let i: number = 0; i < this.SIGNIFICANT_NUM_SIZE; i++) {
      if (i < num) {
        let tmpStr: string = this._data[i].toString(16);
        if (this._data[i] < 0)
          tmpStr = NString.Substring(tmpStr, tmpStr.length - 2);
        else if (this._data[i] < 16)
          hexStr.Append('0');
  ...
  private from_a(Alpha: string, pic: PIC, compIdx: number): void {
    ...
    if (!no_pic) {
      ...
      if (Alpha.length > pic.getMaskSize())
        Alpha = NString.Substring(Alpha, 0, pic.getMaskSize());
          ...
          if (NgtvPrmt && Signed === 0) {
            ...
            if (idx === 0) {
              if (pic.getNegPref_().length > 0)
                if (NString.StartsWith(Alpha, pic.getNegPref_())) {
                  idx += pic.getNegPref_().length;
                  continue;
                }
              if (pic.getPosPref_().length > 0)
                if (NString.StartsWith(Alpha, pic.getPosPref_())) {
              ...
              if (pic.getNegSuff_().length > 0)
                if (NString.EndsWith(Alpha, pic.getNegSuff_())) {
                  idx += pic.getNegSuff_().length;
                  continue;
                }
              if (pic.getPosSuff_().length > 0)
                if (NString.EndsWith(Alpha, pic.getPosSuff_())) {
  ...
  NUM_4_LONG(longVal: number): void {
    ...
    if (longVal !== 0) {
      ...
      if (longVal > 0)
        hexStr = NString.Substring(NUM_TYPE.INT_ZERO_HEX, hexStr.length) + hexStr;

      this._data[1] = NUM_TYPE.toSByte(parseInt(NString.Substring(hexStr, 6, (8) - (6)), 16));
      this._data[2] = NUM_TYPE.toSByte(parseInt(NString.Substring(hexStr, 4, (6) - (4)), 16));
      this._data[3] = NUM_TYPE.toSByte(parseInt(NString.Substring(hexStr, 2, (4) - (2)), 16));
      this._data[4] = NUM_TYPE.toSByte(parseInt(NString.Substring(hexStr, 0, (2) - (0)), 16));
    }
  }
  ...
  protected to_a_pic(pic: PIC): string {
    ...
    outVal = NString.ToCharArray(NString.Substring(pic.getMask(), 0, len));
    ...
    if (mask_chars === 0) {
      ...
      return NString.FromChars(out_buf.slice(str_pos, str_pos + len));
    }
  ...
  protected to_str(str: string[], len: number, pic: PIC): number {
    ...
    for (j = last; j >= first; j--) {
      ...
      if ((j < 2) || (j >= this.SIGNIFICANT_NUM_SIZE * 2))
        ...
        str[i--] = String.fromCharCode((((j & 1) !== 0) ? (num_char % 10) : (num_char / 10)) + '0'.charCodeAt(0));
      }
  ...
  static mul(num1: NUM_TYPE, num2: NUM_TYPE): NUM_TYPE {
    ...
    for (i = 0; i < significantNumSize - 1; i++)
  ...
  static div(num1: NUM_TYPE, num2: NUM_TYPE): NUM_TYPE {
    ...
    quot = 100 / (NUM_TYPE.toUByte(divisor[1]) + 1);
    if (quot > 1) {
      carry = 0;
      for (pos2 = len2; pos2 > 0; pos2--) {
    ...
    for (pos1 = 1; pos1 < SIGNIFICANT_NUM_SIZE; pos1++) {
      quot = (NUM_TYPE.toUByte(dividend[pos1 - 1]) * 100 + NUM_TYPE.toUByte(dividend[pos1])) / NUM_TYPE.toUByte(divisor[1]);
      if (quot >= 100)
        quot = 99;
  ...
  static numHexStrIsLong(numHexStr: string): boolean {
    return NString.Substring(numHexStr, 0, 2) === "FF";
  }
  ...
  NUM_LONG(): number {
    ...
    i = BitConverter.ToInt32(Misc.ToByteArray(this._data), 1);
  ...
  round(decs: number): void {
    ...
    this._data = new Int8Array[temp._data.length];
      ...
      for (let _ai: number = 0; _ai < this._data.length; ++_ai)
      this._data[_ai] = addval._data[_ai];
  ...
  num_2_ulong(): number {
    ...
    if (last)
      if (val <= (0xFFFFFFFF - NUM_TYPE.toUByte(this._data[5])) / 100)
  ...
  private static double_2_sbyteArray(d: number): Int8Array {
    let bytearray: Int8Array = Misc.ToSByteArray(BitConverter.GetBytes(d));
    return bytearray;
  }
  ...
  private static hash_rand(): number {
   ...
   static hash_str_new(strByte: Uint8Array): number {
    return 0;
  //   let array: number[] = new Array<number>(4);
  //   for (let _ai: number = 0; _ai < array.length; ++_ai)
  //     array[_ai] = 0;
  //   let array2: number[] = Misc.ToSByteArray(strByte);
  //   for (let i: number = 0; i < array2.length; i = i + 1) {
  //     let num: number = <number>(i & 255);
  //     let num2: number = (<number>array2[i] ^ num) & 255;
  //     let num3: number = <number>(<number>num2 << 1 & 255);
  //     let num4: number = <number>Misc.URShift(<number>num2, 7);
  //     let num5: number = num3 | num4;
  //     let num6: number = NUM_TYPE._randtbl[<number>((num5 ^ array[0]) & 255)];
  //     array[0] = ((array[0] ^ num6) & 255);
  //     num3 = <number>(<number>num2 << 2 & 255);
  //     num4 = <number>Misc.URShift(<number>num2, 6);
  //     num5 = (num3 | num4);
  //     num6 = NUM_TYPE._randtbl[<number>((num5 ^ array[1]) & 255)];
  //     array[1] = ((array[1] ^ num6) & 255);
  //     num3 = <number>(<number>num2 << 3 & 255);
  //     num4 = <number>Misc.URShift(<number>num2, 5);
  //     num5 = (num3 | num4);
  //     num6 = NUM_TYPE._randtbl[<number>((num5 ^ array[2]) & 255)];
  //     array[2] = ((array[2] ^ num6) & 255);
  //     num3 = <number>(<number>num2 << 4 & 255);
  //     num4 = <number>Misc.URShift(<number>num2, 4);
  //     num5 = (num3 | num4);
  //     num6 = NUM_TYPE._randtbl[<number>((num5 ^ array[3]) & 255)];
  //     array[3] = ((array[3] ^ num6) & 255);
  //   }
  //   let num7: number = <number>array[3];
  //   for (let j: number = array.length - 2; j >= 0; j = j - 1) {
  //     num7 = num7 << 8;
  //     num7 = (num7 | <number>(<number>(<number>array[j])));
  //   }
  //   return num7;
   }
  ...
  static get_hash_code(str: Uint8Array): number {
    let hashval: number;
