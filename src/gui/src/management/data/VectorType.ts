import {
  ApplicationException, Encoding, List, NNumber, NString,
  StringBuilder
} from "@magic/mscorelib";
import {FieldDef} from "./FieldDef";
import {Manager} from "../../Manager";
import {BlobType} from "./BlobType";
import {StrUtil, UtilStrByteMode, ISO_8859_1_Encoding, StorageAttribute, StorageAttributeCheck} from "@magic/utils";
...
export class VectorType {
  ...
  private _cellContentType: string;
  private _cellSize: number = 0;
  private _cellsAttr: StorageAttribute; // all cells in the vector must be of the same type
  ...
  constructor(cellsTypeOrBlobStringOrFld: any, contentType?: string, defualt?: string, isDefNull?: boolean, nullAlowed?: boolean, length?: number) {
    if (arguments.length === 6 && (cellsTypeOrBlobStringOrFld === null || cellsTypeOrBlobStringOrFld.constructor === Number) && (contentType === null || contentType.constructor === Number) && (defualt === null || defualt.constructor === String) && (isDefNull === null || isDefNull.constructor === Boolean) && (nullAlowed === null || nullAlowed.constructor === Boolean) && (length === null || length.constructor === Number)) {
      this.constructor_0(cellsTypeOrBlobStringOrFld, contentType, defualt, isDefNull, nullAlowed, length);
      return;
    }
    if (arguments.length === 1 && (cellsTypeOrBlobStringOrFld === null || cellsTypeOrBlobStringOrFld.constructor === String)) {
  ...
  private init(): void {
    ...
    // blob
    tokens = StrUtil.tokenize(NString.Substring(this._originalflatData, pos), ",");
    ...
    if (!this._initialized) {
      if (VectorType.validateBlobContents(this._originalflatData)) {
        ...
        if (this._cellsAttr === StorageAttribute.NUMERIC || this._cellsAttr === StorageAttribute.DATE || this._cellsAttr === StorageAttribute.TIME)
          this._cellsDefaultVal = StrUtil.stringToHexaDump(NString.Substring(this._originalflatData, pos, <number>this._cellSize), 2);
          ...
          if (UtilStrByteMode.isLocaleDefLangDBCS()) {
            ...
            this._cellsDefaultVal = UtilStrByteMode.leftB(NString.Substring(this._originalflatData, pos), <number>this._cellSize);
          }
          else
            this._cellsDefaultVal = NString.Substring(this._originalflatData, pos, <number>this._cellSize);
          if (this._encoding !== null && !isFlatDataTranslated) {
            try {
              let ba: Uint8Array = ISO_8859_1_Encoding.getInstance().GetBytes(this._cellsDefaultVal);
        ...
        let data: string;
        if (isDbcsAlpha) {
          data = UtilStrByteMode.leftB(NString.Substring(this._originalflatData, pos), <number>(vecSize * this._cellSize));
          pos = pos + data.length + 1;
        }
        else {
          data = NString.Substring(this._originalflatData, pos, <number>(vecSize * this._cellSize));
        ...
        // parse the null buf
        let nullBuf: string = NString.Substring(this._originalflatData, pos, <number>vecSize);
        ...
        if (this._cellsAttr !== StorageAttribute.BLOB && this._cellsAttr !== StorageAttribute.BLOB_VECTOR) {
          for (let i: number = 0; i < vecSize; i++) {
            ...
            if (this._cellsAttr === StorageAttribute.NUMERIC || this._cellsAttr === StorageAttribute.DATE || this._cellsAttr === StorageAttribute.TIME)
              this._cells.Add(new CellElement(StrUtil.stringToHexaDump(NString.Substring(data, <number>(<number>i * this._cellSize), <number>this._cellSize), 2), isNull));
            else if (this._cellsAttr === StorageAttribute.ALPHA || this._cellsAttr === StorageAttribute.MEMO) {
              ...
              if (isDbcsAlpha)
                ...
                cellData = NString.Substring(data, <number>(<number>i * this._cellSize), <number>this._cellSize);

              if (this._encoding !== null && !isFlatDataTranslated) {
                try {
                  let ba: Uint8Array = ISO_8859_1_Encoding.getInstance().GetBytes(cellData);
              ...
              this._cells.Add(new CellElement(cellData, isNull));
            }
            else if (this._cellsAttr === StorageAttribute.UNICODE) {
              let cellData: string = NString.Substring(data, <number>(<number>i * this._cellSize), <number>this._cellSize);
              try {
                let ba: Uint8Array = ISO_8859_1_Encoding.getInstance().GetBytes(cellData);
              ...
              this._cells.Add(new CellElement(NString.Substring(data, <number>(<number>i * this._cellSize), <number>this._cellSize), isNull));
          }
        }
        else {
          ...
          for (let i: number = 0; i < vecSize; i++) {
            ...
            let cellData: string = NString.Substring(this._originalflatData, pos, <number>size);

            // add the cell to the vec
            let isNull: boolean = nullBuf.charAt(i) > '\0';
            this._cells.Add(new CellElement(cellData, isNull, blobHeader));
  ...
  ToString(): string {
    ...
    if (this._initialized) {
      // build the headers
      res = NString.Concat([
        res, 0, ",", 0, ",\0,", StorageAttribute[<string>this._cellsAttr], ",", this._cellContentType, ";"
      ]);
  ...
  getVecSize(): number {
    if (!this._initialized)
      ...
      return <number>this._cells.Count;
  }
  ...
  setVecCell(idx: number, newValue: string, isNull: boolean): boolean {
    ...
    try {
      if (idx > 0) {
        this.init();
        let localCellSize: number = (this._cellsAttr === StorageAttribute.UNICODE) ? (this._cellSize / 2) : this._cellSize;
        ...
        if (idx <= this._cells.Count) {
          ...
          if (!isNull) {
            ...
            if (this._cellsAttr === StorageAttribute.BLOB || this._cellsAttr === StorageAttribute.BLOB_VECTOR) {
              ...
              curr.blobFieldPrefix = NString.Substring(newValue, 0, blobPrefixLength - 1);

              // treat empty blob
              if (newValue.length > blobPrefixLength)
                curr.data = NString.Substring(newValue, blobPrefixLength);
              else
                curr.data = "";
            }
            // simple type
              ...
              if (UtilStrByteMode.isLocaleDefLangDBCS() && (this._cellsAttr === StorageAttribute.ALPHA || this._cellsAttr === StorageAttribute.MEMO)) {
                ...
                if (this._cellsAttr === StorageAttribute.ALPHA || this._cellsAttr === StorageAttribute.UNICODE || this._cellsAttr === StorageAttribute.MEMO)
                  curr.data = NString.Substring(this._dataBuf.ToString(), <number>(<number>(idx - 1) * localCellSize), <number>localCellSize);
              }
            }
            // update the null buf
        ...
        // new record
          ...
          while (this._cells.Count < idx) {
            if (this._cellsAttr === StorageAttribute.BLOB || this._cellsAttr === StorageAttribute.BLOB_VECTOR) {
              this._cells.Add(new CellElement(NString.Substring(insertVal, VectorType.EMPTY_BLOB_PREFIX_LEN), this._cellsIsNullDefault, NString.Substring(insertVal, 0, VectorType.EMPTY_BLOB_PREFIX_LEN)));
              this._dataBuf.Append(this.getNullString(VectorType.BLOB_TYPE_STRUCT_SIZE));
            }
            else {
              this._cells.Add(new CellElement(insertVal, this._cellsIsNullDefault));
            ...
            // update the null buf
            this._nullBuf.Insert(this._cells.Count - 1, [
    ...
    if (field.getType() === StorageAttribute.BLOB_VECTOR) {
      ...
      if (StorageAttributeCheck.isTheSameType(srcAttr, dstAttr)) {
        ...
        if (StorageAttributeCheck.IsTypeAlphaOrUnicode(srcAttr) && StorageAttributeCheck.IsTypeAlphaOrUnicode(dstAttr)) {
          ...
          let srcSizeInChars: number = <number>((this._cellsAttr === StorageAttribute.UNICODE) ? (this._cellSize / 2) : this._cellSize);
              ...
              if (!curr.isNull) { // unicode cell size are saved internally as byte length while the field returns char
                ...
                if (srcSizeInChars > dstSizeInChars) {
                  if (isByteMode)
                    ...
                    curr.data = NString.Substring(curr.data, 0, dstSizeInChars);
  ...
  private buildHeadersString(): string {
    let res: string = NString.Concat([
      VectorType.BLOB_TABLE_STR, VectorType.VECTOR_STR, ",", VectorType.VERSION, ",", 1, ",", StorageAttribute[<string>this._cellsAttr], ","
    ]);
    ...
    res = NString.Concat([
      res, def, ",", this._cellsIsNullDefault ? "1" : "0", ","
    ]);
    res = NString.Concat([
      res, this._allowNull ? "1" : "0", ",", this._cellSize, ",", this._cells.Count, ","
    ]);
  ...
  private getBlobsBuf(): string {
    let res: StringBuilder = new StringBuilder();
    for (let i: number = 0; i < this._cells.Count; i = i + 1) {
      let data: string = "";
      ...
      res.Append(blobSize);
      res.Append(NString.Concat([
        ",", curr.blobFieldPrefix, ",", data, ";"
      ]));
    }
    return (res + ",");
  }
  ...
  static getCellsAttr(blob: string): StorageAttribute {
    if (VectorType.validateBlobContents(blob)) {
      let tokens: string[] = StrUtil.tokenize(NString.Substring(blob, BlobType.blobPrefixLength(blob)), ",");
  ...
  static getVecSize(blob: string): number {
    ...
    if (VectorType.validateBlobContents(blob)) {
      ...
      let tokens: string[] = StrUtil.tokenize(NString.Substring(blob, pos), ",");
      ...
      tokens = StrUtil.tokenize(NString.Substring(blob, pos), ",");
  ...
  static adjustAlphaStringsInFlatData(srcBlob: string): string {
    ...
    if (VectorType.validateBlobContents(srcBlob)) {
      ...
      let strToken: string = NString.Substring(srcBlob, 0, pos);
      destBuf.Append(strToken);

      let tokens: string[] = StrUtil.tokenize(NString.Substring(srcBlob, pos), ",");
      let tokenIdx: number = 0;
      ...
      strToken = UtilStrByteMode.leftB(NString.Substring(srcBlob, pos), len);
      destBuf.Append(strToken + ",");
      pos = pos + (strToken.length + 1);

      tokens = StrUtil.tokenize(NString.Substring(srcBlob, pos), ",");
      tokenIdx = 0;
      ...
      // copy the rest
      strToken = NString.Substring(srcBlob, pos);
      destBuf.Append(strToken);
  ...
  isUnicode(flatData: string): boolean {
    ...
    let catcher: string = NString.Substring(flatData, start, VectorType.BLOB_TABLE_STR_LEN);
    return NString.CompareOrdinal(catcher, VectorType.BLOB_TABLE_STR) === 0;
  }
}
...
export class CellElement {
  ...
  constructor(val: string, is_null: boolean, ctrlData?: string) {
    if (arguments.length === 2 && (val === null || val.constructor === String) && (is_null === null || is_null.constructor === Boolean)) {
