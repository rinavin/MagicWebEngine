...
import {Base64, FldStorage, ISO_8859_1_Encoding, Misc, StorageAttribute, StrUtil, UtilStrByteMode} from "@magic/utils";
...
export class RecordUtils {
  ...
  private static byteStreamToString_0(stream: StringBuilder): string {
    let currStr: string;
    let currChar: string;
    let result: StringBuilder = new StringBuilder(stream.Length / 2);
  ...
  static serializeItemVal(itemVal: string, itemAttr: StorageAttribute, cellAttr?: StorageAttribute, toBase64?: boolean): any {
    if (arguments.length === 2 && (itemVal === null || itemVal.constructor === String) && (itemAttr === null || itemAttr.constructor === Number)) {
  ...
  private static serializeItemVal_0(itemVal: string, itemAttr: StorageAttribute): Uint8Array {
    ...
    switch (itemAttr) {
      ...
      case StorageAttribute.BLOB:
        ...
        noOfPackets = <number>fldValLen / 0xFFFF;
        ...
        for (let i: number = 0; i < noOfPackets; i++) {
          ...
          let tempItemValBytes: Uint8Array = ISO_8859_1_Encoding.getInstance().GetBytes(tempItemVal);
        ...
        if (lastPacketSize > 0) {
          ...
          let tempContentWithLength: Uint8Array = new Uint8Array (contentWithLength.length +
                                                                ...
                                                                ISO_8859_1_Encoding.getInstance().GetBytes(tempItemVal).length);
          ...
          tempContentWithLength.set(ISO_8859_1_Encoding.getInstance().GetBytes(tempItemVal), contentWithLength.length + Manager.Environment.GetEncoding().GetBytes(tmpBufLen).length);
  ...
  static deSerializeItemVal(itemVal: Uint8Array, itemAttr: StorageAttribute, itemLen: number, fieldStorage: FldStorage, offset: number[]): string;
  static deSerializeItemVal(itemVal: any, itemAttr: StorageAttribute, itemLen: number, useHexOrFieldStorage: any, cellAttrOrOffset: any, parsedLen?: RefParam<number>): string {
    if (arguments.length === 6 && (itemVal === null || itemVal.constructor === String) && (itemAttr === null || itemAttr.constructor === Number) && (itemLen === null || itemLen.constructor === Number) && (useHexOrFieldStorage === null || useHexOrFieldStorage.constructor === Boolean) && (cellAttrOrOffset === null || cellAttrOrOffset.constructor === Number) && (parsedLen === null || parsedLen instanceof RefParam)) {
      return RecordUtils.deSerializeItemVal_0(itemVal, itemAttr, itemLen, useHexOrFieldStorage, cellAttrOrOffset, parsedLen);
    }
    return RecordUtils.deSerializeItemVal_1(itemVal, itemAttr, itemLen, useHexOrFieldStorage, cellAttrOrOffset);
  }
  ...
  /// <returns></returns>
  private static deSerializeItemVal_0(itemVal: string, itemAttr: StorageAttribute, itemLen: number, useHex: boolean, cellAttr: StorageAttribute, parsedLen: RefParam<number>): string {
    let val: string = null;
    let idx: number = 0;
  ...
  private static deSerializeItemVal_1(itemVal: Uint8Array, itemAttr: StorageAttribute, itemLen: number, fieldStorage: FldStorage, offset: RefParam<number>): any {
    let val: any = null;
    let count: number = 0;
    let endIdx: number = 0;
    let len: number = 0;
    let tmp: string = NString.Empty;
    let suffixBuf: StringBuilder;
    let tempVal: string = NString.Empty;
    let noOfPackets: number = 0;

    switch (itemAttr) {
      case StorageAttribute.ALPHA:
        count = 4;
        endIdx = offset.value + count;
        tmp = Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count);
        len = NNumber.Parse(tmp, NumberStyles.HexNumber);

        count = len;
        offset.value = endIdx;
        val = Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count);
        break;
      ...
      case StorageAttribute.UNICODE:
        ...
        count = len * 2;
        offset.value = endIdx;
        val = Encoding.Unicode.GetString(itemVal, offset.value, count);
        break;
      ...
      case StorageAttribute.BOOLEAN:
        ...
        val = BitConverter.ToInt16(Manager.Environment.GetEncoding().GetBytes(Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count)), 0);
        break;
      ...
      case StorageAttribute.BLOB:
        ...
        endIdx = offset.value + count;
        tmp = Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count);
        noOfPackets = BitConverter.ToInt16(Manager.Environment.GetEncoding().GetBytes(Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count)), 0);
        ...
        if ((len & 0x8000) > 0) {
          suffixBuf = new StringBuilder();
          count = this.getSpannedField(itemVal, len, offset.value, itemAttr, suffixBuf, false, noOfPackets);
          tempVal = suffixBuf.ToString();
        }
        else {
          if (UtilStrByteMode.isLocaleDefLangDBCS())
            tempVal = ISO_8859_1_Encoding.getInstance().GetString(itemVal, offset.value, count);
          else
            tempVal = Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count);
        }
      ...
      case StorageAttribute.NUMERIC: {
        ...
        switch (fieldStorage) {
          ...
          case FldStorage.NumericSigned:
            let array: Uint8Array = new Uint8Array(count);
            array.set(itemVal.slice(offset.value, offset.value + array.length));
      ...
      case StorageAttribute.DATE:
        ...
        switch (fieldStorage) {
          ...
          case FldStorage.DateInteger:
            val = BitConverter.ToInt32(Manager.Environment.GetEncoding().GetBytes(Manager.Environment.GetEncoding().GetString(itemVal, offset.value, count)), 0);
            break;
  ...
  static getString(str: string, type: StorageAttribute, useHex: boolean, useEnvCharset?: boolean): string {
    if (arguments.length === 3 && (str === null || str.constructor === String) && (type === null || type.constructor === Number) && (useHex === null || useHex.constructor === Boolean)) {
  ...
  static getSpannedField(fldsVal: any, firstSegLen: number, idx: number, type: StorageAttribute, result: StringBuilder, useHex: boolean, noOfPackets?: number): number {
    if (arguments.length === 6 && (fldsVal === null || fldsVal.constructor === String) && (firstSegLen === null || firstSegLen.constructor === Number) && (idx === null || idx.constructor === Number) && (type === null || type.constructor === Number) && (result === null || result instanceof StringBuilder) && (useHex === null || useHex.constructor === Boolean)) {
  ...
  private static getSpannedField_0(fldsVal: string, firstSegLen: number, idx: number, type: StorageAttribute, result: StringBuilder, useHex: boolean): number {
    ...
    result.Append(RecordUtils.getString(NString.Substring(fldsVal, idx, endIdx - idx), type, useHex));

    idx = idx + firstSegLen;

    // next 4 characters are the length of the string (hex number) of special bytes.
    endIdx = idx + 4;
    tmp = NString.Substring(fldsVal, idx, endIdx - idx);
    len = NNumber.Parse(tmp, NumberStyles.HexNumber);
    idx = endIdx;
    ...
    while (idx < endIdx) {
      tmp = NString.Substring(fldsVal, idx, 2);
