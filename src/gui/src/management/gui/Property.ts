import {
  Int32, NString, NNumber, ApplicationException, Debug, List, NumberFormatInfo, StringBuilder,
  StringComparison, Single, RefParam
...
export class Property {
  ...
  private _parentType: string; // Control|Form|TaskBase
  private _prevValues: MgArrayList<string> = null; // reverence to property values in a table
  ...
  constructor(cId?: number, cParentObj?: PropParentInterface, parType?: string, val?: string) {
    ...
    if (arguments.length === 3 && (cId === null || cId.constructor === Number) && (parType === null || parType.constructor === Number)) {
  ...
  fillData(parentRef: PropParentInterface, parType: string): void {
    ...
    let endContext: number = NString.IndexOf(parser.getXMLdata(), ">", parser.getCurrIndex(), StringComparison.Ordinal);
  ...
  setValue(mgVal: string): void {
    ...
    switch (this._dataType) {
      case StorageAttribute.NUMERIC:
        ...
        this._val = DisplayConvertor.Instance.mg2disp(mgVal, "", this._pic, compIdx, false).trim();
        break;
  ...
  RefreshDisplay(forceRefresh: boolean, currLine: number): void;
  RefreshDisplay(forceRefresh: boolean, currLine: number, checkSkipRefresh: boolean): void;
  RefreshDisplay(forceRefresh: boolean, currLine ?: number, checkSkipRefresh ?: boolean): void {
    if (arguments.length === 1) {
      this.RefreshDisplay_0(forceRefresh);
      return;
    }
    if (arguments.length === 2) {
      this.RefreshDisplay_1(forceRefresh, currLine);
      return;
    }
    this.RefreshDisplay_2(forceRefresh, currLine, checkSkipRefresh);
  }
  ...
  private RefreshDisplay_0(forceRefresh: boolean): void {
    this.RefreshDisplay(forceRefresh, Int32.MinValue);
  }
  ...
  private ShouldSkipRefreshForm(): boolean {
    ...
    switch (this._id) {
      ...
      case PropInterface.PROP_TYPE_WIDTH:
      case PropInterface.PROP_TYPE_HEIGHT:
        skip = this.ShouldSkipNavigationProperties();
        break;
    ...
    if (!skip && mgFormBase.isSubForm()) {
      ...
      switch (this._id) {
        ...
        case PropInterface.PROP_TYPE_HEIGHT:
          ...
          let autoFit = <AutoFit>mgControl.getProp(PropInterface.PROP_TYPE_AUTO_FIT).getValueInt();
          switch (autoFit) {
            case AutoFit.None:
  ...
  ///   (Integer.MIN_VALUE)
  /// </param>
  private RefreshDisplay_1(forceRefresh: boolean, currLine: number): void {
    this.RefreshDisplay(forceRefresh, currLine, true);
  }
  ...
  ///   (Integer.MIN_VALUE)
  /// </param>
  private RefreshDisplay_2(forceRefresh: boolean, currLine: number, checkSkipRefresh: boolean): void {
    ...
    switch (this._parentType) {
      case GuiConstants.PARENT_TYPE_CONTROL:
        ...
        prevValue = this._prevValues[line];
        ...
        this._prevValues[line] = this._val;
        break;

      case GuiConstants.PARENT_TYPE_FORM:
        // check if the value of the property was changed to prevent sending redundant
        // javascript commands to the browser
        prevValue = this._prevValues[0];
        if (!forceRefresh && (this._val === prevValue))
          return;
        this._prevValues[0] = this._val;
        break;
    ...
    switch (this._id) {
      ...
      case PropInterface.PROP_TYPE_EXPANDED_IMAGEIDX:
        this.onExpandedImageIdx();
        break;

      case PropInterface.PROP_TYPE_COLLAPSED_IMAGEIDX:
        this.onCollapsedImageIdx();
        break;

      case PropInterface.PROP_TYPE_PARKED_COLLAPSED_IMAGEIDX:
        this.onParkedCollapsedImageIdx();
        break;

      case PropInterface.PROP_TYPE_PARKED_IMAGEIDX:
        this.onParkedImageIdx();
        break;
  ...
  static UpdateValByStudioValue(propParentInterface: PropParentInterface, propId: number): void {
    let property: Property = propParentInterface.getProp(propId);
  ...
  private updateNavigatorResult(result: string): string {
    ...
    let dispVal: string = DisplayConvertor.Instance.mg2disp(result, "", tempPic, this._parentObj.getCompIdx(), false);
    result = dispVal.trim();
    ...
    let numberFormatInfo: NumberFormatInfo = NumberFormatInfo.CurrentInfo.Clone();
    numberFormatInfo.NumberDecimalSeparator = Manager.Environment.GetDecimal();
    let val: number = Single.Parse(result, numberFormatInfo);
  ...
  getPrevValue(line: number): string {
    if (line < this._prevValues.Count)
  ...
  private onBorderColor(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      let borderColor: MgColor = null;

      if (colorIndex > 0)
        borderColor = Manager.GetColorsTable().getFGColor((colorIndex));

      Commands.addAsync(CommandType.PROP_SET_BORDER_COLOR, this.getObjectByParentObj(), this.getLine(), borderColor);
    }
    else
  ...
  private onGradientStyle(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL || this._parentType === GuiConstants.PARENT_TYPE_FORM)
      Commands.addAsync(CommandType.PROP_SET_GRADIENT_STYLE, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  private onColorBy(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_COLOR_BY, this.getObjectByParentObj(), 0, this.getValueInt());
    else
  ...
  /// </summary>
  private onBGColor(): void {
    let colorIndex: number = this.getValueInt();
    if (colorIndex > 0) {
      let mgColor: MgColor = Manager.GetColorsTable().getBGColor(colorIndex);
      Commands.addAsync(CommandType.PROP_SET_BACKGOUND_COLOR, this.getObjectByParentObj(), this.getLine(), mgColor, colorIndex);
    }
  }
  ...
  ///
  /// </summary>
  private onFGColor(): void {
    let colorIndex: number = this.getValueInt();
    if (colorIndex > 0) {
      let mgColor: MgColor = Manager.GetColorsTable().getFGColor(colorIndex);
      Commands.addAsync(CommandType.PROP_SET_FOREGROUND_COLOR, this.getObjectByParentObj(), this.getLine(), mgColor);
    }
  }
  ...
  private onFocuseColor(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (mgControlBase.isTextControl()) {
        let colorIndex: number = this.getValueInt();
        let mgFGColor: MgColor = null;
        let mgBGColor: MgColor = null;
        ...
        Commands.addAsync(CommandType.PROP_SET_FOCUS_COLOR, this.getObjectByParentObj(), this.getLine(), mgFGColor, mgBGColor, colorIndex);
      }
    }
  }
  ...
  private onWallpaper(): void {
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_FORM || this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      let wallpaperTrans: string = Events.TranslateLogicalName(wallpaper);

      Commands.addAsync(CommandType.PROP_SET_WALLPAPER, this.getObjectByParentObj(), 0, wallpaperTrans,
  ...
  private onImageFileName(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      let mgControlBase: MgControlBase = <MgControlBase>this._parentObj;
      if (mgControlBase.isTabControl() || (mgControlBase.getField() === null && mgControlBase.IsImageButton())) {
        ...
        mgControlBase.setImage(this.getValue());
      }
    }
    else
  ...
  private onHint(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (!control.IsDateTimePicture() && (!control.IsTableChild || control.IsTableHeaderChild)) {
        Commands.addAsync(CommandType.PROP_SET_EDIT_HINT, this.getObjectByParentObj(), this.getLine(), translatedString);
      }
    }
    else
  ...
  private onHintColor(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (!control.IsDateTimePicture() && !control.IsTableChild) {
        ...
        Commands.addAsync(CommandType.PROP_SET_EDIT_HINT_COLOR, this.getObjectByParentObj(), this.getLine(), mgColor);
      }
    }
    else
  ...
  private addCommandTypeText(line: number): void {
    ...
    Commands.addAsync(CommandType.PROP_SET_TEXT, this._parentObj, line, mlsTransValue, 0);
  }
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (ctrl.isTextControl()) {
        ...
        if ((readOnlyValue || task.getMode() !== Constants.TASK_MODE_QUERY) && ctrl.GetCurrReadOnly() !== readOnlyValue) {
          ...
          Commands.addAsync(CommandType.PROP_SET_READ_ONLY, this.getObjectByParentObj(), this.getLine(), readOnlyValue);
        }
      }
    }
    else
  ...
  private onTooltip(): void {
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      Commands.addAsync(CommandType.PROP_SET_TOOLTIP, mgControlBase, this.getLine(), toolTip, 0);
    }
    else
  ...
  private onSelectionRows(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_VISIBLE_LINES, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  private onChoiceColumn(line: number): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_LAYOUT_NUM_COLUMN, this.getObjectByParentObj(), line, this.getValueInt());
    else
  ...
  /// </summary>
  private ShouldSkipNavigationProperties(): boolean {
    let skip: boolean = false;
    let form: MgFormBase = <MgFormBase>this._parentObj;

    Debug.Assert((this._id === PropInterface.PROP_TYPE_LEFT) || (this._id === PropInterface.PROP_TYPE_TOP) ||
      (this._id === PropInterface.PROP_TYPE_WIDTH) || (this._id === PropInterface.PROP_TYPE_HEIGHT));

    switch (form.GetStartupPosition()) {
      ...
      case WindowPosition.DefaultLocation:
        skip = ((this._id === PropInterface.PROP_TYPE_LEFT) || (this._id === PropInterface.PROP_TYPE_TOP));
        break;
    }
  ...
  private onAllowColumnResiz(): void {
    ...
    if (mgControlBase.isTableControl())
      Commands.addAsync(CommandType.PROP_SET_RESIZABLE, mgControlBase, 0, this.getValueBoolean());
    else
  ...
  private onRowHeight(): void {
    ...
    if (mgControlBase.isTableControl())
      Commands.addAsync(CommandType.PROP_SET_ROW_HEIGHT, mgControlBase, 0, this.getValueInt());
    else
  ...
  private onTitleHeight(): void {
    ...
    if (mgControlBase.isTableControl())
      Commands.addAsync(CommandType.PROP_SET_TITLE_HEIGHT, mgControlBase, 0, this.getValueInt());
    else
      throw new ApplicationException("in Property.onRowHeight() the control isn't Table control");
  }

  /// <summary> set the bottom position interval of the table </summary>
  private onBottomPositionInterval(): void {
    let mgControlBase: MgControlBase = <MgControlBase>this._parentObj;

    if (mgControlBase.isTableControl())
      Commands.addAsync(CommandType.PROP_SET_BOTTOM_POSITION_INTERVAL, mgControlBase, 0, this.getValueInt());
    else
  ...
  private onAllowReOrder(): void {
    ...
    if (mgControlBase.isTableControl()) {
      Commands.addAsync(CommandType.PROP_SET_ALLOW_REORDER, mgControlBase, 0, this.getValueBoolean());
    }
    else
  ...
  ResetPrevValueArray(): void {
    let count: number = this._prevValues.Count;
    this._prevValues.Clear();
  ...
  RemoveAt(idx: number): void {
    if (!this.ShouldSkipRefresh() && this._prevValues.Count > 0) {
      Debug.Assert(idx < this._prevValues.Count);
  ...
  ///   auto wide property
  /// </summary>
  private onAutoWide(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_AUTO_WIDE, this.getObjectByParentObj(), this.getLine(), this.getValueBoolean());
    else
  ...
  /// </summary>
  private onExpandedImageIdx(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_EXPANDED_IMAGEIDX, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  /// </summary>
  private onCollapsedImageIdx(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_COLLAPSED_IMAGEIDX, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  /// </summary>
  private onParkedImageIdx(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_PARKED_IMAGEIDX, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  /// </summary>
  private onParkedCollapsedImageIdx(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_PARKED_COLLAPSED_IMAGEIDX, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  private onVerticalAlignment(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      let mgControlBase: MgControlBase = <MgControlBase>this._parentObj;
      let verAligment: number = this.getValueInt();
  ...
  private onRadioButtonAppearance(): void {
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (ctrlType === MgControlType.CTRL_TYPE_RADIO) {
        failed = false;
        Commands.addAsync(CommandType.PROP_SET_RADIO_BUTTON_APPEARANCE, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
      }
    }
  ...
  private onPassword(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_PASSWORD_EDIT, this.getObjectByParentObj(), this.getLine(), this.getValueBoolean());
    else
  ...
  private onMultilineWordWrapScroll(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_MULTILINE_WORDWRAP_SCROLL, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  private onBorderStyle(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (ctrlType === MgControlType.CTRL_TYPE_CHECKBOX || ctrlType === MgControlType.CTRL_TYPE_RADIO)
        Commands.addAsync(CommandType.PROP_SET_BORDER_STYLE, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    }
  }
  ...
  private onCheckBoxMainStyle(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_CHECKBOX_MAIN_STYLE, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
      throw new ApplicationException("in Property.onMultilineVerticalScroll()");
  }
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      switch (ctrlType) {
        ...
        case MgControlType.CTRL_TYPE_SB_LABEL:
          Commands.addAsync(CommandType.PROP_SET_BORDER, this.getObjectByParentObj(), ctrl.getDisplayLine(false), this.getValueBoolean());
          break;
  ...
  private onMinBox(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_FORM)
      Commands.addAsync(CommandType.PROP_SET_MINBOX, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onMaxBox(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_FORM)
      Commands.addAsync(CommandType.PROP_SET_MAXBOX, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onSystemMenu(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_FORM)
      Commands.addAsync(CommandType.PROP_SET_SYSTEM_MENU, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  ///   onStartupPosition
  /// </summary>
  private onStartupPosition(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_FORM) {
      // update the StartupPosition on the form
      let startupPosition: WindowPosition = (<MgFormBase>this._parentObj).GetStartupPosition();
      Commands.addAsync(CommandType.PROP_SET_STARTUP_POSITION, this.getObjectByParentObj(), 0, startupPosition);
    }
    else
      throw new ApplicationException("in Property.onSystemMenu()");
  }
  ...
  private onShowFullRow(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SHOW_FULL_ROW, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onLineDivider(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_LINE_DIVIDER, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onRowHighLightType(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_ROW_HIGHLITING_STYLE, this.getObjectByParentObj(), 0, this.getValueInt());
    else
  ...
  private onStaticType(): void {
    ...
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL) {
      ...
      if (ctrl.isLineControl()) {
        let val: number = this.getValueInt();
  ...
  private onSelectionMode(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_SELECTION_MODE, this.getObjectByParentObj(), this.getLine(), this.getValueInt());
    else
  ...
  private onShowButtons(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SHOW_BUTTONS, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onLinesAsRoot(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_LINES_AT_ROOT, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onTabControlSide(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_TAB_CONTROL_SIDE, this.getObjectByParentObj(), 0, this.getValueInt());
    else
  ...
  private onTabControlTabsWidth(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_TAB_SIZE_MODE, this.getObjectByParentObj(), 0, this.getValueInt());
    else
  ...
  private onTopBorderMargin(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL && (<MgControlBase>this._parentObj).isGroup())
      Commands.addAsync(CommandType.PROP_SET_TOP_BORDER_MARGIN, this.getObjectByParentObj(), 0, this.getValueBoolean());
  }
  ...
  private onFillWidth(): void {
    ...
    Commands.addAsync(CommandType.PROP_SET_FILL_WIDTH, this.getObjectByParentObj(), this.getValueBoolean());
  }
  ...
  private onMultiColumnDisplay(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_MULTI_COLUMN_DISPLAY, this.getObjectByParentObj(), this.getValueBoolean());
    else
  ...
  private onShowEllipsis(): void {
    if (this._parentType === GuiConstants.PARENT_TYPE_CONTROL)
      Commands.addAsync(CommandType.PROP_SET_SHOW_ELLIPSIS, this.getObjectByParentObj(), this.getValueBoolean());
    else
      throw new ApplicationException("in Property.onMultiColumnDisplay()");
  }
  ...
  private onTitlePadding(): void {
    ...
    Commands.addAsync(CommandType.PROP_SET_TITLE_PADDING, this.getObjectByParentObj(), 0, this.getValueInt());
  }
