...
export class FieldValidator {
  ...
  checkVal(valDet: ValidationDetails): ValidationDetails {
    ...
    let modInQueuy: boolean = ctrl.checkProp(PropInterface.PROP_TYPE_MODIFY_IN_QUERY, false, ctrl.getDisplayLine(false));
    ...
    if (ctrl.DataType !== StorageAttribute.ALPHA && ctrl.DataType !== StorageAttribute.UNICODE && ctrl.DataType !== StorageAttribute.BLOB) {
          if (NString.Trim(this._newvalue).length === 0 || (ctrl.DataType === StorageAttribute.DATE && (ctrl.isAllBlanks(this._newvalue) || this.isNullDisplayVal()))) {
            ...
            if (field.isNullDefault()) {
              ...
              if (NString.IsNullOrEmpty(text) && ctrl.DataType === StorageAttribute.DATE) {
                text = DisplayConvertor.Instance.mg2disp(defaultValue, valDet.getRange(), valDet.getPIC(), field.getTask().getCompIdx(), false);
              }
            }
            if (text === null) {
              text = DisplayConvertor.Instance.mg2disp(defaultValue, valDet.getRange(), valDet.getPIC(), field.getTask().getCompIdx(), false);
    ...
    if (!(StrUtil.rtrim(this._newvalue) === StrUtil.rtrim(this._oldvalue)) || ctrl.ModifiedByUser)
    {
      try
      {
  ...
  private checkAlphaField(): string {
    ...
    if (IsAttrAlpha && UtilStrByteMode.isLocaleDefLangDBCS()) {
      ...
      if (this._newvalue.length > this._picture.length) {
        this._newvalue = NString.Substring(this._newvalue, 0, this._picture.length);
      }
    }
        ...
        if (!FieldValidator.isAlphaPositionalDirective(this._picture.charAt(currPicture)) && this._newvalue.charAt(i) !== this._picture.charAt(currPicture)) {
          ...
          if (FieldValidator.isAlphaPositionalDirective(this._picture.charAt(currPicture)) && this.isPossibleAlphaLetter(i, currPicture)) {
            if (UtilStrByteMode.isLocaleDefLangDBCS()) {
              let strVal: string = NString.Substring(this._newvalue, i, 1);
  ...
  private isPossibleAlphaLetter(indexInput: number, indexPicture: number): boolean {
    ...
    switch (this._picture.charCodeAt(indexPicture)) {
      ...
      case PICInterface.PIC_U:
        let upper: string = NString.ToUpper(this._newvalue[indexInput]);
        if (upper !== this._newvalue[indexInput])
          this.changeStringInside("" + upper, indexInput);
        return true;

      case PICInterface.PIC_L:
        let lower: string = NString.ToLower(this._newvalue[indexInput]);
      ...
      case PICInterface.PIC_N:
        ...
        if (UtilStrByteMode.isDigit(number1) || NChar.IsWhiteSpace(number1.charCodeAt(0)) || UtilStrByteMode.asNumeric(number1))
          return true;
        else
        {
      ...
      default:
        Events.WriteErrorToLog("FieldValidator.isPossibleAlphaLetter: illegal Char Directive: " + NString.IndexOf(this._picture, indexPicture) + " for " + this._newvalue[indexInput]);
        return false;
    }
  }
  ...
  /// </summary>
  /// <returns> String - new value of validated field</returns>
  private checkLogicalField(): string {
    ...
    let newbuffer: StringBuilder = new StringBuilder();

    if (this._newvalue.length > this._picture.length)
      this._newvalue = NString.Substring(this._newvalue, 0, this._picture.length);

    let i: number;
    for (i = 0; i < this._newvalue.length; i = i + 1) {
      if (!this.isLogicPositionalDirective(this._picture.charAt(i))) {
        if (this._newvalue.charAt(i) !== this._picture.charAt(i)) {
          this.printMessage(MsgInterface.STR_RNG_TXT, true);
          return this._oldvalue;
        }
      }
    ...
    if (i < this._picture.length && this.indexOfPositionalNonDirective(NString.Substring(this._picture, i)) !== -1) {
      this._newvalue = this.fillAllAlpha();
    }
    for (i = 0; i < this._newvalue.length; i = i + 1) {
      if (this.isLogicPositionalDirective(this._picture.charAt(i))) {
        newbuffer.Append(this._newvalue.charAt(i));
      }
    }
    this._newvalue = NString.Trim(newbuffer.ToString());
    ...
    if (!(this._newvalue === this._oldvalue)) {
      ...
      let dispvalue: StringBuilder = new StringBuilder();
      let num: number = 0;
      for (i = 0; i < this._picture.length; i = i + 1) {
        if (this.isLogicPositionalDirective(this._picture.charAt(i))) {
          if (num < this._newvalue.length) {
            dispvalue.Append(this._newvalue.charAt(num));
          }
          else {
            dispvalue.Append(' ')/*' '*/;
          }
          ...
          dispvalue.Append(this._picture.charAt(i));
        }
      }
      this._newvalue = NString.Trim(dispvalue.ToString());
    }
    return this._newvalue;
  }
  ...
  /// </summary>
  /// <returns> String - new value of validated field</returns>
  private checkNumericField(): string {
    if (this._newvalue.length === 0) {
      this._newvalue = "0";
    }
    let delimeterRealIndex: number = NString.IndexOf(this._newvalue, this._decimal);
    let delimeterPictIndex: number = NString.IndexOf(this._pictureReal, this._decimal);
      ...
      if (delimeterPictIndex !== -1 && delimeterRealIndex === -1) {
        ...
        if (delimeterPictIndex === -1 && delimeterRealIndex === -1) {
          ...
          if (delimeterRealIndex > 0) {
            this._newvalue = NString.Substring(this._newvalue, 0, delimeterRealIndex);
          }
          else {
            this._newvalue = "0";
          }
    ...
    if ((evaluatedRight.length === 0 && evaluatedLeft.length === 0) || (evaluated.length === 0 && this._newvalue.length > 0))
    {
      ...
      if (dispDefValue == null)
        dispDefValue = DisplayConvertor.Instance.mg2disp(defValue, this._valDet.getRange(), this._valDet.getPIC(), field.getTask().getCompIdx(), false);
    ...
    if (this._valDet.getIsNegative() && (NString.StartsWith(NString.Trim(this._newvalue), "-") || (this._valDet.getNegativeSignPref().length !== 0 && NString.StartsWith(NString.Trim(this._newvalue), this._valDet.getNegativeSignPref())))) {
      evaluated = "-" + evaluated;
    }
    ...
    evaluated = DisplayConvertor.Instance.mg2disp(evaluated, "", this._valDet.getPIC(), this._valDet.getControl().getForm().getTask().getCompIdx(), true);
  ...
  private checkNumericPicture(): void {
    ...
    {
      indexOfDec = NString.IndexOf(this._pictureEnable, "1", currIndx);
      ...
      {
        if (indexOfDec === NString.IndexOf(this._pictureReal, POINT_DEC, currIndx))
        // it's real decimal point and not \. into the picture
        {
          ...
          // the point on the first place .###
          {
            ...
            // only 1 .
              second = NString.Substring(this._pictureReal, indexOfDec + 1);
          }
          else if (indexOfDec === maxLength)
          // the point on the last place ###.
          {
            first = NString.Substring(this._pictureReal, 0 , indexOfDec);
          }
          else
          {
            first = NString.Substring(this._pictureReal, 0 , indexOfDec);
  ...
  private fillNumericByRange(): string {
    ...
    let checkedStr: string = this.setNumericValueWithoutPicture(this._newvalue);
    ...
    if (checkedStr.length === 0)
    {
      ...
      let checkedNum: NUM_TYPE = new NUM_TYPE(this._newvalue, this._valDet.getControl().getPIC(), 0);

      if (discreteRangeValues !== null) {
        for (let i: number = 0; i < discreteRangeValues.Count; i = i + 1) {
      ...
      if (continuousRangeValues !== null) {
        for (let i: number = 0; i < continuousRangeValues.Count; i = i + 1) {
  ...
  private getDoubleVal(val: string): number {
    ...
    if (i > 0) {
      return NNumber.Parse(NString.Substring(val, 0, i));
    }
  ...
  private setNumericValueWithoutPicture(val: string): string {
    ...
    for (let i: number = 0; i < this._pictureReal.length && i < val.length; i++)
    {
      ...
      {
        checkedStr.Append(NString.ToCharArray(NumberFormatInfo.CurrentInfo.NumberDecimalSeparator).toString());
        isFirstDecimal = true;
      }
      else if (currChar === '-' && this._valDet.getIsNegative())
        checkedStr.Append(NString.ToCharArray(NumberFormatInfo.CurrentInfo.NegativeSign).toString());
    }
  ...
  /// </summary>
  /// <returns> String - new value of validated field</returns>
  private checkDateField(): string {

    let buffer: StringBuilder = new StringBuilder(this._picture.length);
    let currValue: number = 0;
    let letters: number = 0; // number of letters in the block
    let isDay: boolean = false; // number inserted to D need be checked after inserting the date
    let isWeek: boolean = false; // number inserted to W need be checked after inserting the date
    let isDoy: boolean = false; // day has picture DDD -> day of year
    let isJpnEraYear: boolean = false; // number inserted to J and YY need be checked after
    ...
    for (i = 0; i < this._newvalue.length; i++)
    {
      ...
      {
        if ((UtilStrByteMode.isDigit(this._newvalue[i])) || (this._newvalue[i] !== ' '))
          break;
      }
    }


    if ((NString.Trim(this._newvalue).length === 0 && this._valDet.getIsZeroFill()) || this._valDet.getControl().isAllBlanks(this._newvalue))
    {
      ...
      this._newvalue = DisplayConvertor.Instance.mg2disp(this._newvalue, "", this._valDet.getPIC(), this._valDet.getControl().getForm().getTask().getCompIdx(), false);
    }
    else if (i === this._newvalue.length)
    {
    ...
    if (this.isNumberAllowedInDate() && this.isAllNumbers0())
    // && isYearPartOfDate())
    {
      // all date is 0 and need to be zero filled
      // there is no Range or the 0 value is in Range
      if (((this._valDet.getContinuousRangeValues() != null || this._valDet.getDiscreteRangeValues() != null) && this.isDateLegal4Range(this._valDet.getContinuousRangeValues(),
          this._valDet.getDiscreteRangeValues(), 0)) || (this._valDet.getContinuousRangeValues() == null && this._valDet.getDiscreteRangeValues() == null))
      {
        if (this._valDet.getIsZeroFill() || this.isAllNumbers0())
        {
          let add: string = '0'; // isAllNumbers0(), default
          if (this._valDet.getIsZeroFill())
            add = this._valDet.getZeroFillChar();
          ...
          if (!this._valDet.getIsZeroFill())
          {
            for (i = 0; i < this._picture.length; i++)
            {
              if (this.isDataPositionalDirective(this._picture[i]))
                buffer.Append(add);
              else
                buffer.Append(this._picture[i]);
            }
          }
          return buffer.ToString();
        }
    ...
    let minLen: number = 0;
    if (UtilStrByteMode.isLocaleDefLangJPN())
      minLen = UtilStrByteMode.getMinLenPicture(NString.TrimEnd(this._newvalue), this._picture);
    else
      minLen = Math.min(NString.TrimEnd(this._newvalue).length, this._picture.length);

    for (i = 0; i < minLen && (!this.hebrew || currValue < minLen); i++)
    {
      if (this.isDataPositionalDirective(this._picture[i]))
      {
        switch (this._picture.charCodeAt(i))
        {
        case PICInterface.PIC_DD:
          currValue = this.getEndOfNumericBlock(currValue);
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_DD);
          isDay = true;
          i++;
          break;
        ...
        case PICInterface.PIC_DDD:
          currValue = this.getEndOfNumericBlock(currValue);
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_DDD);
          isDay = true;
          isDoy = true;
          i += 2;
          break;
        ...
        case PICInterface.PIC_DDDD:
          currValue = this.getEndOfNumericBlock(currValue);
          let bufferDate: StringBuilder  = new StringBuilder();
          currValue = this.add2Date(currValue, bufferDate, PICInterface.PIC_DDDD);
          bufferDate.Append(this.addDateSuffix(bufferDate.ToString()));
          let count: number = 4 - bufferDate.Length;
          while (--count > 0)
            buffer.Append(' ');
        ...
        case PICInterface.PIC_MMD:
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_MMD);
          i++;
          break;

        case PICInterface.PIC_MMM:
          // get size of PIC_MMM block
          letters = this.getSizeOfBlock(i, PICInterface.PIC_MMM);
          currValue = this.addM2Date(currValue, letters, buffer, PICInterface.PIC_MMM);
          i += letters - 1;
          break;

        case PICInterface.PIC_YY:
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_YY);
          i++;
          break;

        case PICInterface.PIC_YYYY:
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_YYYY);
          i += 3;
          break;

        case PICInterface.PIC_W:
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_W);
          isWeek = true;
          break;

        case PICInterface.PIC_WWW:
          letters = this.getSizeOfBlock(i, PICInterface.PIC_WWW);
          currValue = this.addM2Date(currValue, letters, buffer, PICInterface.PIC_WWW);
          isWeek = true;
          i += letters - 1;
          break;
        ...
        case PICInterface.PIC_HDD:
          letters = this.getSizeOfBlock(i, this._picture.charCodeAt(i));
          currValue = HebrewDate.add2Date(this._newvalue, currValue, letters, buffer);
          i += letters - 1;
          break;

        case PICInterface.PIC_BB:  // JPN: Japanese date picture support
          letters = this.getSizeOfBlock(i, PICInterface.PIC_BB);
          currValue = this.addM2Date(currValue, letters / 2, buffer, PICInterface.PIC_BB);
        ...
        case PICInterface.PIC_JY1:  // JPN: Japanese date picture support
          ...
          currValue = this.addM2Date(currValue, letters, buffer, PICInterface.PIC_JY1);
          isJpnEraYear = true;
          break;

        case PICInterface.PIC_JY2:  // JPN: Japanese date picture support
          letters = 1; // a number of characters in newvalue, not a number of bytes
          currValue = this.addM2Date(currValue, letters, buffer, PICInterface.PIC_JY2);
          isJpnEraYear = true;
          i++;
          break;
        ...
        case PICInterface.PIC_JY4:  // JPN: Japanese date picture support
          ...
          currValue = this.addM2Date(currValue, letters, buffer, PICInterface.PIC_JY4);
          isJpnEraYear = true;
          i += 3;
          break;

        case PICInterface.PIC_YJ:  // JPN: Japanese date picture support
          currValue = this.add2Date(currValue, buffer, PICInterface.PIC_YJ);
          isJpnEraYear = true;
          i++;
          break;
        }
      ...
      {
        buffer.Append(this._picture[i]);
        if (NString.IndexOf(this._newvalue, "" + this._picture[i], currValue) !== -1)
        // try to find the letter in newvalue
        {
          currValue = NString.IndexOf(this._newvalue, "" + this._picture[i], currValue) + 1;
        }
      }
    }
    ...
    // range date
    if (this._valDet.getContinuousRangeValues() != null || this._valDet.getDiscreteRangeValues() != null)
      buffer = this.fillDateByRange(this._valDet.getContinuousRangeValues(), this._valDet.getDiscreteRangeValues(), buffer.ToString());

    if (isJpnEraYear)
    // JPN: Japanese date picture support
      buffer = this.checkJpnEraYear(buffer.ToString());

    if (isDay)
    // D - day value in picture need be checked, if it exists
      buffer = this.checkDay(buffer.ToString(), isDoy);
    if (isWeek)
    // W - week value in picture need be checked, if it exists
      buffer = this.checkWeek(buffer.ToString());

    // BUG: 908424
    // check for wrong dates like 0000/01/02
    // for this reason the vlue must be evaluated
    let evaluatedValue: string = buffer.ToString();
    // currentlly used picture
    let pic: PIC = this._valDet.getPIC();
    let innerValue: number = DisplayConvertor.Instance.a_2_date_pic_datemode(NString.ToCharArray(evaluatedValue), evaluatedValue.length, pic, NString.ToCharArray(pic.getMask()),
      this._valDet.getControl().getForm().getTask().getCompIdx());
  ...
  private addM2Date(currValue: number, letters: number, buffer: StringBuilder, type: number): number {
      ...
      if (type === PICInterface.PIC_WWW || type === PICInterface.PIC_W) {
        ...
        if (type === PICInterface.PIC_BB) { // JPN: Japanese date picture support
          ...
          if (type === PICInterface.PIC_JY1 || type === PICInterface.PIC_JY2 || type === PICInterface.PIC_JY4)
          {
            ...
            let text: string = NString.Substring(this._newvalue, currValue, letters);
            buffer.Append(text);
    ...
    let max: number = this.getMonthOrWeek(NString.Substring(this._newvalue, currValue), letters, type);

    if (NString.Trim(array[max]).length >= letters) {
      buffer.Append(NString.Substring(NString.Trim(array[max]), 0, letters));
    }
    else {
      buffer.Append(NString.Trim(array[max]));
      for (let i: number = letters - NString.Trim(array[max]).length; i > 0; i = i - 1) {
        buffer.Append(' ')/*' '*/;
      }
    }
    return currValue + letters;
  ...
  private getMonthOrWeek(month_str: string, letters: number, type: number): number {
    ...
    if (this.hebrew)
    {
      let hstr: string = NString.Trim(NString.Substring(NString.TrimStart(month_str, ' '), 0, letters).toString());

      for (let i: number = 1; i < array.length; i++)
      // start checking from 1st member, because 0 = ""
      {
        if (NString.Trim(array[i]) === hstr)
        {
          if (i === 14)
            i = 6;
          return i;
    ...
    try {
        ...
        let str: string = NString.Trim(NString.Substring(month_str, 0, letters));
    ...
    // start checking from 1st member, because 0 = ""
    {
      ...
      {
        if (NString.ToLower(array[tmp])[i] === NString.ToLower(month_str)[from])
          found[tmp]++;
        else
          break;
      }
  ...
  private checkCentury(yearIn: number): StringBuilder {
    ...
    let century: number = start_century / 100;
  ...
  private isDateLegal4Range(ContinuousRange: List<string>, DiscreteRange: List<string>, currDate: number): boolean {
    ...
    if (ContinuousRange != null)
    {
      ...
      {
        downDate = DisplayConvertor.Instance.a_2_date_pic(ContinuousRange[i], pic, pic.getMask(), this._valDet.getControl().getForm().getTask().getCompIdx());
        topDate = DisplayConvertor.Instance.a_2_date_pic(ContinuousRange[++i], pic, pic.getMask(), this._valDet.getControl().getForm().getTask().getCompIdx());
    ...
    if (DiscreteRange != null)
    {
      ...
      {
        if (currDate === DisplayConvertor.Instance.a_2_date_pic(DiscreteRange[i], pic, pic.getMask(), this._valDet.getControl().getForm().getTask().getCompIdx()))
          return true;
      }
    }
  ...
  private fillDateByRange(ContinuousRange: List<string>, DiscreteRange: List<string>, buffer: string): StringBuilder {
    let pic: PIC = this._valDet.getPIC();
    let i: number;
    let downDate: number;
    let topDate: number;
    let currDate = DisplayConvertor.Instance.a_2_date_pic(buffer, pic, pic.getMask(), this._valDet.getControl().getForm().getTask().getCompIdx());
  ...
  private getDateElement(date: string, element: DateElement): number {
    ...
    for (let i: number = 0; i + intPicIdxOfs < minLen && !exitLoop; i++)
    {
      if (this.isDataPositionalDirective(this._picture[i + intPicIdxOfs]))
      {
        ...
        case PICInterface.PIC_DD:
          ...
          {
            buffer = NString.Substring(date, i, 2);
            exitLoop = true;
            break;
          }
          i++;
        ...
        case PICInterface.PIC_DDD:
          if (element === DateElement.DAY)
          {
            buffer = NString.Substring(date, i, 3);
            exitLoop = true;
            break;
          }
          i += 2;
        ...
        case PICInterface.PIC_DDDD:
          if (element === DateElement.DAY)
          {
            buffer = NString.Substring(date, i, 2); // not +4 need not suffix of 'nd', 'rd', 'st'
            exitLoop = true;
          }
          i += 3;
          break;
        ...
        case PICInterface.PIC_MMM:
          ...
          if (element === DateElement.MONTH)
          {
            buffer = NString.Substring(date, i, letters);
            exitLoop = true;
            break;
          }
          i += letters - 1;
        ...
        case PICInterface.PIC_YYYY:
          if (element === DateElement.YEAR)
          {
            buffer = NString.Substring(date, i, 4);
            exitLoop = true;
          }
          i += 3;
          break;
        ...
        case PICInterface.PIC_W:
          ...
          {
            buffer = NString.Substring(date, i, 1);
            exitLoop = true;
          }
          break;
        ...
        case PICInterface.PIC_BB:  // JPN: Japanese date picture support
          ...
          {
            buffer = NString.Substring(date, i, (letters / 2));
    ...
    buffer = NString.Trim(buffer.toString());
    ...
    {
      while (NString.StartsWith(buffer.toString(), "0") || NString.StartsWith(buffer.toString(), " "))
        buffer = NString.Substring(buffer.toString(), 1);
      if (buffer.length === 0)
        letters = 0;
      else
  ...
  private changeDow(date: string, dow: number): string {
    ...
    for (let i: number = 0; i + intPicIdxOfs < this._picture.length && !isDate; i++)
    {
      if (this.isDataPositionalDirective(this._picture[i + intPicIdxOfs]))
      {
        ...
        case PICInterface.PIC_W:
          startPart = NString.Substring(date, 0, i);
          finishPart = NString.Substring(date, i + 1);
        ...
        case PICInterface.PIC_BB:  // JPN: Japanese date picture support
          ...
          startPart = NString.Substring(date, 0, i);
          finishPart = NString.Substring(date, i + (letters / 2));
  ...
  private checkTimeField(): string {
    ...
    for (let i: number = 0; i < this._picture.length; i++)
    {
      if (this.isTimePositionalDirective(this._picture[i]))
      {
        switch (this._picture.charCodeAt(i))
        {
        case PICInterface.PIC_HH:
          currValue = this.add2Time(currValue, buffer, PICInterface.PIC_HH);
          hour = NNumber.Parse(NString.Substring(buffer.ToString(), buffer.Length - 2));
          break;

        case PICInterface.PIC_MMT:
          currValue = this.add2Time(currValue, buffer, PICInterface.PIC_MMT);
          break;

        case PICInterface.PIC_SS:
          currValue = this.add2Time(currValue, buffer, PICInterface.PIC_SS);
          break;

        case PICInterface.PIC_MS:
          currValue = this.add2Time(currValue, buffer, PICInterface.PIC_MS);
          break;

        case PICInterface.PIC_PM:
          currValue = this.addPM2Time(currValue, buffer);
          if (NString.ToUpper(NString.Substring(buffer.ToString(), buffer.Length - 2)) === "PM")
            hour += 12;
          break;
        }
    ...
    // range of the time
    if (this._valDet.getContinuousRangeValues() != null || this._valDet.getDiscreteRangeValues() != null)
      buffer = this.fillTimeByRange(this._valDet.getContinuousRangeValues(), this._valDet.getDiscreteRangeValues(), buffer.ToString());
  ...
  private addPM2Time(currValue: number, buffer: StringBuilder): number {
    ...
    if ((i = NString.IndexOf(NString.ToUpper(this._newvalue), "AM", currValue)) !== -1)
    {
      buffer.Append("am");
      return i + 2;
    }
    ...
    {
      if ((i = NString.IndexOf(NString.ToUpper(this._newvalue), "PM", currValue)) !== -1)
      {
        buffer.Append("pm");
        return i + 2;
      }
  ...
  private fillTimeByRange(ContinuousRange: List<string>, DiscreteRange: List<string>, buffer: string): StringBuilder {
    let  pic: PIC = this._valDet.getPIC();
    let  currTime: number = DisplayConvertor.Instance.a_2_time(buffer, pic, false);
  ...
  private changeStringInside(to: string, index: number): void {
    this._newvalue = NString.Substring(this._newvalue, 0, index) + to + NString.Substring(this._newvalue, index + to.length);
  }
  ...
  private init(validationDetails: ValidationDetails): void {
    ...
    if (!(pIC === null || pIC.getAttr() === StorageAttribute.BLOB || pIC.getAttr() === StorageAttribute.BLOB_VECTOR)) {
      ...
      this._picture = NString.Substring(this._picture, 0, pIC.getMaskSize());
    }
  }
  ...
  private digitsBeforeDelimeter(inputString: string, searchStart: number, delimeter: string): number {
    ...
    let searchEndPos: number = NString.IndexOf(inputString, delimeter, searchStart);
