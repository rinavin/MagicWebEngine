...
export class DisplayConvertor {
  ...
  /// </returns>
  mg2disp(mgValue: string, rangeStr: string, pic: PIC, compIdx: number, time_date_pic_Z_edt: boolean): string;
  mg2disp(mgValue: string, rangeStr: string, pic: PIC, alwaysFill: boolean, compIdx: number, time_date_pic_Z_edt: boolean): string;
  mg2disp(mgValue: string, rangeStr: string, pic: PIC, alwaysFill: boolean, compIdx: number, convertCase: boolean, time_date_num_pic_Z_edt: boolean): string;
  mg2disp(mgValue: string, rangeStr: string, pic: PIC, compIdxOrAlwaysFill: any, time_date_pic_Z_edtOrCompIdx: any, time_date_pic_Z_edtOrConvertCase?: boolean, time_date_num_pic_Z_edt?: boolean): string {
    if (arguments.length === 5 && (mgValue === null || mgValue.constructor === String) && (rangeStr === null || rangeStr.constructor === String) && (pic === null || pic instanceof PIC) && (compIdxOrAlwaysFill === null || compIdxOrAlwaysFill.constructor === Number) && (time_date_pic_Z_edtOrCompIdx === null || time_date_pic_Z_edtOrCompIdx.constructor === Boolean)) {
      return this.mg2disp_0(mgValue, rangeStr, pic, compIdxOrAlwaysFill, time_date_pic_Z_edtOrCompIdx);
    }
    if (arguments.length === 6 && (mgValue === null || mgValue.constructor === String) && (rangeStr === null || rangeStr.constructor === String) && (pic === null || pic instanceof PIC) && (compIdxOrAlwaysFill === null || compIdxOrAlwaysFill.constructor === Boolean) && (time_date_pic_Z_edtOrCompIdx === null || time_date_pic_Z_edtOrCompIdx.constructor === Number) && (time_date_pic_Z_edtOrConvertCase === null || time_date_pic_Z_edtOrConvertCase.constructor === Boolean)) {
      return this.mg2disp_1(mgValue, rangeStr, pic, compIdxOrAlwaysFill, time_date_pic_Z_edtOrCompIdx, time_date_pic_Z_edtOrConvertCase);
    }
    return this.mg2disp_2(mgValue, rangeStr, pic, compIdxOrAlwaysFill, time_date_pic_Z_edtOrCompIdx, time_date_pic_Z_edtOrConvertCase, time_date_num_pic_Z_edt);
  }

  private mg2disp_0(mgValue: string, rangeStr: string, pic: PIC, compIdx: number, time_date_pic_Z_edt: boolean): string {
    return this.mg2disp(mgValue, rangeStr, pic, false, compIdx, time_date_pic_Z_edt);
  }

  private mg2disp_1(mgValue: string, rangeStr: string, pic: PIC, alwaysFill: boolean, compIdx: number, time_date_pic_Z_edt: boolean): string {
    return this.mg2disp(mgValue, rangeStr, pic, alwaysFill, compIdx, false, time_date_pic_Z_edt);
  ...
  /// <returns> string to display
  /// </returns>
  private mg2disp_2(mgValue: string, rangeStr: string, pic: PIC, alwaysFill: boolean, compIdx: number, convertCase: boolean, time_date_num_pic_Z_edt: boolean): string {
    let str: string = "";
    ...
    switch (pic.getAttr()) {
      ...
      case StorageAttribute.BOOLEAN:
        str = this.fromLogical(mgValue, pic, tmpRange);
        break;
  ...
  /// </returns>
  disp2mg(dispValue: string, rangeStr: string, pic: PIC, compIdx: number, blobContentType: string): string {
    let str: string = "";
    ...
    switch (pic.getAttr()) {
      ...
      case StorageAttribute.NUMERIC: {
        ...
        if (pic.isNegative() && NString.StartsWith(NString.Trim(dispValue), "-"))
      ...
      case StorageAttribute.DATE:
        str = this.toDate(dispValue, pic, compIdx);
        break;


      case StorageAttribute.TIME:
        str = this.toTime(dispValue, pic);
        break;


      case StorageAttribute.BOOLEAN:
        str = this.toLogical(dispValue, pic, rangeStr);
        break;
    ...
    if (pic.getMaskChars() > 0) {
      ...
      // JPN: DBCS support
      {
        ...
        if (convertCase) {
          ...
          resStr = NString.Substring(dataStr, 0, min_len);
        }
      }
    }
  ...
  private caseConvertedChar(data: string, mask: string): string {
    ...
    if (mask.charCodeAt(0) === PICInterface.PIC_U) {
      ch = NChar.ToUpper(data);
    }
    else {
      if (mask.charCodeAt(0) === PICInterface.PIC_L) {
        ch = NChar.ToLower(data);
      }
      else {
        ch = data;
      }
  ...
  private win_data_cpy(maskStr: any, maskLen: number, dataStr: any, isAttrAlpha: boolean, convertCase?: boolean): string {
    if (arguments.length === 4 && (maskStr === null || maskStr instanceof Array) && (maskLen === null || maskLen.constructor === Number) && (dataStr === null || dataStr instanceof Array) && (isAttrAlpha === null || isAttrAlpha.constructor === Boolean)) {
  ...
  private win_data_cpy_1(maskStr: string[], maskLen: number, dataStr: string[], isAttrAlpha: boolean, convertCase: boolean): string {
    ...
    if (isAttrAlpha && UtilStrByteMode.isLocaleDefLangDBCS()) {
      ...
      return NString.Substring(NString.FromChars(resStr), 0, num2);
    }
  ...
  to_a_pic_datemode(outStr: string, out_len: number, date: number, pic: PIC, date_pic_Z_edt: boolean, ignore_dt_fmt: boolean, compIdx: number): string {
    ...
    let hyear: number = 0;
    let hmonth: number = 0;
    let leap: boolean = false; // Used only for hebrew
    ...
    if (pic.getMaskSize() > out_len) {
      ...
      let j: number = out_len = pic.getMaskSize();
      ...
      if (pic.isHebrew() && (date === 0 || <number>date === 1000000000)) {
        for (i = 0; i < out_len; i = i + 1) {
          outVal[i] = ' '/*' '*/;
        }
        return NString.FromChars(outVal);
    ...
    // Break data into its components
      let dateBreakParams: DateBreakParams = new DateBreakParams(dow, year, len, day, doy);
      this.date_break_datemode(dateBreakParams, date, ignore_dt_fmt, compIdx);
      ...
      dow = dateBreakParams.year;
      year = dateBreakParams.month;
      len = dateBreakParams.day;
      day = dateBreakParams.doy;
      doy = dateBreakParams.dow;
      ...
      while (j > 0) {
        ...
        case PICInterface.PIC_MMM:
          ...
          let dateNameParams: DateNameParams = new DateNameParams(outVal, outIdx, len);
  ...
  date_break_datemode(breakParams: DateBreakParams, date: number, ignore_dt_fmt: boolean, compIdx: number): void {
    ...
    if (date <= 0) {
      ...
      dow = date % 7 + 1;
      date = date - 1;
      let cent4: number = date / PICInterface.DAYSINFOURCENT;
      date = date - cent4 * PICInterface.DAYSINFOURCENT;
      let cent: number = date / PICInterface.DAYSINCENTURY;
      if (cent > 3) {
        cent = 3;
      }
      date = date - cent * PICInterface.DAYSINCENTURY;
      let year4: number = date / PICInterface.DAYSINFOURYEAR;
  ...
  private int_2_a(str: string[], strPos: number, len: number, n: number, lead: string): number {
    ...
    do {
      str[strPos + (pos = pos - 1)] = (48 + n % 10).toString();
      n = n / 10;
    }
    while (pos > 0 && n > 0);
  ...
  private lib_a_fill(str: string[], strPos: number, len: number, pos: number, lead: number): number {
    ...
    if (lead === 0) {
      ...
      if (pos > 0) {
        DisplayConvertor.char_memset(str, strPos, lead.toString(), pos);
      }
    }
    return len;
  }
  ...
  private date_i_2_nm_bytemode(strOut: string, dateNameParams: DateNameParams, strNm: string, intOutLen: number): string {
    ...
    strWork = NString.Substring(strWork, 0, dateNameParams.outIdx);
  ...
  /// <returns> the magic number</returns>
  toDate(dispValue: string, pic: PIC, compIdx: number): string {

    let mgNum: NUM_TYPE = new NUM_TYPE();

    if (UtilStrByteMode.isLocaleDefLangJPN()) {
      dispValue = NString.TrimEnd(dispValue, null);
    }
    else {
      dispValue = NString.Trim(dispValue);
    }
    ...
    let tmp: string = dispValue + "       ";
    let maskStr: string = pic.getMask();
    let numVal: number = this.a_2_date_pic_datemode(NString.ToCharArray(tmp), dispValue.length, pic, NString.ToCharArray(maskStr), compIdx);
    ...
    while (pos < str_len && pos + intPicIdxOfs < len) {
      ...
      switch (mask[pos + intPicIdxOfs].charCodeAt(0)) {
        ...
        case (PICInterface.PIC_YJ): // JPN: Japanese date picture support
          ...
          let str1: NString = new NString(str);
          era_year = UtilDateJpn.getInstance().getStartYearOfEra(str1.toString(), pic.getMask());
  ...
  private date_MMM_2_m(str: string[], mask: string[], pos: IntRef, len: number, isHebrew: boolean): number {
    ...
    if (len === 0) {
      ...
      for (let i: number = 1; i < monthNames.length; i = i + 1) {
        if (NString.Compare(NString.Substring(monthNames[i], 0, len), strB, true) === 0)
          return i;
      }
    }
    return 0;
  ...
  date_4_calender(year: number, month: number, day: number, doy: number, usedoy: boolean): number {
    ...
    year = year - 1;
    let num: number = year / 400;
    year = year - num * 400;
    let num2: number = year / 100;
    year = year - num2 * 100;
    let num3: number = year / 4;
    year = year - num3 * 4;
  ...
  /// <param name = "picStr">      The picture to use</param>
  /// <returns> the magic number</returns>
  toTime(dispValue: string, pic: PIC): string {
    let mgNum: NUM_TYPE = new NUM_TYPE();
    dispValue = NString.Trim(dispValue);

    let numVal: number = this.a_2_time(dispValue, pic, false);
    mgNum.NUM_4_LONG(numVal);
    return mgNum.toXMLrecord();
  }
  ...
  a_2_time(Ascii: any, picOrAsciiL: any, milliSecondsOrMask: any, milliSeconds?: boolean): number {
    if (arguments.length === 3 && (Ascii === null || Ascii.constructor === String) && (picOrAsciiL === null || picOrAsciiL instanceof PIC) && (milliSecondsOrMask === null || milliSecondsOrMask.constructor === Boolean)) {
  ...
  private a_2_time_0(Ascii: string, pic: PIC, milliSeconds: boolean): number {
    return this.a_2_time(NString.ToCharArray(Ascii), Ascii.length, NString.ToCharArray(pic.getMask()), milliSeconds);
  }
  ...
  private a_2_time_1(Ascii: string[], AsciiL: number, mask: string[], milliSeconds: boolean): number {
    ...
    while (I < len) {
      ...
      switch (mask[I].charCodeAt(0)) {
        case (PICInterface.PIC_HH):
        {
          let str: NString = new NString(Ascii, I, maskLength);
          hour = mgNum.a_2_long(str.toString());
        }
        break;
        ...
        {
          let str: NString = new NString(Ascii, I, maskLength);
          minute = mgNum.a_2_long(str.toString());
        }
        break;
        ...
        {
          let str: NString = new NString(Ascii, I, maskLength);
          second = mgNum.a_2_long(str.toString());
        }
        break;
        ...
        {
          let str: NString = new NString(Ascii, I, maskLength);
          millisecond = mgNum.a_2_long(str.toString());
        }
        break;
  ...
  static toBoolean(boolChar: string): boolean;
  static toBoolean(boolStrOrBoolChar: any): boolean {
    if (arguments.length === 1 && (boolStrOrBoolChar === null || boolStrOrBoolChar.constructor === String)) {
      return DisplayConvertor.toBoolean_0(boolStrOrBoolChar);
    }
    return DisplayConvertor.toBoolean_1(boolStrOrBoolChar);
  }
  ...
  /// <returns> true if "1" or false if others
  /// </returns>
  private static toBoolean_0(boolStr: string): boolean {
  ...
  /// <returns> true if '1' or false if others
  /// </returns>
  private static toBoolean_1(boolChar: string): boolean {
    return boolChar === '1'/*'1'*/;
  }
  ...
  private disp_2_logical(dispValue: string, mask: string, rangeStr: string): string {
    ...
    let i: number = NString.IndexOf(rangeStr, ',');
    let trueStr: string = NString.Substring(rangeStr, 0, i);
    ...
    if (trueStr.length < val.length) {
      val = NString.Substring(val, 0, trueStr.length);
    }
    else {
      if (val.length < trueStr.length) {
        trueStr = NString.Substring(trueStr, 0, val.length);
      }
    }
    return (NString.ToUpper(val) === NString.ToUpper(trueStr)) ? "1" : "0";
  }
    ...
    for (currPicture = i = 0; i < dispValue.length && currPicture < picture.length; i++, currPicture++)
    {
      if (FieldValidator.isAlphaPositionalDirective(picture.charAt(currPicture))) {
        ...
        if (IsAlphaDBCS) {
          ...
          let strDspValOneChar: string = NString.Substring(dispValue, i, 1);
  ...
  fillAlphaByDiscreteRangeValues(discreteRangeValues: List<string>, newValue: string): string {
    ...
    if (discreteRangeValues !== null) {
      for (let i: number = 0; i < discreteRangeValues.Count; i = i + 1) {
        ...
        let truncatedValue: string = NString.Substring(discreteValue, 0, Math.min(newValue.length, discreteValue.length));
        if (NString.ToUpper(newValue) === NString.ToUpper(truncatedValue)) {
  ...
  completeAlphaByRange(discreteRangeValues: List<string>, newValue: string): string {
    ...
    while (caseLetters === CHECK_CASE || caseLetters === IGNORE_CASE) {
      ...
      if (caseLetters === CHECK_CASE) {
        ...
        lowerValue = NString.ToLower(newValue);
      }
      ...
      for (let i: number = 0; i < discreteRangeValues.Count; i = i + 1) {
        ...
        if (caseLetters === CHECK_CASE) {
          ...
          rangeItem = NString.ToLower(discreteRangeValues.get_Item(i));
        }
