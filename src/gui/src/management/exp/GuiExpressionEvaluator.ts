...
export abstract class GuiExpressionEvaluator {
    ...
    if (val1.MgNumVal === null || val2.StrVal === null) {
      ...
      if (!(!val1.MgNumVal.num_is_zero() && parent === 0)) {
        ...
        if (((parent >= 0 && parent < currTask.GetTaskDepth()) || parent === GuiExpressionEvaluator.TRIGGER_TASK) && len === 1) {
          ...
          let s: string = NChar.ToUpper(temp.charAt(0));
          let tsk: TaskBase = this.GetContextTask(parent);
  ...
  eval_op_PixelsToFormUnits(pixels: ExpVal, isX: ExpVal, resVal: ExpVal): void {
    ...
    let uom: number = currentForm.pix2uom(pix, isXcoordinate);
  ...
  eval_op_FormUnitsToPixels(uom: ExpVal, isX: ExpVal, resVal: ExpVal): void {
    ...
    let pix: number = form.uom2pix(inUOM, isXcoordinate);
  ...
  eval_op_ColorSet(colorNumber: ExpVal, foregroundColor: ExpVal, backgroundColor: ExpVal, resVal: ExpVal): void {
    let index: number = colorNumber.MgNumVal.NUM_2_LONG();
    let foreColor: number = NNumber.Parse(foregroundColor.StrVal, NumberStyles.HexNumber);
    let backColor: number = NNumber.Parse(backgroundColor.StrVal, NumberStyles.HexNumber);
    resVal.Attr = StorageAttribute.BOOLEAN;
    resVal.BoolVal = Manager.GetColorsTable().SetColor(index, foreColor, backColor);
    let boolVal: boolean = resVal.BoolVal;
    if (boolVal) {
      let form: MgFormBase = (<TaskBase>this.ExpTask.GetContextTask()).getForm().getTopMostForm();
      if (form !== null) {
  ...
  eval_op_FontSet(fontNumberVal: ExpVal, fontNameVal: ExpVal, sizeVal: ExpVal, scriptCodeVal: ExpVal, orientationVal: ExpVal, boldVal: ExpVal, italicVal: ExpVal, strikeVal: ExpVal, underlineVal: ExpVal, resVal: ExpVal): void {
    let index: number = fontNumberVal.MgNumVal.NUM_2_LONG();
    let font: string = fontNameVal.StrVal;
    let size: number = sizeVal.MgNumVal.NUM_2_LONG();
    let scriptCode: number = scriptCodeVal.MgNumVal.NUM_2_LONG();
    let orientation: number = orientationVal.MgNumVal.NUM_2_LONG();
    let bold: boolean = boldVal.BoolVal;
    let italic: boolean = italicVal.BoolVal;
    let strike: boolean = strikeVal.BoolVal;
    let underline: boolean = underlineVal.BoolVal;

    resVal.Attr = StorageAttribute.BOOLEAN;
    resVal.BoolVal = Manager.GetFontsTable().SetFont(index, font, size, scriptCode, orientation, bold, italic, strike, underline);

    if (resVal.BoolVal) {
      let topMostForm: MgFormBase = (<TaskBase>this.ExpTask.GetContextTask()).getForm().getTopMostForm();
      if (topMostForm !== null) {
        topMostForm.UpdateFontValues();
      }
    }
  }
